...
strain.BivData.Table <- ddply(mouse.level.BivData_table, c("strain", "sex"), summarise,
                        ...
                        Nmice = length(Nmice),
                        Ncells  = sum(Ncells), #might not to be sum
                        mean.SC = mean(mean_SC),
                        var.SC = mean(var), #mouse average variance of bivalent per mouse
                        
                        #chrm classes
                        av.proportion_0CO = mean(proportion_0CO),
                        av.proportion_1CO = mean(proportion_1CO),
                        av.proportion_2CO = mean(proportion_2CO),
                        av.proportion_3CO = mean(proportion_3CO),
                        
                        #1CO pos
                        mean.1CO.pos = mean(mean_1CO.pos),
                        
                        #mean.IFD
                        mean.IFD.ABS = mean(mean_IFD.2CO_ABS),
                        mean.IFD.PER = mean(mean_IFD.2CO_PER)
                        )
...
#this dataframe doesn't work

#don't run here, run in the MS file
#DF.HetC.MixedModel.HQ
#DF.HetC.MixedModel.HQ <- subset()
...
```{r Female.specific.glm, echo=FALSE, eval=TRUE}
...
```{r Male.specific.glm, echo=FALSE, eval=TRUE}

#these are M4
#M4
MLH1.male.glm.M4 <- glm(mean_co ~ subsp * strain, data= male_mouse.avs_for.analysis)
sum_MLH1.male.glm.M4 <- summary(MLH1.male.glm.M4)
#round(summry.male.strain.effects$coefficients[,c(1,4)], 5)

#sum_MLH1.male.specific.glm.M2$coefficients[5,][4]
...
#M4.2
MLH1.male.glm.M4.2 <- glm(mean_co ~ strain, data= male_mouse.avs_for.analysis)
...
```{r totSC_setup, echo=FALSE, eval = F}

#Total.SC_DF is the main dataframe and should be set up cleanly in the SetUpData.Rmd
#SC.skel.OG <- SC.skel #5683

tot.sc.x.list <- Total.SC_DF %>% filter(X == "X") 

Total.SC_DF.og <- Total.SC_DF[! Total.SC_DF$Random.Name %in% tot.sc.x.list$Random.Name, ]
#quality is still a factor
str(Total.SC_DF.og)
Total.SC_DF.og$quality <- as.numeric(Total.SC_DF.og$quality)

Total.SC_DF.og <- Total.SC_DF.og %>% filter(quality < 5)
...
-- The total_SC outliers should be removed in the total.SC RData script
...
totSC.strain.av.table <- ddply(.data=mouse.total.skel, 
                 .(sex,strain),
                 summarize, 
                 ncells = sum(ncells),
                 nmice = length(unique(mouse)),
...
# Q1 single bivalent section


```{r Q1.single.biv_position_table, echo=FALSE, eval=FALSE}
#strain averages for single bivalent 
#
#colnames(Curated_BivData99)
...
#unordered for the mixed models (move this to the MM blocks...?)
Short.biv_mouse_table$subsp <- factor(Long.biv_mouse_table$subsp, ordered = FALSE)
Short.biv_mouse_table$sex <- factor(Long.biv_mouse_table$sex, ordered = FALSE)
Short.biv_mouse_table$strain <- factor(Long.biv_mouse_table$strain, ordered = FALSE)
...
#1) Long Biv for 
#remove all the non-used strains!!
#Short.biv_mouse_table_HetC <- Short.biv_mouse_table

#Short.biv_mouse_table_HetC <- Short.biv_mouse_table_HetC[Long.biv_mouse_table$strain != "AST",]

#Short.biv_mouse_table_HetC <- Short.biv_mouse_table_HetC[Curated_BivData$strain != "TOM",]

#Short.biv_mouse_table_HetC <- Short.biv_mouse_table_HetC[Long.biv_mouse_table_HetC$strain != "CZECH",]
...
```{r Q2_tot.sc_plot, echo=FALSE, warning=FALSE, eval=FALSE}
...
```
The molf male t-tests don't work because I only have total Sc measures for 1 MOLF male. to of the todo list if I get back to lab.
...
#all of these are M4
#mouse table SC
#mouse.level.BivData_table .. mouse.avs_4MM
...
t.test_mouse.av.IFD_ABS_MOL <- t.test(  (mouse.level.BivData_table %>% filter(Rec.group == "1") %>% filter(sex == "male") %>% filter(subsp == "Mol") %>% filter(species == "M.musculus"))$mean_IFD.2CO_ABS,
   (mouse.level.BivData_table %>% filter(Rec.group == "0") %>% filter(sex == "male") %>% filter(subsp == "Mol") %>% filter(species == "M.musculus") )$mean_IFD.2CO_ABS  )
```
...
#save Rdata
save.image("~./MLH1repo/data/Results_setup_5.21.20.RData")
