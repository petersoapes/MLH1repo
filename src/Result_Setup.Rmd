...
```{r Q1.rbar, echo=FALSE, eval=FALSE}
...
#no mice removed for low cell numbers?
AP_mouse_table.HQ <- add_species(AP_mouse_table.HQ)

#DF.HetC.MixedModel <- add_species(DF.HetC.MixedModel)
DF.HetC.MixedModel <- AP_mouse_table.HQ[AP_mouse_table.HQ$species == "M.musculus",]
#DF.HetC.MixedModel
...
strain.BivData.Table <- ddply(mouse.BivData.Table, c("strain", "sex"), summarise,
...
```{r Q1.biv.data_table.4MM.Male, echo=FALSE, eval=TRUE}
...
#these are M4
#M4
MLH1.male.glm.M4 <- glm(mean_co ~ subsp * strain, data= just.male_mouse.av)
...
#M4.2
MLH1.male.glm.M4.2 <- glm(mean_co ~ strain, data= just.male_mouse.av)
...
```{r M2.MLH1.var, echo=FALSE, include=FALSE, eval=TRUE}
...
```{r M3.MLH1.var, echo=FALSE, eval=TRUE}
...
```{r MLH1.var.M4, echo=FALSE, eval=TRUE}
#male
...
```{r FEMALE.VAR.MODELS, echo=FALSE, eval=TRUE}
...
```{r MALE.SPECIFIC.variance.model, echo=FALSE, eval=TRUE}
## within mouse variance models 
...
```{r tot.SC_strain.setup, echo = FALSE}
#move to setup
...
```{r Q1.F1pos_long.short_setup, echo=FALSE, eval=TRUE}
...
#save Rdata
save.image("~./MLH1repo/data/Results_setup_5.14.20.RData")
