...
load(file = "~./MLH1repo/data/MLH1_data_setup_5.30.20.RData")#added SKIVE_f BiveData
...
#non-passing mice
SPI.remove.mice <- c("30may18_SPIC_f1", "8jun18_SPIC_m2")
...
127 mice, 8 strains (correct), 0 MOLF females! -- this was fixed
...
FOR new supplemental table write out all of the model coeffiecnts (into raw -- then reformat), use these to double check the text values.
...
#goal get all the results below to a .tex format
...
tot.SC_old.mice <- totSC.mouse.av.table[totSC.mouse.av.table$mouse  %in% old.mice.list, ]

totSC.mouse.av.table <- totSC.mouse.av.table[! totSC.mouse.av.table$mouse  %in% juvi_mice, ]

totSC.mouse.av.table <- totSC.mouse.av.table[! totSC.mouse.av.table$mouse  %in% old.mice.list, ]

tot.SC_old.mice.DF <- Total.SC_DF[Total.SC_DF$mouse  %in% old.mice.list, ]


#
totSC.mouse.av.table <- totSC.mouse.av.table %>% filter(strain!="CAST" & strain !="F1" & strain !="SPI" & strain !=  "CAROLI" & strain !="SPRET"& strain !="HMI"& strain !="PERC")
...
#not working currently.. might be same 'ol error
...
```{r Q1.M1_short_biv_lmer, echo=FALSE, warning=FALSE, message=FALSE, eval=FALSE}
...
write.table(Q1lmer.MM_SC_short_results, "~./MLH1repo/ShortSC_Q1_M1.csv", sep=",",row.names = TRUE)

#write_xlsx(Q1lmer.MM_SC_short_results, "~./MLH1repo/ShortSC_Q1_M1.xlsx", col_names = TRUE)

write.xlsx(Q1lmer.MM_SC_short_results, "ShortSC_Q1_M1.xlsx", col.names = TRUE)
...
```{r Q2.totSC.glm, echo=FALSE, eval=FALSE}
#this model shouln't be used -- switch to M4 and M5 males
