...
female_mouse.avs_for.analysis <-  Mouse.Table_HQ %>% filter(sex== "female") %>% filter(species == "M.musculus") %>% filter(strain != "CZECH") %>% filter(strain != "CAST")
...
#there's alread a Mouse.Table_BivData --mouse.level.BivData_table
...
```{r M1.MLH1.var_cv, echo=FALSE, warning=FALSE, message=FALSE, eval=FALSE}
#why is this block false? are the models in the MS.RMD file?
...
```{r M2.MLH1.var, echo=FALSE, include=FALSE, eval=F}
...
```{r M3.MLH1.var, echo=FALSE, eval=F}
...
Q2.glm_mus.av_totSC_M4 <- glm(mean.skel ~ subsp * strain, data= mouse.total.skel %>% filter(sex == "male") %>% filter(species == "M.musculus") )
...
Q2.glm_mus.av_totSC_M4.2 <- glm(mean.skel ~ strain, data= mouse.total.skel %>% filter(sex == "male") %>% filter(species == "M.musculus") )
...
```{r Q2.1COpos_mouse.av.t.test_all, echo=FALSE, eval=FALSE}
...
```{r Q2.1COpos_within.subsp_ttest, echo=FALSE, eval=FALSE}
