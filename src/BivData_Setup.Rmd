...
declare the Strain specific BivData_DFs  (for convinice)
...
add Q1 and Q4 to the cell summary stats table
...
just.SC.length <- Curated_BivData[is.na(Curated_BivData$hand.foci.count),]#650 lines
...
## Male specific data set

```{r male.sc.length.strain, echo=FALSE}

#these measure ect should be in the male - mouse 
#isolate all musc and male
Curated_BivData_male <- Curated_BivData[ (Curated_BivData$sex == "male") & (Curated_BivData$species == "Musculus"),]
...
The below cell level table is used for making the **long.biv.dataset**, (any others?)
...
Mouse level tables are the main meat of the models. There will be several --- which account for different combinations of quality scores.

BC their for MM, format the tables for run the mixed models (unordered factors, ect)
...
#is there a way that the above table would be different than the 
...
Mouse.Table_BivData_4MM <- Mouse.Table_BivData_4MM[Mouse.Table_BivData_4MM$strain != "AST",]
Mouse.Table_BivData_4MM <- Mouse.Table_BivData_4MM[Curated_BivData$strain != "TOM",]
Mouse.Table_BivData_4MM <- Mouse.Table_BivData_4MM[Mouse.Table_BivData_4MM$strain != "CZECH",]
Mouse.Table_BivData_4MM$strain <- droplevels(Mouse.Table_BivData_4MM$strain)
#MOLF


#These shouldn't be needed for now.
...
#save Rdata
save.image("~./MLH1repo/data/CleanBivData_1.28.20.RData")
