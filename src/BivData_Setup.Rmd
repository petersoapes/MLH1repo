...
files that will be made in setup
...
#found that one of the data sheets is missin bunch of SKIVE female measures
#read in missing SKIVE -- then 
...
missing.SKIVE.female <- missing.SKIVE.female[!(is.na(missing.SKIVE.female$fileName) | missing.SKIVE.female$fileName==""), ]
#this takes a while to run

#new.Curate.FULL <- add_mouse(new.Curate.FULL)#this function is v.slow for this big file

missing.SKIVE.female <- add_mouse(missing.SKIVE.female)
#make OG copy
...
```{r mk.Curate_BivData, echo=FALSE}
...
str(missing.SKIVE.female)

missing.SKIVE.female$fileName <- as.character(missing.SKIVE.female$fileName)
...
#SC PASS
missing.SKIVE.female <- missing.SKIVE.female[missing.SKIVE.female$SC.pass == '1',]#
#10480 #9819, #9769
...
missing.SKIVE.female <- add_mouse(missing.SKIVE.female)
missing.SKIVE.female <- add_category(missing.SKIVE.female)
missing.SKIVE.female <- add_strain(missing.SKIVE.female)
missing.SKIVE.female <- add_sex(missing.SKIVE.female)
missing.SKIVE.female <- add_subsp(missing.SKIVE.female)
missing.SKIVE.female <- add_species(missing.SKIVE.female)
...
missing.SKIVE.female$fileName <- as.character(missing.SKIVE.female$fileName)
missing.SKIVE.female$Obj.ID <- as.character(missing.SKIVE.female$Obj.ID)

missing.SKIVE.female$centromere_ABS_Position <- as.character(missing.SKIVE.female$centromere_ABS_Position)
missing.SKIVE.female$centromere_PER_Position <- as.character(missing.SKIVE.female$centromere_PER_Position)
missing.SKIVE.female$chromosomeLength <- as.character(missing.SKIVE.female$chromosomeLength)

missing.SKIVE.female$hand.foci.count <- as.character(missing.SKIVE.female$hand.foci.count)

missing.SKIVE.female$Foci1 <- as.character(missing.SKIVE.female$Foci1)
missing.SKIVE.female$Foci2 <- as.character(missing.SKIVE.female$Foci2)
missing.SKIVE.female$Foci3 <- as.character(missing.SKIVE.female$Foci3)
#missing.SKIVE.female$Foci4 <- as.character(missing.SKIVE.female$Foci4)

##Numeric
missing.SKIVE.female$centromere_ABS_Position <- as.numeric(missing.SKIVE.female$centromere_ABS_Position)
missing.SKIVE.female$centromere_PER_Position <- as.numeric(missing.SKIVE.female$centromere_PER_Position)
missing.SKIVE.female$chromosomeLength <- as.numeric(missing.SKIVE.female$chromosomeLength)

missing.SKIVE.female$hand.foci.count <- as.numeric(missing.SKIVE.female$hand.foci.count)

missing.SKIVE.female$Foci1 <- as.numeric(missing.SKIVE.female$Foci1)
missing.SKIVE.female$Foci2 <- as.numeric(missing.SKIVE.female$Foci2)
missing.SKIVE.female$Foci3 <- as.numeric(missing.SKIVE.female$Foci3)
...
missing.SKIVE.female <- add_IFD(missing.SKIVE.female)#fixed a bug
missing.SKIVE.female$IFD1_PER = missing.SKIVE.female$IFD1_ABS / missing.SKIVE.female$chromosomeLength
...
missing.SKIVE.female$PER_Foci_1 <- missing.SKIVE.female$Foci1 / missing.SKIVE.female$chromosomeLength
missing.SKIVE.female$PER_Foci_2 <- missing.SKIVE.female$Foci2 / missing.SKIVE.female$chromosomeLength
missing.SKIVE.female$PER_Foci_3 <- missing.SKIVE.female$Foci3 / missing.SKIVE.female$chromosomeLength
missing.SKIVE.female$PER_Foci_4 <- missing.SKIVE.female$Foci4 / missing.SKIVE.female$chromosomeLength
...
#not working
missing.SKIVE.female$dis.cent <- missing.SKIVE.female$Foci1 - missing.SKIVE.female$centromere_ABS_Position
missing.SKIVE.female$dis.cent.PER  <- missing.SKIVE.female$dis.cent / missing.SKIVE.female$chromosomeLength


#add telomere.dist
missing.SKIVE.female$telo_dist <- ifelse(grepl(1, missing.SKIVE.female$hand.foci.count), (missing.SKIVE.female$chromosomeLength - missing.SKIVE.female$Foci1), 
                     ifelse(grepl(2, missing.SKIVE.female$hand.foci.count), (missing.SKIVE.female$chromosomeLength -  missing.SKIVE.female$Foci2), 
                     ifelse(grepl(3, missing.SKIVE.female$hand.foci.count), (missing.SKIVE.female$chromosomeLength - missing.SKIVE.female$Foci3),
                    ifelse(grepl(4, missing.SKIVE.female$hand.foci.count), (missing.SKIVE.female$chromosomeLength - missing.SKIVE.female$Foci4),   0))) )
  

missing.SKIVE.female$telo_dist_PER <- missing.SKIVE.female$telo_dist / missing.SKIVE.female$chromosomeLength

#sis.co.ten
missing.SKIVE.female <- add_SisCoTen(missing.SKIVE.female)

#add rec.group
#divide musc males into groups
missing.SKIVE.female$Rec.group <- ifelse(grepl("PWD male", missing.SKIVE.female$category), 1, 
                                  ifelse(grepl("MSM male", missing.SKIVE.female$category), 1,
                                  ifelse(grepl("SKIVE male", missing.SKIVE.female$category), 1,
                                           0)))
...
```{r no.Foci.Pos, echo=FALSE, eval=FALSE}

#no posittion
#just.SC.length <- Curated_BivData[is.na(Curated_BivData$Foci1),] #NAs not registering

#not sure why these are different DFs
just.SC.length2 <- Curated_BivData %>% filter(Foci1 == " ")
just.SC.length <- Curated_BivData[(Curated_BivData$Foci1 == ""),]
#these are different than hand.foci.count

table(Curated_BivData$hand.foci.count, exclude = NULL)
#there are ~600 rows with SC and no hand foci count (blah)
```


```{r neg.IFD, echo=FALSE, eval=FALSE}

#fixed a bug for the multi CO IFD

#source("~./MLH1repo/src/CommonFunc_MLH1repo.R")
#remove IFD
#Curated_BivData <- add_IFD(Curated_BivData)
#t.test.4CO <- Curated_BivData %>% filter(Obj.ID == "8feb17_4jan17_LEW_f1_sp1_3_rev_17")

neg.IFD <- Curated_BivData[Curated_BivData$IFD1_ABS < 0, ]
neg.ifd <- Curated_BivData %>% filter(hand.foci.count > 1) %>% filter(IFD3_ABS < 0 )
...
```{r remove.DUP, echo=FALSE}

#check for duplicates, and track them down
anyDuplicated(new.Curate.FULL$Obj.ID)  #5335, #3467

dup.curated <- Curated_BivData[duplicated(Curated_BivData$Obj.ID),] #784
dup.curated <- add_mouse(dup.curated)

#table(dup.curated$category)#MSM male, PWD male
#obj.ID is not complete for all rows

Curated_BivData_w.dup <- Curated_BivData

Curated_BivData <- Curated_BivData %>% distinct(Obj.ID, .keep_all = TRUE)
...
Duplicates removed, `r length(Curated_BivData_w.dup$Obj.ID) - length(Curated_BivData$Obj.ID)`. most were MSM and PWD males.

```{r remove.mice, echo=FALSE, eval=TRUE}

#reaxamine these mice for if they should be included in 
#this list is the same as setupData
...
exmice.Curated_BivData <- Curated_BivData[ Curated_BivData$mouse %in% exclude_mice_list_MLH1, ]

Curated_BivData <- Curated_BivData[ ! Curated_BivData$mouse %in% exclude_mice_list_MLH1, ]
...
`r length(exmice.Curated_BivData$Obj.ID)` rows removed for excluded mice.


after all of these chunks are run, save the excel file for Curated_BivData

DUPs, negative IFDs, remove juvi and MLH1 exclude mice,

There are 3 bivalents with Per.Foci2 > 1 (remove or go correct)



```{r write.Curated.BivData, echo=FALSE}

#this file should have all the category columns, and pass the curation levels above

#write.xlsx2(Curated_BivData, "Curated_BivData.xlsx", sheetName = "Sheet1", 
#            col.names = TRUE, row.names = FALSE, append = FALSE)
#this excel causes an error

#write.csv(Curated_BivData, "Curated_BivData.csv",   row.names = FALSE)
...
table(WSB.Bivalents.DF$sex, WSB.Bivalents.DF$hand.foci.count, exclude = NULL)

#plot the differences
wsb.ploty <- ggplot(WSB.Bivalents.DF,  aes(y=chromosomeLength, x = sex, color= as.factor(hand.foci.count)))+geom_jitter()+ggtitle(WSB.Bivalents.DF$strain)
wsb.ploty
...
table(LEW.Bivalents.DF$sex, LEW.Bivalents.DF$hand.foci.count, exclude = NULL)

#plot the differences
LEW.ploty <- ggplot(LEW.Bivalents.DF,  aes(y=chromosomeLength, x = sex, color= as.factor(hand.foci.count)))+geom_jitter()+ggtitle(LEW.Bivalents.DF$strain)
...
table(G.Bivalents.DF$sex, G.Bivalents.DF$hand.foci.count, exclude = NULL)

#plot the differences
G.ploty <- ggplot(G.Bivalents.DF,  aes(y=chromosomeLength, x = sex, color= as.factor(hand.foci.count)))+geom_jitter()+ggtitle(G.Bivalents.DF$strain)
...
table(PWD.Bivalents.DF$sex, PWD.Bivalents.DF$hand.foci.count, exclude = NULL)

#plot the differences
PWD.ploty <- ggplot(PWD.Bivalents.DF,  aes(y=chromosomeLength, x = sex, color= as.factor(hand.foci.count)))+geom_jitter()+ggtitle(PWD.Bivalents.DF$strain)
...
table(KAZ.Bivalents.DF$sex, KAZ.Bivalents.DF$hand.foci.count, exclude = NULL)

#plot the differences
KAZ.ploty <- ggplot(KAZ.Bivalents.DF,  aes(y=chromosomeLength, x = sex, color= as.factor(hand.foci.count)))+geom_jitter()+ggtitle(KAZ.Bivalents.DF$strain)
...
table(SKIVE.Bivalents.DF$sex, SKIVE.Bivalents.DF$hand.foci.count, exclude = NULL)

#plot the differences
SKIVE.ploty <- ggplot(SKIVE.Bivalents.DF,  aes(y=chromosomeLength, x = sex, color= as.factor(hand.foci.count)))+geom_jitter()+ggtitle(SKIVE.Bivalents.DF$strain)
...
table(MSM.Bivalents.DF$sex, MSM.Bivalents.DF$hand.foci.count, exclude = NULL)

#plot the differences
MSM.ploty <- ggplot(MSM.Bivalents.DF,  aes(y=chromosomeLength, x = sex, color= as.factor(hand.foci.count)))+geom_jitter()+ggtitle(MSM.Bivalents.DF$strain)
...
```{r MOLF.SC, echo=FALSE, eval=FALSE}
#make the dataset
MOLF.Bivalents.DF <- Curated_BivData[Curated_BivData$strain == "MOLF",]
table(MOLF.Bivalents.DF$sex, MOLF.Bivalents.DF$hand.foci.count, exclude = NULL)

#plot the differences
MOLF.ploty <- ggplot(MOLF.Bivalents.DF,  aes(y=chromosomeLength, x = sex, color= as.factor(hand.foci.count)))+geom_jitter()+ggtitle(MOLF.Bivalents.DF$strain)
...
Redo the plots from above, just male data with strain on the x

```{r BivLengths_by_strain, echo=FALSE}

#set the order of strains


BivLen.Male <- ggplot(
  (Curated_BivData %>% filter(sex == "male") %>% filter(species == "M.musculus")),  aes(y=chromosomeLength, x = strain, color= as.factor(hand.foci.count)))+geom_jitter()+ggtitle("Male Single Bivalent Lengths")
BivLen.Male
```
...
# reduced Long and Short BivData Set
...
```{r short.biv.data, echo=FALSE, include=FALSE, eval=TRUE}
...
#save Rdata
save.image("~./MLH1repo/data/CleanBivData_5.12.20.RData")
