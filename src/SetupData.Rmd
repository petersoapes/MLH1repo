...
```{r MLH1.DF.setup, warning=FALSE}

#todo :
#set the order to batches
#2 fileNames from batch 0 appear 2x, (1 is missing random name, the other has the same random name)
...
```
 ...
 Settting up main data file with explanation and examination of data.  Make the raw original DF and main working DF with bad cells exlcuede.  (inline code add the numbers)

There are `r length(original_DF_duplicates$fileName)` duplicated file names. These data will be used to quantify repeatability, and remove from the final DFs.
Using the distinct() function for the MLH1_data, the first duplicated filename is kept and the second removed.
...
The original dataset contains the quantification batches 

`r kable(final_batches, caption = "This is the table caption") ) %>% kable_styling()`
...
mice.frm.meta <- AP_mouse_table[(AP_mouse_table$mouse %in% meta.data$mouse),]
...
AP_mouse_table_w.Ages <- merge(AP_mouse_table, meta.data, by.y = "mouse", by.x = "mouse", all = TRUE)
...
X number of mice are in the main DF (discribe the results for missing mice)
...
male_mouse_table_w.Ages <- mouse.table_w.Ages[(mouse.table_w.Ages$sex == "male") &(mouse.table_w.Ages$category != "other"),]

all.ages_plot <- ggplot(male_mouse_table_w.Ages, aes(age.weeks))+geom_histogram(binwidth = 3)+facet_wrap(~category)+ggtitle("Male Age Distributions")
all.ages_plot
...
colors_of_strains <- c('WSB'= "#56B4E9",'G' ='cadetblue','LEW'= 'lightblue','PERC'= 'blue',
                       ...
                       'PWD'= 'red','MSM'= 'darkorange', 'MOLF'= 'orange3',#red mucs, orange Molossi
                       'SKIVE'= 'coral1','KAZ'= 'coral4', 'TOM'= 'indianred1','AST' = 'red4',
                       'CZECH'= '#E69F00',
                       
                    'CAST'='green','HMI'= 'forestgreen', #green
                    
                    'SPRET'='gold','SPIC'= 'goldenrod1',#gold?
                    'CAROLI'='gold3',
...
save.image("~./MLH1repo/data/MLH1/MLH1_data_setup_9.26.19.RData")
