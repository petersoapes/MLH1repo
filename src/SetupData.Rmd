...
#total SC -- the one file should be all I need?
Total.SC_DF = read.xlsx2("~./MLH1repo/data/Total.SC_merge.MLH1.xlsx", header=TRUE, 1)

#Curated Bivdata - this excel file is too big to work with read.xlsx2
#Curated_BivData = read.xlsx2("~./MLH1repo/data/Curated_BivData.xlsx", header=TRUE, 1)

Curated_BivData <-  read.csv("~./MLH1repo/data/Curated_BivData.csv", header=TRUE)
...
#make a new DF -- with one fileName and the MLH1-1 and MLH1-2
#  (duplicate / dostinct fileName )
...
`r length(original_DF_duplicates$Random.Name)` rows are marked as duplicate.
...
```{r all.data.table, echo=FALSE, eval=FALSE}
...
#NA cause by CAST female
#this table isn't used much
```
...
DF.HetC.MixedModel <- Mouse.Table_HQ %>% filter(species == "M.musculus")
...
mlh1male_mouse.av <-  Mouse.Table_HQ %>% filter(sex== "male")
...
not sure if SC.Length.male_table is used for anything.
...
```{r Look.short.biv.data, echo=FALSE, include=FALSE, eval=FALSE}
...
The block below creates clean.BivData, which removes ~600 rows if just SC measures
(This might be important for the proportion code)

```{r FULL.mouse.BivData.table, echo=FALSE}

#length( Curated_BivData %>% filter(hand.foci.count == "NA") )#36
...
```{r SHORT.biv_setup, echo=FALSE, eval=TRUE}
...
total sc is processed in the results setup.
...
save.image("~./MLH1repo/data/MLH1/MLH1_data_setup_5.12.20.RData")
