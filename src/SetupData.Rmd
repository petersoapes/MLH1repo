...
```{r remove.non.foci, echo=FALSE}
#length( Curated_BivData %>% filter(hand.foci.count == "NA") )#36
str(Curated_BivData)
table(Curated_BivData$hand.foci.count, exclude = NULL)
#there are ~600 rows with SC and no hand foci count
table( (Curated_BivData %>% filter(  is.na(hand.foci.count) ) )$category )
#KAZ male and WSB female have >100 observations without hand.foci.counts
...
length( (Curated_BivData  %>%  filter(  is.na(hand.foci.count) == T ) )$fileName )

table(cleanCurated_BivData$hand.foci.count, exclude = NULL)
#600 removed
```
