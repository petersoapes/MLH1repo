...
# sample size
#sample_cells_pvals
sample_cells <- function(list, DF, ncells, sim_num){
  outlist = list()
  for(mouse in 1:length(unique(list$mouse))){
    #print(list$mouse[mouse])
    mus_data <- DF[DF$mouse == list$mouse[mouse],]
    outlist[mouse] <- list(replicate(sim_num, t.test(sample(DF$adj_nMLH1.foci, ncells, replace = FALSE),
                                        sample(DF$adj_nMLH1.foci, ncells, replace = FALSE) )$p.value ) )
