...
MLH1_data$strain <- as.factor(MLH1_data$strain)

MLH1_data$strain<- factor(MLH1_data$strain,levels =c( "G", "WSB", "LEWES", 
                                                        "PWD", "MSM",
                                                       "CAST", "HMI",
                                                        "SPRET", "other"), order=T )

MLH1_data$sex<- factor(MLH1_data$sex,levels =c( "female", "male"), order=T )
...
qq <- ggplot(highest_qual_df, aes(x = mouse, y = adj_nMLH1.foci)) + geom_boxplot(data = highest_qual_df, aes(fill = factor(strain) ) ) +  ggtitle("Cells with quality scores of 2 or 1") +
  ...
  "coral1", "#E69F00", "yellowgreen","black", "grey"  ))
