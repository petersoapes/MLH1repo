...
SC.KAZ <- ggplot(data = KAZ_curate_BivData, aes(color=fileName,x=mouse, y= chromosomeLength))+geom_jitter()+theme(legend.position="none")+ylim(c(35,150))+ggtitle("KAZ SC lengths")
#can I add the means?
...
bin.short.PWD <- DF.w.rank2[DF.w.rank2$rank < 5,]
bin.short.PWD <- bin.short.PWD[bin.short.PWD$fileName %in% full.cell.list$fileName,]

kk <- ggplot(data=bin.short.PWD, aes(x=fileName,y=chromosomeLength))+geom_jitter(width = 0.25, aes(color =Foci1, shape=mouse))+ggtitle("short bins")
...
foci.passed.KAZ <- curBivData_KAZ[curBivData_KAZ$foci.pass == 1,]

foci.passed.PWD <- curBivData_PWD[curBivData_PWD$foci.pass == 1,]


qq <- ggplot(data = foci.passed.KAZ, aes(color=fileName,x=mouse, y= hand.foci.count))+geom_jitter()+theme(legend.position="none")
