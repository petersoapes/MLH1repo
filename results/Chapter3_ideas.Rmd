...
for(oar in 1:length(male.whole.cell.mini$fileName)){
  print(oar)
   if(male.whole.cell.mini$hand.foci.count[oar] == 1){
    print("oner")
     ...
     male.whole.cell.mini$rand.Foci1[oar] <- as.numeric(pos.pool_1CO$Foci1[which(pos.pool_1CO$Obj.ID == rand.obj.id.1CO)])
  ...
  if(male.whole.cell.mini$hand.foci.count[oar] == 2){
    ...
    male.whole.cell.mini$rand.Foci1[oar] <-as.numeric(pos.pool_2CO$Foci1[which(pos.pool_2CO$Obj.ID == rand.obj.id)])
  #  male.whole.cell.mini$rand.Foci2[oar] <- "blah"
    male.whole.cell.mini$rand.Foci2[oar] <-as.numeric(pos.pool_2CO$Foci2[which(pos.pool_2CO$Obj.ID == rand.obj.id)])
    male.whole.cell.mini$rand.Foci3[oar] <- ""
 }
  ...
  if(male.whole.cell.mini$hand.foci.count[oar] == 3){
    ...
    male.whole.cell.mini$rand.ID[oar] <- rand.obj.id_3co
        ...
        male.whole.cell.mini$rand.Foci1[oar] <-as.numeric(pos.pool_3CO$Foci1[which(pos.pool_3CO$Obj.ID == rand.obj.id_3co)])
  #  male.whole.cell.mini$rand.Foci2[oar] <- "blah"
    male.whole.cell.mini$rand.Foci2[oar] <-as.numeric(pos.pool_3CO$Foci2[which(pos.pool_3CO$Obj.ID == rand.obj.id_3co)])
    
    male.whole.cell.mini$rand.Foci3[oar] <-as.numeric(pos.pool_3CO$Foci2[which(pos.pool_3CO$Obj.ID == rand.obj.id_3co)])
...
random_pos <- data.frame(male.whole.cell$fileName, male.whole.cell$SC.length, male.whole.cell$hand.foci.count, male.whole.cell$Obj.ID  )


#INITIALIZE EMPTY list of DFs
#fill in with full DF

LIST.DFs <- setNames( replicate( length(male.biv.count_table$fileName), data.frame( obj.ID = c())), male.biv.count_table$fileName)

ids <-    male.whole.cell$Obj.ID[which(male.whole.cell$fileName == '14oct16_30jun16_CAST_m3_sp1_3.2')]  

LIST.DFs[1]$obj.ID[1:19] <- ids[1:19]

#INITALIZE empty LIST (make DF in the loop)

LIST <- list()

#Loop for rand sampling  
for( male.cell in male.biv.count_table$fileName){
  
  #for each fileName, make a new DF
  DF.in.loop <- data.frame(matrix(ncol = 1, nrow = 19))
  #making the Within loop DF
  DF.in.loop$X1 <- male.whole.cell$fileName[male.whole.cell$fileName ==male.cell]
  DF.in.loop$obj.ID <- male.whole.cell$Obj.ID[male.whole.cell$fileName ==male.cell]
  DF.in.loop$SC.length <- male.whole.cell$SC.length[male.whole.cell$fileName ==male.cell]
  DF.in.loop$hand.foci.count <- male.whole.cell$hand.foci.count[male.whole.cell$fileName ==male.cell]
  
  
  #LIST.DFs[[male.cell]]$obj.ID[1:19] <- male.whole.cell$Obj.ID[male.whole.cell$fileName == male.cell]
    #print(male.cell)
    n0CO = male.biv.count_table$nCO0[male.biv.count_table$fileName == male.cell] #this reference needs to be from male.biv.count_table
    n1CO = male.biv.count_table$nCO1[male.biv.count_table$fileName == male.cell]
    n2CO = male.biv.count_table$nCO2[male.biv.count_table$fileName == male.cell]
    n3CO = male.biv.count_table$nCO3[male.biv.count_table$fileName == male.cell]
    
   #rand samples for foci.pos
    #JUST DO THIS FOR 1CO bivs
    DF.in.loop$rand_F1
    sample.n1CO <- sample(pos.pool_1CO$Foci1.PER, n1CO, replace = FALSE )  #sample from the cell.table
   
    #sample a random 2CO chrm (obj.ID),    #doesn't make sense to sample a random position from 2CO pool, need to sample the linked positions, or get the 
    #chrm observation
   sample.n2CO.obj.id <- sample(pos.pool_2CO$Obj.ID, n2CO, replace = FALSE )
   sample.n3CO.obj.id <- sample(pos.pool_2CO$Obj.ID, n3CO, replace = FALSE )
   
   #MAP normalized positions onto SC lengths/chrms of the fileName cell obs
   #map the 1CO rand Foci position onto 1CO SCLength
   
   #for i in n1CO
#   which(random_pos$fileName == male.cell)
   random_pos$Rand.F1[which(random_pos$fileName == male.cell)][i] <- sample(pos.pool_1CO$Foci1.PER, 1, replace = FALSE)
   
   #list of the chrm lengths for all the 1COs for the current cell obs
   #SC.Length_1COs <- 
   
}


#need to define number of chrm classes within each nuc
n1CO = 9

BIG.DF.LIST_COpos_no.rank <- replicate(10,
                  list(bind_rows(list( 
          
  pos.pool_1CO[ sample( which( pos.pool_1CO$hand.foci.count == 1), n1CO, replace = FALSE ),]
