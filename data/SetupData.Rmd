...
opts_knit$set(root.dir = "~./MLH1repo/data/")
setwd("~./MLH1repo/data/")
...
MLH1_data = read.xlsx2("~./MLH1repo/data/MLH1.AnonData_formated.xlsx", header=TRUE, 1)
#remove all X's from MLH1, remove all old mice?
#add date imaged
#remove the Remove mice, (juvi, old, bad quality)

#DMC1.DF <- read.csv("~./MLH1repo/data/DMC1newJuviQuant.csv", header = TRUE)
DMC1.DF <- read.csv("~./MLH1repo/data/DMC1_data.csv", header = TRUE)
...
```{r MLH1.DF.setup, warning=FALSE, echo=FALSE, eval=FALSE}

#duplicate file names and rows have been removed
#they can be found in older files for accuracy checks

#check that these should all be redundant
...
colors_of_strains <- c('WSB'= "#56B4E9",'G' ='cadetblue','LEW'= 'lightblue','PERC'= 'blue',
                    ...
                    'SPRET'='gold','SPI'= 'goldenrod1', #browns? color of mounds?
                    'CAROLI'='gold3',
...
The original dataset was cleaned up in the following ways before analysis. Settting up main data file with explanation and examination of data. Make the raw original DF and main working DF with bad cells exlcuede. The original dataset had a number of duplicated file names. These were removed and can be used to quantify repeatability or repair. Using the distinct() function for the MLH1_data, the first duplicated filename is kept and the second removed.
...
#there are ~26 old mice 

Mouse.Table_HQ.OG <- Mouse.Table_HQ

#this doesn't remove
Mouse.Table_HQ <- Mouse.Table_HQ.OG[ ! Mouse.Table_HQ.OG$mouse %in% old.mice.list, ]
...
```{r Age.Hists, echo=FALSE, warning=FALSE, eval=FALSE}
#age. weeks was removed from the Mouse.Table.HQ for now
...
#write excel file
#write.xlsx2(XX, "XX.xlsx", sheetName = "Sheet1", 
#            col.names = TRUE, row.names = FALSE, append = FALSE)
...
Total.SC_DF <- Total.SC_DF[Total.SC_DF$subsp != "other", ]
...
#last outliers
#Total.SC_DF <- Total.SC_DF[Total.SC_DF$skel_size < 10000, ]#one very high female measure
#Total.SC_DF <- Total.SC_DF[Total.SC_DF$skel_size < 1900, ]

Total.SC_DF.OG <- Total.SC_DF


#Total.SC_DF
tot.SC_old.mice.DF <- Total.SC_DF[Total.SC_DF$mouse  %in% old.mice.list, ]#581 lines from old mice
tot.SC_juvi.mice.DF <- Total.SC_DF[ Total.SC_DF$mouse  %in% juvi_mice.list, ]#118 lines from juvi mice

Total.SC_DF <- Total.SC_DF[! Total.SC_DF$mouse  %in% old.mice.list, ] 
Total.SC_DF <- Total.SC_DF[! Total.SC_DF$mouse  %in% juvi_mice.list, ] 

```



```{r total.SC.outliers, echo=FALSE, eval=FALSE}

#yy <- Total.SC_DF %>% filter(subsp == "Dom") %>% filter(sex == "male")
Total.SC_DF %>% filter(subsp == "Dom") %>% filter(sex == "male")

show.total.SC <- ggplot(data= Total.SC_DF %>% filter(strain == "SKIVE"), aes(y = skel_size, x=sex, color=strain))+
  scale_color_manual(values=colors_of_strains)+
  geom_jitter() +ylim(c(950,3500 ))+ facet_wrap(~mouse, scales="free")


 # geom_boxplot(alpha=.2)+   facet_wrap(~REDO.crop, scales="free", ncol = 3) +  #geom_hline(data =totSC.strain.av.table, aes(yintercept=mean.skel) )+
...
#axis.title.y = element_text(size = 10) ) +
 ...
 # labs(y = "Total SC Length")+ylim(c(1000,5000 ))+ggtitle("")
...
save.image("~./MLH1repo/data/MLH1_data_setup_5.30.20.RData")
