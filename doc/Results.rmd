...
Because all of the mixed model effects had significant effects, (p= `r  round(MLH1.MMlmer_results$sub.sp, 4)`, `r  MLH1.MMlmer_results$sex.results`, and  `r  MLH1.MMlmer_results$interaction.pval` for the sex, subspecies and interaction fixed effects and `r  MLH1.MMlmer_results$rand.pval` for the random strain effect), we follow up with two general linear models to investigate the specific strain effects.
...
glmMLH1Var_M3 <- lm( var ~ sex * strain,  data=DF.HetC.MixedModel.HQ )

#round(lm.male.strain_sum$coefficients[,c(1,4)], 5)
glmMLH1Var_M3_sum <- summary(glmMLH1Var_M3)

#as.data.frame( round(summary(glmMLH1Var_M3)$coefficients[,c(1,4)], 6))
MLH1cv_M3 <- lm(cV ~ sex * strain, data=DF.HetC.MixedModel.HQ)
...
### Q12 var
MLH1var_M3_Q12 <- lm( var ~ sex*strain, data=Q12_mouse_table_w.Ages )
...
```
The coefficients for mouse level variance of the linear model listed above.
...
<!-- 
-how to display all the model results...
...
**more variance within females animals -- this is also seen in humans (cite Gruhn et al),** 

also measures of variance are more supciptibal to thechnical error effects (noise). For this reasons, we applied // analysze the data to a subset cells which were scored as highest quality  -- the general result of female sex effect was consistant between the two sets of models.

 While some results different between the full cell data set and high quality ( slightly significant P values for strain effects, not shown)

 In an analysis of the within animal variance for MLH1 foci per cell, sex was the most significant effect (p = ). Indicating that females consistantly had higher variation for the number of MLH1 foci across cells.
 the overall pattern was significant sex effect 
...
**within sex specific strain effecsts on variance**

Some strains more than others (investigate potential outliers). (LEW females with top 3 variance are all from different dissection dates)
...
```{r Q1.SC.show.long.biv99, echo=FALSE, eval=FALSE}
...
glm1.SC.male_long.biv <- glm(mean_SC ~ subsp * strain, data= Long.biv_mouse_table)
#summary(glm1.SC.male_long.biv) 

glm2.SC.male_long.biv <- glm(mean_SC ~ strain, data= Long.biv_mouse_table)
