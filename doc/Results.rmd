...
Bivalent_1o2  (for logistic regression, 1CO and 2COs)
male.long.biv
...
```{r Q2.SClength.t.test, echo=FALSE}
#MOUSE AVE, all
...
The top t.tests indicate that when all the mice averages are pooled, there's a sig difference in SC lengths. But When the means are compared across chrm class, the mouse averages are no longer significant.


### Q2 SC length Chrm class prediction


How well does SC length predict chromosome class?  Prediction, High rec strains will have more sig pvalue, given the lower overlap in SC lengths across chrm class. (Note this test can only be done with bivalent level observations, because 1CO or 2CO)
...
The t.tests return very significant pvalues for the normalized IFD between the high and low rec groups (for both mouse av and single bivalent observations levels).

### Q2 Long Bivalent Dataset

Examine the pattern in the Long bivalent dataset to get a feel for if the chromosome size effect skews the pattern. In the long bivalant data set, the longest bivalents (top 25%) from each cell are isolated, so they are more likely to be the same chromosome identities (Chrm1 to Chrm5). The section on IFD only looks at the 2CO bivalents.
...
There are alot fewer long bivalent data observations, but the same positive correlation with 
...
There are not enough observations for subsetting the data into strains or use the mouse averages.
