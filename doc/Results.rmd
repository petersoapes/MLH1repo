...
We used counts of MLH1 foci per cell to estimate the genome wide recombination rates across our panel of `r length(unique(  (main.table  %>% filter(species == "M.musculus") )$strain) )` wild derived inbred strains sampled from three subspecies within the house mouse species complex;  _M. m. domesticus,_ _M. m. musculus_. and _M. m. molossinus_. MLH1 foci were quantified from a total of `r sum( (main.table %>% filter(species == "M.musculus") %>% filter(sex == "male"))$Ncells)` spermatocytes and `r sum( (main.table %>% filter(species == "M.musculus") %>% filter(sex == "female"))$Ncells)` oocytes. <!-- state the average number of mice
...
Figure 1A shows the strain averages for each sex together - this is a way to display the general patterns of sexual dimorphism (or heterochiasmy) across strains. 
Taking note of the direction and the magnitude our results confirm two general patterns that i) the genome wide recombination rates averages are greater females compared to males and ii) the degree of heterochiasmy (F:M rates) is generally low, rangeing from `r G.HETC.val` in _G_ to `r LEW.HETC.val` in _LEW_. 

The notable exceptions to this pattern are from strains with male averages that exceed the female averages, with heterochiasmy values of `r PWD.HETC.val`, `r MSM.HETC.val` and `r SKIVE.HETC.val` for PWD, MSM and SKIVE respectively. 

Examining the mouse means for the genome wide recombinaiton rate seperately for each sex reveals disctinct patterns of variation (Figure1 B and C). 

The female means can be discribed as more evenly distributed around the sex-wide mean of ~25

While in males, the strain specific means -- can be seperated into groups of high genome wide recombination rates (near 30 COs per cell) and low genome wide recombiantion rates, near 23 COs per cell.
...
Examinations of the sex specific data revealed that greater variety in across the strain means in males compared to females.

<!-- female strain means are have more variation in a random way. The range of genome wide rec strains is X to X, -- no clustering of strain means 

-->

In females the strains with the greatest difference
in mean gwnome wide recombiantion rate was `r G.female.diff` ( _domesticus^G^_) more compared to other strains. 
...
While in the male data (most strain averages are close to 23 (Figure1).



the genome wide recombination rate for had a much larger range, up to `r (mean(MSM_male$mean_co, na.rm = TRUE) / mean(sans_LewRec_male$mean_co, na.rm = TRUE) )` ( _molossinus^MSM^_ ). 

 

_musculus^PWD^_ ,and  _musculus^SKIVE^_ are  `r mean(PWD_male$mean_co, na.rm = TRUE) /mean(sans_LewRec_male$mean_co, na.rm = TRUE)`,  and `r mean(SKIVE_male$mean_co, na.rm = TRUE) / mean(sans_LewRec_male$mean_co, na.rm = TRUE)` higher than the other strain means. 
...
We applied a series of models to our dataset of mean MLH1 foci counts from `r length(DF.HetC.MixedModel.HQ$mean_co )` mice to fit the patterns of variation across sex, subspecies and strain into a evolutionary framework. 
...
All of the effects within the mixed model M1. were highly significant (LRT: subsp p= `r MLH1.MMlmer_results$sub.sp`, sex p = `r MLH1.MMlmer_results$sex.results`, subspecies * sex p = `r MLH1.MMlmer_results$interaction.pval`) and random LTR; strain p= `r MLH1.MMlmer_results$rand.pval`). Confirming the qualitative observations of wide spread variance. <!-- indicating XXX -->
...
The linear models (M2. and M3.) allow more == specific comparisons
