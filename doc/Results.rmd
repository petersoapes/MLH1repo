...
The mean interfocal distance (IFD) for raw and normalized values was calculated from X female and X male mice.
...
nrm.IFD_sex.comp_BIV.data <- ggplot( (Curated_BivData %>% filter(strain =="KAZ" ) %>%  filter(hand.foci.count == "2") ) , aes(y = IFD1_PER, x= fileName, color=chromosomeLength))+geom_point()+ggtitle("Normalized Interfocal Distances, ")+facet_wrap(~sex, scales = "free")+ylim(c(0,1))+theme(legend.position="none") 
```

strains where males have higher upper threshold, 

G - kinda, little weak

WSB - higher upper threshold (female's have many below the 25% lower threshold, males have a few below 25% threshold)

LEW -- upper threshold looks very similar, much fewer low threshold obs in males, 

PWD -- female lower threshold extends muchlower .25 (like .15?), male lower threshold very different (higher like .3) the male upper threshold is above .75

MSM -- female thresdholds, upper set around .75(to .65) lower at mostly .25, some cells(mice) have more .1 to .2 IFD observatsions.  Male upper threshold it at .75 while lower is set to .3 or .4.
...
KAZ  -- female upper threshold around .75, lower threshold around .25 the distribution is quite uniform and spread out.  Male pattern; upper threshold slightly higher than .75 (.8) and lower threshold seems to extend beyond .25 -- but maybe low sampling contributes to the spread? (the spread in IFD values seems much larger for KAZ males compared to males from other strains.)
