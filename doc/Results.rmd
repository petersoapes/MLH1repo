...
bhh11 <- lm(mean_co ~ subsp * strain, data= fomo.male_mouse.av)
#bhh11$coefficients 

booo <- summary(bhh11)
booo$coefficients

lm.male.strain <- lm(mean_co ~ strain, data= fomo.male_mouse.av)
ll <- summary(lm.male.strain)
ll$coefficients
...
```{r male.show.plots, echo=FALSE, eval=FALSE}
...
top.male <-  G.Bivalents.DF %>% filter(sex== "male") %>% filter(IFD1_PER <= .3) #this makes DF
bottom.male <-  G.Bivalents.DF %>% filter(sex== "male")
ratio.male <- 18/1031 #2%


top.female <-  G.Bivalents.DF %>% filter(sex== "female") %>% filter(IFD1_PER <= .3)
bottom.female <-  G.Bivalents.DF %>% filter(sex== "female")
ratio.female <- 36/766  #5%
