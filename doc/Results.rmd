...
In order to estimate the genome wide recombination rates across our panel of three subspecies from the house mouse species complex, _M. m. domesticus,_ _M. m. musculus_. and _M. m. molossinus_ using `r length(unique(  (main.table  %>% filter(species == "M.musculus") )$strain) )` wild derived inbred strains. MLH1 foci were quantified from a total of `r sum( (main.table %>% filter(species == "M.musculus") %>% filter(sex == "male"))$Ncells)` spermatocytes and `r sum( (main.table %>% filter(species == "M.musculus") %>% filter(sex == "female"))$Ncells)` oocytes. The sampling of prophase oocytes is diffecult and was the limiting factor thus there are some strains with only MLH1 counts from spermatocytes, which are included for sex specific analysises. 
...
Intial examinations of the sex specific data revealed that.. there is more variety in across the strain means in meals compared to females. 

The overall pattern for the female mean MLH1 counts per cell is low variation. 
(the most significantly differnt strains in the female data), For _domesticus^G^_ the G strain is `r G.female.diff` higher the the other means. While _domesticu^LEW^_  is `r round(LEW.female.diff,4)` higher  and _musculus^MSM^_ is `r MSM.female.diff` higher compared to the other strain means in females.

while in the male data the strain average for  _musculus^PWD^_ , _molossinus^MSM^_ and  _musculus^SKIVE^_ are  `r mean(PWD_male$mean_co, na.rm = TRUE) /mean(sans_LewRec_male$mean_co, na.rm = TRUE)`,

`r (mean(MSM_male$mean_co, na.rm = TRUE) / mean(sans_LewRec_male$mean_co, na.rm = TRUE) )`, and `r mean(SKIVE_male$mean_co, na.rm = TRUE) / mean(sans_LewRec_male$mean_co, na.rm = TRUE)` higher than the other strain means respectively.
...
-These results are above the minimum of 20 which is the expected minimum following a obligate crossover per chromosomes.



MOVE THESE MODEL RESULTS WITH THE OTHER MODEL RESULTS

Two glm models were run to test the effects of strain and subspecies on mean MLH1 count per cell per mouse. The coefficients for the following model are below. 

<!--
$$mouse\ variance\ in \ CO \ per cell  ~=~ sex * strain + \varepsilon$$ -->

- There is a low degree of strain varaince in Dom with the range of mouse means `r min( ( male_mouse.av %>% filter(subsp  == "Dom"))$mean_co, na.rm=TRUE  )` to `r max( ( male_mouse.av %>% filter(subsp  == "Dom"))$mean_co, na.rm=TRUE )`


- Musc and Mol have a much larger amount of variance across means with the range in mouse means being `r min( ( male_mouse.av %>% filter(subsp  == "Musc"))$mean_co,na.rm = TRUE  )` to `r max( ( male_mouse.av %>% filter(subsp  == "Musc"))$mean_co,na.rm = TRUE  )` in Musc and `r min( ( male_mouse.av %>% filter(subsp  == "Mol"))$mean_co,na.rm = TRUE  )` to `r max( ( male_mouse.av %>% filter(subsp  == "Mol"))$mean_co, na.rm = TRUE )` and Molossinus.


**general linear models** -- indicate the 3 strains have significant strains effects

Due to this and the significant strain effects they will be designedated into the high rec group for later analyses.
...
--notes on error / 
-- connections to previous measures in the literature

The same model framework was applied to the male specific data.
...
In order to test the effects of subspecies, sex and strain on the mean of MLH1 counts per cell, we fit `r length(DF.HetC.MixedModel.HQ$mean_co )` mouse averages of MLH1 foci per cell to a series of mixed and linear models.  (*note on sampling /sample numbers, the numbers of mice across strains, is shown in supplemental table )
...
```{r MLH1co.lmer.MM, echo=FALSE, warning = FALSE, digits = 3}
...
```{r MLH1.glmM2, echo=FALSE,  digits = 3}
#strain = fixed
...
Our models were designed to test the effects of the subspecies, sex, their interaction, and strain as a random effect on the mouse means. 

All of the effects from the mixed model significant (Table X) 
All of the effects within the mixed model - came out as significant -- (LRT: subsp p= `r MLH1.MMlmer_results$blah`
sex p = `r MLH1.MMlmer_results$blah`, subspecies * sex p = `r MLH1.MMlmer_results$blah`).
(random LTR; strain p= `r MLH1.MMlmer_results$blah`).
...
Following up with linear models which tested for (subsecies M1), and strain effects alone (M3)
indecated ... identified several signifcant sex by strain interaction effects, (MSM and PWD) (glm; PWD*male p= and MSM-male p = )


In both models strain by sex interaction effects were significant for (p = `r summ.strain.fixed$coefficients[,4][12]` and `r summ.strain.fixed$coefficients[,4][13]` for the second mixed model) for _musculus^PWD^_ and _molossinus^MSM^_  respectively. These results confirm the qualitative observation that within these two strains there has been sex-specific evolution in the average MLH1 count per cell. Additionally, _domesticus^G^_ had significant strain and sex by strain effects (p =`r  summ.strain.fixed$coefficients[,4][3]` and `r summ.strain.fixed$coefficients[,4][10]` for strain and strain by sex effect). These results confirm the qualtative pattern of the _domesticus^G^_ strain having the largest degree of sexual dimorphism (heterochiasmy) in the dataset.
...
RENAME THE MODELS M2 subsp * strain \* sex
...
For both the mixed model and linear model, sex was the only significant effect while the subspecies, while interaction and random strain effects were not significant (Table X) (LRT; p= `r round(LmerMLH1_VAR_results$sex.results, 5)`, glm; p= `r round(MLH1var_M3_Q12_sum$coefficients[,4][2], 5)` mixed and glm models respectively).

Since the measures for within mouse variance may be more susceptible to technical error effects from the staining protocol (i.e. background noise). For this reasons, we replicated the model analysis from data sets in the subset of cells with higher quality scores compared to the full data set. (these analysis/ models replicated the results of the full data set (sex being the most significant 
(LTR; p = `r round(LmerMLH1_M1_Q12_VAR_results$sex.results, 5)`, and glm; p = `r  round(MLH1var_M3_Q12_sum$coefficients[2,4], 5)` )

<!--*notes on caveats for calculating variance -- lower power-->
For the estimates of variance, the replicates are at the level of mouse, unlike MLH1 counts, in which the replicates are at the level of the cell. This limits the power for detecting significance differences.

**female strongest effect**, for within mouse variance and coefficient of variance (cv) for both the full and higher quality cell set.

in the sex specific data was analyzed in glm's to test for significance differences in the magnitude of within mouse variance across strains.
The male specific models, showed no significant strain effects

For male specific glms for within mouse variance -- had no significant strain effects
the female data models  --- inconsistant and moderate significance p values 
...
For mean CO count, _domesticus^G^_  strain effects in both models (p = )
_musculus^MSM^_ was a significant strain effect on female mean MLH1 counts per cell for only the second model (p= ).
...
In an attempt to locatize the above male specific crossoever number evolution within the meiotic pathway we quantified a marker for DSBs, DMC1, in early prophase spermtaocytes. DMC1 foci were scored from a total of `r sum( (DMC1.mean.table %>%  filter(stage == "L") )$ncells)` leptotene and `r sum( (DMC1.mean.table %>%  filter(stage == "Z") )$ncells)` zygotene staged spermatocytes of (X for each strain) juvenile mice (12 to 18 days) when the proportion of early prophase cells is higher (cite). A subset of strains were choosen to test for differences between the high and low recombination groups; _musculus^PWD^_, _molossinus^MSM^_, _ musculus^KAZ^_ ,  _domesticus^WSB^_ , and _domesticus^G^_. One mouse was sampled for each strain.


(the differences in mean DMC1 foci across the 2 time points confirms expected results. (t.test for pooled DMC1 foci) )

The high recombining groups have significantly more DMC1 for for leptotene (t.test; p=`r ttest.HighLow.L.pval` , one-way-anova; p=`r round(l.anova.pval, 5)`), but not the zygotene cells (t.test; p=`r ttest.HighLow.Z.pval`, one-way-anova; p=`r z.anova.pval`).  
...
Notably these ratios do not show a significant difference between the high and the low recombining groups (t.test; p = `r DMC1.Lratio_t.test$p.value` and p=`r DMC1.Zratio_t.test$p.value` for leptotene and zygotene based ratios respectively). These results indicate that the best predictor for CO number may be established before DSB formation (cite Baier et al 2014).
...
In order to decompose the cell wide rate, we decided to look at the proportion or chromosomes grouped by the number crossovers on the same bivalent. A total of `r total.biv.all` single bivalents were pooled to calculate the average propotion for each chromosome class (Table X).

**The vast majority of all bivalents in house mouse have one or two crossoveres (1CO an 2CO), even across sexes**

- fits with low rec rates observed in house mouse.

(rates of 0CO, 3CO and 4CO, these are marginal chromsomes classes)


**most of the male proportions are 2/3's 1CO and 1/3 2COs. but The high rec strains are distinguishable by having more 2COs at the cost of 1COs, with closer to 1:1**

**MSM 60%  PWD 50%  SKIVE 30%   20 - 10% remaining (low) strains**

**pvales for chi square test, are larger for PSD-MSM compared to low rec strains within subsp, PWD-KAZ and MSM-MOLF**

**these results confirm the intuitive conclusion/result that the rapid male specific evolution in gwRR proceeds though the chromosome level/though increasing the number of 2CO bivalents at the expense of 1CO bivalents**
...
While the proportions are significantly differennt, PWD male and MSM male proportions are more similar to each other than strains from the same subspcies (PWD and KAZ chi-square test; 1CO p =`r PWD.KAZ.1CO.prop.test$p.value`, 2CO p=`r PWD.KAZ.2CO.prop.test$p.value`)
...
- High female strains, G and Lew have significantly more 2CO bivalents -- there are some differences in the female strains.
...
From our total set of cell images `r length(new.Curate.FULL.org$Obj.ID)` chromosome objects were isolated by the image analysis software. After the human curation step, `r length(new.Curate.FULL.org$Obj.ID)` single bivalent measures were left. (make sure these are the hand.foci bivalents).
...
turn out comparing the means of single bivalents is complicated by the fact we can't identify any of the bivalents. -- so we use total sc
...
The plots below show the distributions of mouse average total SC area.
...
these are the number of mice for each of the categories

`r as.data.frame( table(droplevels( totSC.mouse.av.table$category) ) )`
...
The full data set of SC lengths illustrates the general pattern than females have longer SC-AE.

The most averages for total SC (skeletonizes) are significantly longer in females for all strains (p=
...
$$mouse \ average \ total \ SC  \ length ~=~ subsp * sex + rand(strain) + \varepsilon $$

```{r Q1.lmer_totalSC, echo=FALSE, warning=FALSE}
...
there are more significant effects for strain and subsp for the mixed model of mouse.av total sc

the 3 fixed effects and one random effect were significant (LTR; subsp p= `r Q1.lmer.totSC_results$sub.sp`, 
...
These effects were followed up the glms


**M2.** $$mouse \ average \ total SC  \ area ~=~ sex * strain + \varepsilon $$

**M3.** $$mouse \ average \ total SC  \ area ~=~ sex * strain + \varepsilon $$

```{r Q1.glm_totalSC, echo=FALSE, }
#total-SC

#Q1.lmer_totSC.mouse.av.table

Q1.glm_totSC_M2 <- glm(mean.skel ~ subsp*strain * sex, data=Q1.lmer_totSC.mouse.av.table)
#summary(Q1.glm_totSC_M2)
...
#summary(Q1.glm_totSC_M3)
#LOT OF SIGNIFICant strain effects in this model

```

The results from those models was that there were 
...lots of significant effects.  (G male, )

The summary for the total SC area comparison is...


<!-- transition to short-->

The short bivalent dataset was used -- because there are no confounding factors of the XX -- so makes hapier comparisons across male and females
...
The pattern is generally held for comparisons using the reduced single bivalent data sets (short bivlents are the most comparable set, sinceit is unlikely to include the XX in the female data

with the mouse average for 'short' bivalents

(t.test; p = `r Q1_mus.av_shortSC_t.test.WSB$p.value`, `r format(Q1_mus.av_shortSC_t.test.G$p.value, scientific = F)`, and `r format(Q1_mus.av_shortSC_t.test.LEW$p.value, scientific = F)`
for WSB, G and LEW respectively.

(t.test; p= `r format(Q1_mus.av_shortSC_t.test.PWD$p.value, scientific = F)`, `r Q1_mus.av_shortSC_t.test.KAZ$p.value`, and `r  format(Q1_mus.av_shortSC_t.test.SKIVE$p.value, scientific = F)` for PWD, KAZ, SKIVE

for the short bivalents are not significantly differen, for this strain there are 4 female and 4 male

** there are more SKIVE female mouse observations -- but they are filtered out because I excluded them from the MLH1 dataset.  -- for low quality -- they might have good total SC spreads -- // but 

* it think total sc only has 1 female observation but the short bivalent has 4 skive females

<!--
for the _mmdomesticues^WSB^ _, _mmdomesticues^G^ _, and _mmdomesticues^LEW^ _
-->

Mixed models for the sex differences in mouse averages for short bivalents (reduced dataset)


**M2.** $$mouse \ average \ short \ SC  \ length ~=~ subsp * sex * strain + \varepsilon $$

```{r Q1.SC.Length_M2, echo=FALSE, warning=FALSE}
...
#skive strin very slight sign
#male highly sig
#skive * male slightly sig
```

skive strin very slight sign, male highly sig,  skive * male slightly sig

**M3.** $$mouse \ average \ short \ SC  \ length ~=~ subsp * sex + rand(strain) + \varepsilon $$

```{r Q1_short_biv_lmer, echo=FALSE, warning=FALSE}
...
Sex is the most significant factor for the mixed model of mean short bivalents (LTR; p =`r Q1lmer.MM_SC_short_results$sex.results`). interaction effect also significant (p = `r Q1lmer.MM_SC_short_results$interaction.pval`)


**M3.** $$mouse \ average \ short \ SC  \ length ~=~ sex * strain + \varepsilon $$

```{r Q1.glm_short.SC_M3, echo=FALSE}
...
male is the only significant p value from this model.
...
In summary .... females (oocytes) have different chromatin compaction patterns compared to males.
...
We focus on the foci positions from one crossover (1CO) bivalents since the landscape patterns for multi crossover bivalents will be highly influences by crossover interference. We use the normalized foci positions (smaller meaning closer to the centromere and larger being cloerse to the telomere ). (all chromosomes in house mouse are acrocentric)
...
Below are the general patterns / plots -- short and long bivalent groups are shown to try and account for the chromsomes size effect.
...
```{r Q1show.F1pos_Full, echo=FALSE}
...
are highly significantly more medially placed (t.test; p = `r Q1_1CO.pos_t.test.WSB$p.value`, `r Q1_1CO.pos_t.test.G$p.value`, and `r Q1_1CO.pos_t.test.LEW$p.value` for WSB, G and LEW respectively). With the musclus and molossinus strains displaying a similar pattern (t.test p = `r Q1_1CO.pos_t.test.PWD$p.value` and `r Q1_1CO.pos_t.test.MSM$p.value`) PWD and MSM. Two musculus  strains had a weaker signal, but it was still in the same direction (t.test; p =  `r Q1_1CO.pos_t.test.KAZ$p.value`, and `r Q1_1CO.pos_t.test.SKIVE$p.value` for KAZ and SKIVE respectively). 
...
```{r Q1.CO1pos_lmer.M1, echo=FALSE}
...
```
For the Mixed model, sex and strain are significant effects for the normalized single foci position.
...
```{r Q1.CO1pos_glm.M3, echo=FALSE, eval=FALSE}

summry.Q1.glm_F1pos_M2
...
Lew and KAZ had slightly significant strain speicific effects across the two glms (these results will be explored more fully in the Q2 section). The general pattern that emerges across these models is the sex effect being the most significant, (with the male single foci position being more telomeric compared to females). 
...
The raw IFD measures for 2CO bivalents are slightly significant (t.test; p = `r Q1.IFD.raw_FULL_t.test$p.value`). While the normalized IFD measures are highly significant (t.test; p = `r Q1.IFD.PER_FULL_t.test$p.value`)

(interpertation statement)  Males have stronger interference
...
I made a sereies of plots which show that the expected / general pattern of bivalent SC length is positively associated with having more crossovers per chromsomes (possitive correlation of sc length and CO number).  Yet, given the complications from the chromsomes proportions outlined above, comparing the bivalents across chromosome classes is complecated


```{r juvimice.list.copy, echo=FALSE}
juvi_mice <- c(
  "30dec14_WSB_m2","30dec14_WSB_m4","30dec14_WSB_m1","30dec14_WSB_m3","30dec14_WSB_m5",
  "30sep14_G_m1","30sep14_G_m2","30sep14_G_m3","30sep14_G_m4","30sep14_G_m5",
  "12Feb17_LEW_m1","12Feb17_LEW_m2", "13sep17_LEW_m2", "13sep17_LEW_m1","13sep17_LEW_m3",
...
total SC for males (which ar)
X additional cells were added to the male specifi total SC data set discirbed above

-reduced bivalent dataset (long and short) is made up of bined bivalents from X cells and X mice
...
**we ran t.tests on the SC based metrics to test is the high and low groups are significanly different**
...
- total SC is capturing differences in the XY signal which is excluded from the bivalent dataset
...
(the subspecis effect is tested in M1)
...
strain effects  (basicall all strain effects are significant) (tech effects / error?)
...
In the short bivalent data set -- analyzed by M1, both subspeces are sig (glm; p= Musculus `r sum.glmQ2_short.biv_M1$coefficients[,4][2]` and Molossinus p= `r sum.glmQ2_short.biv_M1$coefficients[,4][3]`). And the strain effec for G is significant (glm; G p =`r sum.glmQ2_short.biv_M1$coefficients[,4][4]`).

For the M2 modeanalyzing the mouse means short bivalents, all strains have significant effect but LEW
...
We tested another aspect of the rec landscape -- focuing on differences between the high and the low rec strains.
...
So try using the bivalent sample levels
...
The t.test at the bivalent level is also NS (test; p = )
