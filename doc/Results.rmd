...
load(file="~./MLH1repo/data/CleanBivData_11.2.20.RData")
...
strain.table.HQ_Fig.og <- ddply(MLH1_data, c("strain", "sex"), summarise,
...
```{r maw.fig1s, echo=FALSE, include=FALSE}
...
In the dom  subspecies the ranges of mouse means are `r min( ( female_mouse.avs_for.analysis %>% filter(strain  == "WSB"))$mean_co  )` to `r max( ( female_mouse.avs_for.analysis %>% filter(strain  == "WSB"))$mean_co  )` in WSB, `r min( ( female_mouse.avs_for.analysis %>% filter(strain  == "G"))$mean_co  )` to `r max( ( female_mouse.avs_for.analysis %>% filter(strain  == "G"))$mean_co  )` in G, `r min( ( female_mouse.avs_for.analysis %>% filter(strain  == "LEW"))$mean_co  )` to `r max( ( female_mouse.avs_for.analysis %>% filter(strain  == "LEW"))$mean_co  )` in LEW.

In the musc subspecies the ranges of mouse means are `r min( ( female_mouse.avs_for.analysis %>% filter(strain  == "PWD"))$mean_co  )` to `r max( ( female_mouse.avs_for.analysis %>% filter(strain  == "PWD"))$mean_co  )` in PWD, `r min( ( female_mouse.avs_for.analysis %>% filter(strain  == "KAZ"))$mean_co  )` to `r max( ( female_mouse.avs_for.analysis %>% filter(strain  == "KAZ"))$mean_co  )` in KAZ. 

The range is `r min( ( female_mouse.avs_for.analysis %>% filter(strain  == "MSM"))$mean_co  )` to `r max( ( female_mouse.avs_for.analysis %>% filter(strain  == "MSM"))$mean_co  )` in MSM.
...
#sans_MSM_female / mean.all.other
...
G is `r mean.G.female / mean.all.other` higher the the other means. LEW is `r mean.LEW.female / mean.all.other` higher  and MSM is `r mean.MSM.female / mean.all.other`. These three will be designated as 'moderate high rec' strains for later sex specific analysis.
...
anova(MLH1.MM.M1, MLH1.MM.M1.reduce.sex)
anova(MLH1.MM.M1, MLH1.MM.M1.reduce.subp)
...
```{r mouse.var.plots.female, echo=FALSE, fig.width = 4, fig.height=2.5}
...
```{r mouse.var.plots.male, echo=FALSE, fig.width = 4, fig.height=2.5}
...
#anova(MLH1var_M1, MLH1var_M1.reduce.sex)
#anova(MLH1var_M1, MLH1var_M1.reduce.subp)
...
#drop1(MLH1cV_M1, test="Chisq") #interaction term NS #when switched to additive fixed, sex is very sig
#anova(MLH1cV_M1, MLH1cV_M1.reduce.sex)
#anova(MLH1cV_M1, MLH1cV_M1.reduce.subp)
...
#anova(MLH1var_M1_Q12, MLH1var_M1_Q12reduce.sex)
#anova(MLH1var_M1_Q12, MLH1var_M1_Q12reduce.subp)
...
pval.sig <- anova(MLH1cv_M1_Q12, MLH1cv_M1_Q12reduce.sex)
CV.anova.p <- pval.sig$`Pr(>Chisq)`[2]

# <- anova(MLH1cv_M1_Q12, MLH1cv_M1_Q12reduce.subp)

#exactRLRT(MLH1cv_M1_Q12)#NS

#make a table of the Mixed model pvalues, which are caluclated for subp, sex, sex:subsp and strain across 4 models ()
```


**General pattern is that sex has a very significant effect.** ( on the order of `r round(CV.anova.p, 6) ` )  this applies to different measures of within animal (across cell) variance and also to subsetted data with higher quality cells.
...
<!-- insert the sex specific variance effect here / maybe also the plots of variance -->
...
Most of the strain specific differences in within mouse variance go away in the Q12 data set.  (These strain effects could indicate that the distribution of across cell variance in CO number evolves.)
...
#round(lm.male.strain_sum$coefficients[,c(1,4)], 5)
#as.data.frame( round(summary(MLH1var_M4)$coefficients[,c(1,4)], 6))
...
#as.data.frame( round(summary(MLH1var_M4_Q12)$coefficients[,c(1,4)], 6))
...
The p-values from the t.tests of the high vs low groups, indicate that the high recombining are significantly higher for the L cells (p value = `r ttest.HighLow.L$p.value`) while the Z cells, there is not a significant difference across the high and low groups (value = `r ttest.HighLow.Z$p.value`).
...
`r data.frame(table( droplevels( Curated_BivData  %>%  filter(hand.foci.count  == 0 | hand.foci.count  == 1 |                                       hand.foci.count  == 2 | hand.foci.count  == 3 | hand.foci.count  == 4 ) )$category ) )`

Validity of comparing bivalent observations
...
lmer.Q1_SCLength_M1_R.sex <- lmer(mean_SC ~ subsp + (1|strain), data=Mouse.Table_BivData_4MM, REML = TRUE) 
lmer.Q1_SCLength_M1_R.subsp <- lmer(mean_SC ~ sex + (1|strain), data=Mouse.Table_BivData_4MM, REML = TRUE) 

drop1(lmer.Q1_SCLength_M1, test="Chisq" ) #other options 

anova(lmer.Q1_SCLength_M1, lmer.Q1_SCLength_M1_R.sex) # also F 
anova(lmer.Q1_SCLength_M1, lmer.Q1_SCLength_M1_R.subsp)
...
Q1.lmer_SCLength_long.biv_M1.R.sex <- lmer(mean_SC ~ subsp + (1|strain), data=Long.biv_mouse_table_HetC, REML = TRUE) 
Q1.lmer_SCLength_long.biv_M1.R.subsp <- lmer(mean_SC ~ sex + (1|strain), data=Long.biv_mouse_table_HetC, REML = TRUE) 

anova(Q1.lmer_SCLength_long.biv_M1, Q1.lmer_SCLength_long.biv_M1.R.sex)
anova(Q1.lmer_SCLength_long.biv_M1, Q1.lmer_SCLength_long.biv_M1.R.subsp)
...
```{r Q1.F1.pos.LONG.BIV_scatter, echo=FALSE}
...
real.long.bivData_1CO <- real.long.bivData %>% filter(species =="M.musculus") %>% filter(hand.foci.count == 1)

#mouse.av.long.biv.table

Q1.show.nrmF1_long.biv <- ggplot(real.long.bivData_1CO, aes(x=strain, y=PER_Foci_1, color=strain) )+geom_jitter( )+ geom_boxplot(aes(y= PER_Foci_1 ),alpha=.3 ) + scale_color_manual(values=colors_of_strains)+facet_wrap(sex~subsp, scales = "free")+ylim(c(0,1.1))+ggtitle("Normalized 1CO Foci pos \nLong Biv")

Q1.show.nrmF1_long.biv
