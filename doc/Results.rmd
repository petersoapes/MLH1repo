...
In all of the strains, the female mouse mean total SC per cell was significantly greater in females than males (ttest; WSB p = `r format(Q1_tot.sc_t.test.WSB$p.value, scientific = F )`, G p = `r format(Q1_tot.sc_t.test.G$p.value, scientific= F )`, LEW p = `r format(Q1_tot.sc_t.test.LEW$p.value, scientific= F )`,  PWD p = `r format(Q1_tot.sc_t.test.PWD$p.value,scientific = F )`, KAZ p = `r format(Q1_tot.sc_t.test.KAZ$p.value, scientific = F )`, MSM `r format(Q1_tot.sc_t.test.MSM$p.value, scientific = F )` 

for the _domesticues^WSB^ _, _domesticues^G^ _, and _domesticues^LEW^ _ respectively), for the _musculus^PWD^ _, _musculus^KAZ^ _, and _molossinus^MSM^_ respectively)
...
The above t.tests were done to establish the sex differences for SC-AE (chromatin condensation), we now apply models (for understanding within an evolutionary framework) -- we apply the same models to both mouse mean short bivalent length and mean mouse total SC.
...
Mixed models for the sex differences in mouse in the mixed models using mean **short bivalents** length as the dependant variable, sex is the most significant effect (LTR; p =`r Q1lmer.MM_SC_short_results$sex.results`). The interaction (subspecies by sex) effect was also slightly significant (p = `r Q1lmer.MM_SC_short_results$interaction.pval`).  

Subsp and sex had the most significant p values, --- For the mixed model run on the total sc metric, there are more significant effects for strain and subsp for the mixed modelusing mean **total sc** per cell as the dependant variable, the three fixed effects and one random effect were significant (LTR; subsp p= `r Q1.lmer.totSC_results$sub.sp`, sex p = `r Q1.lmer.totSC_results$sex.results`, and interaction p = `r Q1.lmer.totSC_results$interaction.pval`, (random LRT; strain p = `r Q1.lmer.totSC_results$rand.pval`).

(these results from the mixed models -- are similar to the mean MLH1 per cell results -- so we follow up -- with tests of significant strain effects).
...
The following effects had significant p values for the **total SC** from the above model (for the linear models M2 and M3), 
subspecies _musculus_ (glm M2; p = `r sum.Q1.glm_totSC_M2$coefficients[,4][2]`), subspecies _molossinus_ (glm M2; p =`r sum.Q1.glm_totSC_M2$coefficients[,4][3]`), sex male (glm M2; p =`r sum.Q1.glm_totSC_M2$coefficients[,4][8]`), strain lew p =`r sum.Q1.glm_totSC_M2$coefficients[,4][5]`, strain g  p =`r sum.Q1.glm_totSC_M2$coefficients[,4][4]`,

For M3 -- many strain effects are significant, but the largest are sex (glm M3; p = `r sum.Q1.glm_totSC_M3$coefficients[,4][8]`)
but also some strain effects, G strain (glm M3; p = `r sum.Q1.glm_totSC_M3$coefficients[,4][2]`), LEW strain (glm M3; p = `r sum.Q1.glm_totSC_M3$coefficients[,4][3]`), PWD strain (glm M3; p = `r sum.Q1.glm_totSC_M3$coefficients[,4][4]`).

Supporting the strong sex difference and also indicating that SC length has evolved across strains within this panel. when chromatin condensation is summarized / quantified at this level. (can't exclude the possibility that the XY morphology is different between these strains)
