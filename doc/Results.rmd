...
mouse.av_Tot.SC <- ggplot(data= Q1.totSC.mouse.av.table, aes(y = mean.skel, x=mouse, color=strain))+ geom_point()+scale_color_manual(values=colors_of_strains)+
  facet_wrap(sex~subsp, scales="free")+ 
  theme(axis.title.x=element_blank(),        axis.text.x=element_blank(), axis.ticks.x=element_blank())+  ylim(c(0,3000 ))+ggtitle("Mouse av. total SC area")

mouse.av_Tot.SC
```

<!-- can't figure out the ordering -->
