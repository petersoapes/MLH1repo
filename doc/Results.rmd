...
`r format(Q1_tot.sc_t.test.WSB$p.value, scientific = F )`, `r format(Q1_tot.sc_t.test.G$p.value, scientific=F )`, and `r Q1_tot.sc_t.test.LEW$p.value`
for the _mmdomesticues^WSB^ _, _mmdomesticues^G^ _, and _mmdomesticues^LEW^ _ respectively).

Some of the p values for PWD and MSM are much closer (less significant), ( p = `r Q1_tot.sc_t.test.PWD$p.value`, `r Q1_tot.sc_t.test.KAZ$p.value`, and `r Q1_tot.sc_t.test.MSM$p.value` for the _M. m. musculus^PWD^ _, _M. m. musculus^KAZ^ _, and _M. m. molossinus^MSM^_ respectively)


$$mouse \ average \ total \ SC  \ length ~=~ subsp * sex + rand(strain) + \varepsilon $$

```{r Q1.lmer_totalSC, echo=FALSE}
...
$$mouse \ average \ total SC  \ area ~=~ sex * strain + \varepsilon $$
...
When skive is plotted alone it does have overlapping ranges.
...
(t.test; p = `r Q1_mus.av_shortSC_t.test.WSB$p.value`, `r Q1_mus.av_shortSC_t.test.G$p.value`, and `r Q1_mus.av_shortSC_t.test.LEW$p.value`
for WSB, G and LEW respectively.

(t.test; p= `r Q1_mus.av_shortSC_t.test.PWD$p.value`, `r Q1_mus.av_shortSC_t.test.KAZ$p.value`, and `r Q1_mus.av_shortSC_t.test.SKIVE$p.value` for PWD, KAZ, SKIVE
...
$$mouse \ average \ short \ SC  \ length ~=~ subsp * sex + rand(strain) + \varepsilon $$
...
Sex is the most significant factor for the mixed model of mean short bivalents.


$$mouse \ average \ short \ SC  \ length ~=~ subsp * sex * strain + \varepsilon $$
...
$$mouse \ average \ short \ SC  \ length ~=~ sex * strain + \varepsilon $$
