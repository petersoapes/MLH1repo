...
We used counts of MLH1 foci per cell to estimate the genome wide recombination rates across our panel of `r length(unique(  (main.table  %>% filter(species == "M.musculus") )$strain) )` wild derived inbred strains sampled from thress subspecies within the house mouse species complex;  _M. m. domesticus,_ _M. m. musculus_. and _M. m. molossinus_. MLH1 foci were quantified from a total of `r sum( (main.table %>% filter(species == "M.musculus") %>% filter(sex == "male"))$Ncells)` spermatocytes and `r sum( (main.table %>% filter(species == "M.musculus") %>% filter(sex == "female"))$Ncells)` oocytes. The sampling of prophase oocytes is diffecult and was the limiting factor thus there are some strains with only MLH1 counts from spermatocytes, which are included for sex specific analysises. 

<!--
more basic stats, error, comparison to other measures

transition first aragraph to strain specific
Main points
...
<!-- transition from first part into female -->
...
Intial examinations of the sex specific data revealed that greater variety in across the strain means in males compared to females. In females, _domesticus^G^_, _musculus^MSM^_, and _domesticu^LEW^_ had the greatest differences in means compared to other strains, `r G.female.diff`, `r round(LEW.female.diff,4)`, and 
`r MSM.female.diff` respectively. While in the male data the strain average for  _musculus^PWD^_ , _molossinus^MSM^_ and  _musculus^SKIVE^_ are  `r mean(PWD_male$mean_co, na.rm = TRUE) /mean(sans_LewRec_male$mean_co, na.rm = TRUE)`, `r (mean(MSM_male$mean_co, na.rm = TRUE) / mean(sans_LewRec_male$mean_co, na.rm = TRUE) )`, and `r mean(SKIVE_male$mean_co, na.rm = TRUE) / mean(sans_LewRec_male$mean_co, na.rm = TRUE)` higher than the other strain means. 

<notabley these strains come from Musc and Mol -- Mol natural hybrid of musculus and castaneus. this comments out text?>

The musculus and molossinus subspecies have a much larger amount of variance across means with the range in mouse means being `r min( ( male_mouse.av %>% filter(subsp  == "Musc"))$mean_co,na.rm = TRUE  )` to `r max( ( male_mouse.av %>% filter(subsp  == "Musc"))$mean_co,na.rm = TRUE  )` in Musc and `r min( ( male_mouse.av %>% filter(subsp  == "Mol"))$mean_co,na.rm = TRUE  )` to `r max( ( male_mouse.av %>% filter(subsp  == "Mol"))$mean_co, na.rm = TRUE )` and Molossinus.
...
<to better understand this varation in RR within an evolutionary framework>
...
Our model, M1., was designed to test the effects of the subspecies, sex, their interaction, and strain as a random effect on the mouse means. 
...
All of the effects within the mixed model were significant (LRT: subsp p= `r MLH1.MMlmer_results$sub.sp`, sex p = `r MLH1.MMlmer_results$sex.results`, subspecies * sex p = `r MLH1.MMlmer_results$interaction.pval`) and random LTR; strain p= `r MLH1.MMlmer_results$rand.pval`).
...
MLH1.M3.strain.fixed <- lm(mean_co ~ sex * strain, data=DF.HetC.MixedModel.HQ)
...
Following up with linear models which tested for (subsecies M2), and strain effects alone (M3)
both showed similar -- results -- **so we just report those from M3. also just report effects which were consisantly significant across both M2 and M3**

<they also have sig strain effects .. PWD strain p = `r summ.MLH1.M3.strain.fixed$coefficients[,4][5]` and MSM strain p = `r summ.MLH1.M3.strain.fixed$coefficients[,4][6]`>

which identified several signifcant sex by strain interaction effects, ( _musculus^PWD^_*male and  _musculus^MSM^_*male ) (glm; p = `r summ.MLH1.M3.strain.fixed$coefficients[,4][12]`
and p = `r summ.MLH1.M3.strain.fixed$coefficients[,4][13]`
)
...
These results confirm the qualitative observation that within these two strains there has been sex-specific evolution in the average MLH1 count per cell. 

<M3 also has a few less significant effects>
<not the reverse coefficient indexing -- this will give the name >
...
_domesticus^G^_ had significant strain (p= `r summ.MLH1.M3.strain.fixed$coefficients[4,][3]`)
...
-->
<!-- glms for sex specific data also run -->

A similar glm models (M4) was run on the sex specific data set, (just containing subspecies, strain and the interaction).

 **M4.** $$sex\ specific\ mouse \ av.\ MLH1\ foci\ per\ cell ~=~ subspecies * strain + \varepsilon$$

For the male specific data, three of the strain effects of 13 strains had significant effects on mean MLH1 per cell, PWD (p=`r sum_MLH1.male.specific.glm.M2$coefficients[5,][4]`), SKIVE (p=`r sum_MLH1.male.specific.glm.M2$coefficients[8,][4]`) and MSM (p=`r sum_MLH1.male.specific.glm.M2$coefficients[6,][4]` ). (with effect sizes ranging from 5, 7, and 2 foci respectively). Given these results, theses strains are classified as 'high rec' strains and point to rapid evolution in the recombiantion rate for spermatocytes.

While the female specific linear model has many more had significant strain effects; _domesticus^G^_ (p=`r sum_MLH1.female.spc.glm.M2$coefficients[2,][4]`), _domesticus^LEW^_ (p=`r sum_MLH1.female.spc.glm.M2$coefficients[3,][4]`), _domesticus^PWD^_ (p=`r sum_MLH1.female.spc.glm.M2$coefficients[4,][4]`), _domesticus^MSM^_(p=`r sum_MLH1.female.spc.glm.M2$coefficients[5,][4]`), _domesticus^MOLF^_(p=`r sum_MLH1.female.spc.glm.M2$coefficients[6,][4]`), and _domesticus^KAZ^_(p=`r sum_MLH1.female.spc.glm.M2$coefficients[8,][4]`) the effect sizes have a smaller range of four to one, indicateing specific, but small strain effects for the mean number of MLH1 foci per cell. 
...
In order to more fully discribe the MLH1 count per cell distributions we examined the variance applying the same mixed models and linear models, using the within mouse variance of MLH1 counts per cell as the dependant variable (Table 1).
...
Following the same basic framework we used for analyzing mean number of MLH1 foci per cell, we applied a set of mixed and linear models -- to test effects (sex differences for the within mouse variance in MLH1 foci per cell).
...
For both the mixed and linear models, sex was the only significant effect (LRT; p= `r round(LmerMLH1_VAR_results$sex.results, 5)`, glm; p= `r round(MLH1var_M3_Q12_sum$coefficients[,4][2], 5)` mixed and glm models respectively). Since the measures for within mouse variance may be more susceptible to technical error effects from the staining protocol (i.e. background noise). For this reasons, we replicated the model analysis using a subset of cells with higher quality scores. These models replicated the results of the full data set with sex being the most significant effect (LTR; p = `r round(LmerMLH1_M1_Q12_VAR_results$sex.results, 5)`, and glm; p = `r  round(MLH1var_M3_Q12_sum$coefficients[2,4], 5)` ). Sex specific effect linear models indicate that there is no significant difference in the amount of within mouse variance in males. While in females, one strain, _domesticus^LEW^_ has significantly higher within mouse variance (glm; p = `r lmVAR.female.strain_sum$coefficients[3,][4]`) for the model with the full dataset. While in the model using the high quality cells, PWD has the only signicant strain effect (glm; p =`r lm.m2.Q12VAR.female_sum$coefficients[4,][4]`) suggesting a lack of a consistant signal for differences in the amount of within mouse variance in oocytes. For the estimates of variance, the replicates are at the level of mouse, unlike MLH1 counts, in which the replicates are at the level of the cell. This limits the power for detecting significance differences across groups, <but the main/general pattern, is that females have significantly higher within mouse variance compared to males. This general pattern is replicated in higher quality dataset and using the coefficient of variance (cv) as a dependant variable.
...
In an attempt to locatize the above male specific crossover number evolution within the meiotic pathway we quantified a marker for DSBs, DMC1, in early prophase spermtaocytes. DMC1 foci were scored from a total of `r sum( (DMC1.mean.table %>%  filter(stage == "L") )$ncells)` leptotene and `r sum( (DMC1.mean.table %>%  filter(stage == "Z") )$ncells)` zygotene staged spermatocytes form juvenille mice (12 to 18 days). A subset of strains were choosen to test for differences between the high and low recombination groups; _musculus^PWD^_, _molossinus^MSM^_, _musculus^KAZ^_ ,  _domesticus^WSB^_ , and _domesticus^G^_. One mouse was sampled for each strain.

For all strains there were significantly more foci from leptotene cells compared to zygotene (t.test; p = ), which reflects the expected patterns across the timing of meiosis.

Confirming the prediction that (more crossovers are associated with more initial DSBs), the two high recombining strains tested have significantly more DMC1 foci compared to low recombining strains in leptoene cells (t.test; p=`r ttest.HighLow.L.pval` , one-way-anova; p=`r format(l.anova.pval, scientific=F)`).

The differences in DMC1 foci were not significant for zygotene cells between the two groups (t.test; p=`r ttest.HighLow.Z.pval`, one-way-anova; p=`r z.anova.pval`).  <!-- which maybe be attributed to noise effects 
...
The ratios of average number of MLH1 and DMC1 foci per cell -- have  .... biological significance

The ratios, calculated for DMC1 means from both stages of prophase, are not significantly differet between the strain groups (t.test; p = `r DMC1.Lratio_t.test$p.value` and p=`r DMC1.Zratio_t.test$p.value` for leptotene and zygotene based ratios respectively). 


These results indicate that the changes -- which lead to differences in crossover numbers may be established before DSB formation (cite Baier et al 2014).
...
In order to decompose the cell wide rate, we decided to look at the proportion of chromosome classes (ie by the number crossovers on the same bivalent). A total of `r total.biv.all` single bivalents were pooled to calculate the average propotion for each chromosome class (Table X).

The first attern we notice from looking at theis large number of samples is that The vast majority of all bivalents in house mouse have one or two crossoverrs (1CO an 2CO), even across sexes. <these results fit with previous observations on the number of crossovers per chromosome (Stapley 2019)>

<note about the rates of 0CO, 3CO and 4CO? these are marginal chromsomes classes / we'll discuss later?)>
...
From our total set of cell images `r length(new.Curate.FULL.org$Obj.ID)` chromosome objects were isolated by the image analysis software. After the human curation step, `r length(Curated_BivData$Obj.ID)` single bivalent measures were left. (make sure these are the hand.foci bivalents).

<how do these agree with other patterns of MLH1>
...
The ranges of of rates of isolating chromosomes per cell are `r min(algorithm.stat.category$mean_pass.rate)` in MSM male to 
`r max(algorithm.stat.category$mean_pass.rate)` ) KAZ female, we assume that the isolation process is not biased. Because there are hundreds of observations per category, we assume that each of the 19 autosomes (chromosomes) is equally represented in the dataset of single bivalents. We assume the bivalent datasets across all categories are equally eqivilent for representing general patterns.
...
For this reason we use a reduced bivalent data set made up of chromsomes (below the 2nd quartile for SC length of single cells). A total of `r sum(Q1.real.short_mouse_table$n_obs)` 'short' bivalents were isolated from `r sum(Q1.real.short_mouse_table$ncells)` oocytes and spermatocytes.

Even though this smaller data set has decreased power due to redcued sample size, ...
...
For all but one strain, _musculus^SKIVE^_, the mouse mean for the short bivalents are significantly longer <how much longer, the largest difference across sexes by strain are>  (t.test; p = `r Q1_mus.av_shortSC_t.test.WSB$p.value`, `r format(Q1_mus.av_shortSC_t.test.G$p.value, scientific = F)`, and `r format(Q1_mus.av_shortSC_t.test.LEW$p.value, scientific = F)`
for _domesticus^WSB^_, _domesticus^G^_ and _domesticus^LEW^_ respectively.
and (t.test; p= `r format(Q1_mus.av_shortSC_t.test.PWD$p.value, scientific = F)`, `r Q1_mus.av_shortSC_t.test.KAZ$p.value`, and `r  format(Q1_mus.av_shortSC_t.test.SKIVE$p.value, scientific = F)` for _musculus^PWD^_ , _musculus^KAZ^_ , _musculus^SKIVE^_  respectively. 
...
<and the short biv>
Mixed models for the sex differences in mouse averages for short bivalents Sex is the most significant factor for the mixed model of mean short bivalents (LTR; p =`r Q1lmer.MM_SC_short_results$sex.results`). interaction (subspecies by sex) effect also significant (p = `r Q1lmer.MM_SC_short_results$interaction.pval`).
...
```{r Q1.glm_totalSC, echo=FALSE, message=FALSE}
#total-SC
...
The following effects had significant p values given the above model
...
These results indicate that there are strain specific effects for the SC-AE area -- 

the large G strain effect is likely driven by the high female mean total SC area

sex is the next strongest (most significant) effect.

<!--
for the _mmdomesticues^WSB^ _, _mmdomesticues^G^ _, and _mmdomesticues^LEW^ _
-->

For the short bivalent data -- run through the same models...

```{r Q1.SC.Length_M2, echo=FALSE, warning=FALSE, message=FALSE}
...
From this (M2) model sex effect, male is most significant (glm; p =`r summ.Q1.glm_short.SC_M2$coefficients[,4][10]`). while the skive strain effect (glm; p=`r summ.Q1.glm_short.SC_M2$coefficients[,4][8]`) and and SKIVE*male (glm; p =`r summ.Q1.glm_short.SC_M2$coefficients[,4][16]`) are slightly significant.

In the M2 model, the Sex effect (male) is the only significant p value from this model (glm; p= `r summ.Q1.glm_short.SC_M3$coefficients[,4][10]`).
<should apply caution to the short bivalent dataset> The significant SKIVE effect is driven by  2 of the 4 female skive mice -- having much shorter bivalents comapred to the other female musc strain means.



<SUMMARY, ttests and models indicate female have strongest effect -- longer AC-AE.  >
...
```{r Q1.SC.long.biv_wacko, echo=FALSE, include=FALSE, eval=FALSE}

#REAL.long_mouse_table

#removed these plots for space
Q1_cell_long.biv <- ggplot(REAL.long_mouse_table, aes(y=mean.sc, x=strain, color=strain))+geom_jitter()+facet_wrap(sex~subsp, scales = "free")+scale_color_manual(values=colors_of_strains)+ylim(c(75,200))+geom_boxplot(alpha=.2) + ggtitle("Cell averages for Long Bivs")+theme(legend.position="none")
...
#cell.avlong.biv <- ggplot( (REAL.long_mouse_table %>% filter(sex == "male")), aes(y=mean.sc, x=strain, color=strain))+geom_jitter()+facet_wrap(~subsp, scales = "free")+scale_color_manual(values=colors_of_strains)+ylim(c(75,140))+geom_boxplot(alpha=.2) + ggtitle("Cell av. \nMale Long Biv SC Lengths")
  
#mouse av for long biv

mouse.show.long.biv <- ggplot((REAL.long_mouse_table %>% filter(sex == "male")), aes(y=mean.sc, x=strain, color=strain))+geom_jitter()+facet_wrap(~subsp, scales = "free")+scale_color_manual(values=colors_of_strains)+ylim(c(75,140))+geom_boxplot(alpha=.2) + ggtitle("Mouse av. \nMale Long Biv SC Lengths")
...
```{r Q1_long_biv_lmer, echo=FALSE, message=FALSE, include=FALSE}

Q1.real.long_mouse_table <- REAL.long_mouse_table
...
We anchor the position measures at the centromere (and normalize by deviding the position by the length of the chromosome. The foci positions then have a range of 0-1, with smaller values representing positions closer to the centromere end of the bivalent, and larger values meaning foci positions closer to the opposite end (refered here as telomeric position).
...
In the domesticus strains the mouse averages for normalized single foci positions were significantly sexually dimorphic (with female)
are highly significantly more medially placed (t.test; p = `r Q1_1CO.pos_t.test.WSB$p.value`, `r Q1_1CO.pos_t.test.G$p.value`, and `r Q1_1CO.pos_t.test.LEW$p.value` for WSB, G and LEW respectively). With the musculus and molossinus strains displaying a similar pattern (t.test p = `r Q1_1CO.pos_t.test.PWD$p.value` and `r Q1_1CO.pos_t.test.MSM$p.value`) PWD and MSM. Two musculus  strains had a weaker signal, but it was still in the same direction (t.test; p =  `r Q1_1CO.pos_t.test.KAZ$p.value`, and `r Q1_1CO.pos_t.test.SKIVE$p.value` for KAZ and SKIVE respectively). 
...
motivation:
the positive correlation, Our prediction for this metric is that the high rec strain will have longer sc lengths (axis lengths)

<pattern for the single bivalent, pos correlation>
...
<NEW STRAINS INTRODUCED NOT IN PREV DATA>

this section / these tests follow up with those made in Q1 which found that the male musc and mol total-SC were signifcantly higher compared to the male Dom total SC measures.
These test include additional measues from strains which only had male observations.
...
total SC for males (which are), X additional cells were added to the male specific total SC data set described above

-reduced bivalent data set (long and short) is made up of binned bivalents from X cells and X mice

<!-- place set ups for total SC, short biv and long biv here -->

```{r Q2.SC.juvimice.list.copy, echo=FALSE}
juvi_mice <- c(
  "30dec14_WSB_m2","30dec14_WSB_m4","30dec14_WSB_m1","30dec14_WSB_m3","30dec14_WSB_m5",
  "30sep14_G_m1","30sep14_G_m2","30sep14_G_m3","30sep14_G_m4","30sep14_G_m5",
  "12Feb17_LEW_m1","12Feb17_LEW_m2", "13sep17_LEW_m2", "13sep17_LEW_m1","13sep17_LEW_m3",
...
ppmm <- OG.totSC.mouse.av.table[OG.totSC.mouse.av.table$mouse  %in% juvi_mice, ]   
```
...
I made a series of plots which show that the expected / general pattern of bivalent SC length is positively associated with having more crossovers per chromosome (positive correlation of SC length and CO number).  Yet, given the complications from the chromosome proportions outlined above, comparing the bivalents across chromosome classes is complicated
...
The average SC lengths increase with the number of MLH1 foci / chromosome classes for all strains, but grouping bivalent observations by chromosome class (foci number) is confounded by the different proportions across strains.

total SC t test
 p = `r Q2tot.sc_t.test_High.Low$p.value`
...
Additionally we ran similar tests using the short and long bivalent data sets which help to alimeorate potential chromosome size effects. 

**The mouse means for the reduced bivalent datasets are not significantly different between the high and low rec strains (ttest; short p = `r Q2short.biv_t.test_High.Low$p.value` and long p = `r Q2long.biv_t.test_High.Low$p.value`). (the summary of mouse averages for this subset of ) **

<this might indicate -there's error in the total SC algorithm
- mouse effect
- low power / sampling bias in reduced data set
- total SC is capturing differences in the XY signal which is excluded from the bivalent data set>
...
#sum.Q2.glm_mus.av_totSC_M1.coef <- 
...
<we ran models to start examine strain specific effects ... modeling the fixed effects of subspeices and strain.>

Thse are our prediction for the glm models -- based on the variation of the gwRR's

1. no difference in DOM strains (for SC lengths (CONFIRM IF THERE ARE DIFFERENECES FOR GWRR))

2. PWD > SKIVE > the rest

3. Msm > mol

(the subspecies effect is tested in M1)
For M2 (glm including subsp), the two subspecies effects are significant (glm; p= Musculus `r sum.Q2.glm_mus.av_totSC_M1[,4][2]` and Molossinus p= `r sum.Q2.glm_mus.av_totSC_M1[,4][3]`)   

strain effects from M1; G, LEW, MSM, SKIVE and TOM

strain effects  (basically all strain effects are significant) (tech effects / error?)
All strain effects were significant when tested in M2 (Supplemental table X)
...
In the short bivalent data set -- analyzed by M1, both subspecies are sig (glm; p= Musculus `r sum.glmQ2_short.biv_M1$coefficients[,4][2]` and Molossinus p= `r sum.glmQ2_short.biv_M1$coefficients[,4][3]`). And the strain effect for G is significant (glm; G p =`r sum.glmQ2_short.biv_M1$coefficients[,4][4]`).

For the M2 model analyzing the mouse means short bivalents, all strains have significant effect but LEW
G,PWD, MSM, MOLF, SKIVE, KAZ, CZECH.


<Compare by chrm class, but only for strains with similar proportions>

We purpose we think that running the models for comparing SC means by chromosome class and divided by rec group. (since the chromosome proportions are similar -- hopefully/we assume the the sets of chromosomes within each set are more similar )

The null hypothesis is that these won't be highly significant strain effects.
If there are still strain effects for these models, that would be evidence that there has been evolution for SC length compaction across these strains
...
-the SKIVE are within dom and kaz level -- so why does SKIVE have intermediate CO proportions and gwRR
...
-- permute the data, sample just 3 mice per category, which have similar numbers of cells represented (or down sample)

above are glm tables for mouse.av SC lengths for each strain.
...
<lots of variation across SC lengths -- doesn't fit clean expectation or signal>
<balancing the interpertation of general positive correlation of CO number and SC length (SC length drives CO number) --- yet the pattern breaks down -it's not completely predictive -- /at finer levels?>  <noise/mouse effects/chromosome identity effects>
...
```{r Q2.SC.Length.stuff, echo=FALSE, include=FALSE}
#REMOVE THIS, COMPARES FULL BIVDATA ACROSS CHRM CLASS
...
```{r Q2.SC.ploy.male, echo=FALSE, include=FALSE}
#REMOVE THIS< COMARES FROM FULL BIVDATA set
...
```{r SC.length.male_tests, echo=FALSE, include=FALSE}
...
```{r SC.length.pairwise.T, echo=FALSE, include=FALSE}
...
# breief transition, rec landscape

the different lack of predictive power of SC length to predict CO number -- indicates that there might be different patterns of 'SC usage' / different patterns in the layout of the recombination landscape /  these patterns could stem from placement or spacing of foci.
...
We tested another aspect of the rec landscape -- focusing on differences between the high and the low rec strains.

To detect if there are significant differences between the high and the low rec strains we performed t.tests across the two groups 
...
```{r Q2.1CO.show_plots, echo=FALSE}
#Curated_BivData_male
...
```{r Q2.1COpos.LONG.BIV, echo=FALSE}
...
#mouse.avs_4MM
#31may19_SKIVE_m2  has NA for norm.first.foci.pos
...
#short
Q2.1COpos_short_ttest_HL <- t.test( (Q2mouse.av.short.biv.table  %>% filter(Rec.group == "0"))$norm.first.foci.pos, 
            (Q2mouse.av.short.biv.table %>% filter(Rec.group == "1"))$norm.first.foci.pos   )
#p-value = 0.06
...
#long
Q2mouse.av.long.biv.table_HL  <-  t.test( (Q2mouse.av.long.biv.table  %>% filter(Rec.group == "0"))$norm.first.foci.pos, 
            (Q2mouse.av.long.biv.table %>% filter(Rec.group == "1"))$norm.first.foci.pos   )
#p-value = 0.5

#long exclude skive  Q2mouse.av.long.biv.table
Q2mouse.av.long.biv.table_HL.ex  <-  t.test( (Q2mouse.av.long.biv.table  %>% filter(Rec.group.exclu == "0"))$norm.first.foci.pos, 
            (Q2mouse.av.long.biv.table %>% filter(Rec.group.exclu == "1"))$norm.first.foci.pos   )
#p-value = 0.08

#they kinda have ooposite effects

```

```{r Q2.1COpos_BIV_ttest, echo=FALSE}
...
t.tests for high and Low rec strains
...
The t.tests are not significant for normalized foci position comparing all bivalets
...
The t-tests for the high and low rec strains for the reduced bivalent datasets are mostly NS

**Most all of the t.tests I've done indecate that there are no sig diff between the High and the low rec groups**

For normalized position of 1CO. 
The normalized 1CO position is not a good metric to distinguish the high and low rec groups.


<!-- GLMs -->

<toal data set, short and long biv>
...
glms are constructed to test if there has been evolution between strains ... ect.
I will use the full data set the reduced datasets have such low power

for M1
Mol subsp is significant, strain G (LEW slight sig), strain MSM -- strain MOLF is NA

for M2,

G, LEW, PWD, MSM, MOLF, SKIVE, KAZ have significant effects

put together, these glm's indeceate that MOLF is the most different in it's 1CO landscape (compared to other males). Is this a sampling issue
...
<motivation background / fit more COs along a chromsome>

t.test, are the high and low rec strains different?

-glms -- are predictions based on the gwRR met for interference
...
<below chunk goes with the above chunk>
