...
MLH1.MM_strain)
```
(not suprisingly) All of the effects within the mixed model M1. were highly significant (LRT: subsp p= `r MLH1.MMlmer_results$sub.sp`, sex p = `r MLH1.MMlmer_results$sex.results`, subspecies * sex p = `r MLH1.MMlmer_results$interaction.pval`) and random LTR; strain p= `r MLH1.MMlmer_results$rand.pval`). Confirming the qualitative observations of wide spread variance. <!-- indicating XXX -->
...
allow us to test / quantifying the significance and focus on how specific levels and interactions create these differences.

The linear models (M2. and M3.) had similar signifncat effects, so we just show the results from M3. In addtion to significant strain effects, ( p = `r summ.MLH1.M3.strain.fixed$coefficients[,4][5]` and MSM strain p = `r summ.MLH1.M3.strain.fixed$coefficients[,4][6]`)  both PWD and MSM had significant strain by sex effects ( _musculus^PWD^_*male and  _musculus^MSM^_*male ) (glm; p = `r summ.MLH1.M3.strain.fixed$coefficients[,4][12]`
...
A linear model was run on the sex specific data set (M4), (just containing subspecies, strain and the interaction). X number of male and female mean MLH1 foci counts were used in each of these moels.

For the male specific data, (the same PWD, MSM strain effects were signifciant -in addition to SKIVE (glm; p = `r sum_MLH1.male.specific.glm.M2$coefficients[8,][4]`)(which is intrmediate)) . Given these results, theses strains are classified as 'high rec' strains and point to rapid evolution in the recombiantion rate for spermatocytes.

<three of the strain effects of 13 strains had significant effects on mean MLH1 per cell, PWD (glm; p=`r sum_MLH1.male.specific.glm.M2$coefficients[5,][4]`), SKIVE (glm; p=`r sum_MLH1.male.specific.glm.M2$coefficients[8,][4]`) and MSM (glm; p=`r sum_MLH1.male.specific.glm.M2$coefficients[6,][4]` ). (with effect sizes ranging from 5, 7, and 2 foci respectively).>
...
MLH1 mean -- is quantitative trait -- variance of a trait can contain information pertinant to it's mode of evolution -- and mechanistically could reflect the strength of constraints acting on the trait.
<in some traits, like gene expression patterns, evolution of the mean -- also has an effect on the variance>

We examine the within animal variance in MLH1 foci count per cell within the same models applied to mean MLH1 foci count (replacing mean MLH1 foci count with variance of MLH1 foci across cells as the dependant variable). <note about reduced power>
...
<The greater amount of within mouse variance in females (almost and 
(males have no detecable difference across strains for within mouse variance)
...
The first pattern we notice from looking at theis large number of samples is that The vast majority of all bivalents in house mouse have one or two crossoverrs (1CO an 2CO), even across sexes. <these results fit with previous observations on the number of crossovers per chromosome (Stapley 2019)>
