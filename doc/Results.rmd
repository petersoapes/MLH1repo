...
The variance across mouse means for each strain is also variable across strains. With _domesticus^LEW^_ having the most significant strain effect in the second model (p= ).


The overall pattern for the female mean MLH1 counts per cell is low variation. 
(the most significantly differnt strains in the female data), For _domesticus^G^_ the G strain is `r G.female.diff` higher the the other means. While _domesticu^LEW^_  is `r round(LEW.female.diff,4)` higher  and _musculus^MSM^_ is `r MSM.female.diff` higher compared to the other strain means.
...
While the male pattern shows more variation across the strain means.
...
The strain average for  _musculus^PWD^_ , _molossinus^MSM^_ and  _musculus^SKIVE^_ are  `r mean(PWD_male$mean_co, na.rm = TRUE) /mean(sans_LewRec_male$mean_co, na.rm = TRUE)`,
...
```{r MLH1co.MM, echo=FALSE, warning = FALSE, digits = 3}
...
All of the effects from the mixed model significant (Table X) so we followed up with general linear models. The first glm contained subspecies, strain, sex, and all interactions, while the second glm, contains coefficients for strain, sex and the interactions. (*what are the points and differences? )*
...
For both the mixed model and linear model, sex was the only significant effect while the subspecies, while interaction and random strain effects were not significant (Table X) (LRT; p= `r LmerMLH1_VAR_results$sex.results`, glm; p= `r MLH1var_M3_Q12_sum$coefficients[,4][2]` mixed and glm models respectively).
...
(LTR; p = `r LmerMLH1_M1_Q12_VAR_results$sex.results`, and glm; p = `r  MLH1var_M3_Q12_sum$coefficients[2,4]` )
...
In an attempt to locatize the above male specific crossoever number evolution within the meiotic pathway we quantified a marker for DSBs, DMC1, in early prophase meiocytes.

DMC1 foci were scored from a total of `r sum( (DMC1.mean.table %>%  filter(stage == "L") )$ncells)` leptotene and `r sum( (DMC1.mean.table %>%  filter(stage == "Z") )$ncells)` zygotene staged spermatocytes of (X for each strain) juvenile mice (12 to 18 days). 

A subset of strains were choosen to test for differences between the high and low recombination groups; _musculus^PWD^_, _molossinus^MSM^_, _ musculus^KAZ^_ ,  _domesticus^WSB^_ , and _domesticus^G^_. One mouse was sampled for each strain.


As can be seen in the figure, the high recombining groups have significantly more DMC1 for for leptotene (t.test; `r ttest.HighLow.L.pval` , one-way-anova; `r round(l.anova.pval, 5)`), but not the zygotene cells (t.test; `r ttest.HighLow.Z.pval`, one-way-anova; `r z.anova.pval`).  


The ratios for MLH1:DMC1^Leptotene^ and MLH1:DMC1^zygotene^ are shown in Table X, and can be used as a proxy for the CO:NCO ratio (instead CO:(CO+NCO) ratio). Notably there is not much variation and do not show a significant difference between the high and the low recombining groups (t.test; p = `r DMC1.Lratio_t.test$p.value` and `r DMC1.Zratio_t.test$p.value` for leptotene and zygotene based ratios respectively). These results indicate that the best predictor for CO number may be established before DSB formation (cite Baier et al 2014).
...
**most of the male proportions are 2/3's 1CO and 1/3 2COs. **
...
**The high rec strains are distinguishable by having more 2COs at the cost of 1COs, with closer to 1:1**
...
`r Q1_tot.sc_t.test.WSB$p.value`, `r Q1_tot.sc_t.test.G$p.value`, and `r Q1_tot.sc_t.test.LEW$p.value`
