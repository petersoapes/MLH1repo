...
The mouse averages for IFDs were significantly longer in the high rec groups ( p = `r t.test_mouse.av.IFD_PER$p.value` for IFD^PER^ and p = `r t.test_mouse.av.IFD_ABS$p.value` for IFD^raw^). The pattern is confirmed with t.test comparing groups within the same subepcies.
...
<IN MOL as chrm length got longer - IFD gets smaller -- also in SKIVE x chrm>

For the strain only model (M2),  
 ...
 **IFD metreics are a significant predictors of rapid genome wide recombination rate evolution.** 
