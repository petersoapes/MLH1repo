...
load(file="~./MLH1repo/data/CleanBivData_3.2.20.RData")
...
```{r mixed.model.data.setup, echo=FALSE, warning=FALSE, eval=FALSE}
# MOVE this into BIVDATA SETUP!!!
...
<!--Can I indicate the average number of cells with go into each mouse.? -->
...
1. $$mouse \ av \ meteric ~=~ subsp * sex  + rand(1|strain) + \varepsilon$$
...
2. $$mouse \ av \ meteric ~=~ subsp * sex * strain + \varepsilon$$


Post hoc model for investigating strain as a fixed effect.


3. $$mouse \ av \ meteric ~=~ sex * strain + \varepsilon$$
...
```{r MLH1co.av.M1, echo=FALSE, digits = 3}
...
#summary(MLH1.MM.M1)
drop1(MLH1.MM.M1, test="Chisq") # p = 0.0002045 ***

as.data.frame(round(summary(MLH1.MM.M1)$coefficients, 6))

#only the interaction term shows up, when the 2 terms are additive, they both show up in the drop1 results
anova(MLH1.MM.M1)

exactRLRT(MLH1.MM.M1) #< 2.2e-16  Random strain effect, the variance due to random strain effect is NOT 0
...
<!--
 Contrasts and ANOVA, mean CO count full vs reduced
...
A general pattern is that the female data has higher within mouse variance (both var and cV). Some strains more than others (investigate potential outliers). (LEW females with top 3 variance are all from different dissection dates)
This pattern also holds for within mouse cv for MLH1 count (not shown).

**todo**

Test if dissection date is more significant predictor than strain (for females) (Effect doc)
Also test how the number of 0CO bivalents per cell affects mouse with variance. (lm)
...
3.	lm(mouse.metric ~ subsp \* sex \* strain, data=DF.HetC.MixedModel.HQ)  

4.	lm(mouse.metric ~ sex * strain, data=DF.HetC.MixedModel.HQ)  
...
```{r M1.MLH1.var_cv, echo=FALSE, include=FALSE}
#modeling within mouse variance
#use  DF.HetC.MixedModel.HQ

#HQ var.CO
MLH1var_M1 <- lmer( var ~ subsp * sex +(1|strain), data=DF.HetC.MixedModel.HQ )
#summary(MLH1var_M1)

drop1(MLH1var_M1, test="Chisq") #interaction term NS
#when switched to additive fixed, sex is very sig

anova(MLH1var_M1)#sex explains most variance...

exactRLRT(MLH1var_M1) #strain
as.data.frame( round(summary(MLH1var_M1)$coefficients, 6))

#CV
MLH1cv_M1 <- lmer( cV ~ subsp * sex +(1|strain), data=DF.HetC.MixedModel.HQ )
#summary(MLH1cv_M1)

drop1(MLH1cv_M1, test="Chisq") #interaction term NS
#when switched to additive fixed, sex is very sig
anova(MLH1cv_M1)  #sex F value is large
exactRLRT(MLH1cv_M1) #strain

as.data.frame( round(summary(MLH1cv_M1)$coefficients, 6))
...
drop1(MLH1var_M1_Q12, test="Chisq")#sig
anova(MLH1var_M1_Q12)
exactRLRT(MLH1var_M1_Q12)#NS

as.data.frame( round(summary(MLH1var_M1_Q12)$coefficients, 6))

#Q12 cv
MLH1cv_M1_Q12 <- lmer( cV ~ subsp * sex +(1|strain), data=Q12_mouse_table_w.Ages )

drop1(MLH1cv_M1_Q12, test="Chisq")#NS
anova(MLH1cv_M1_Q12)#sex large F
exactRLRT(MLH1cv_M1_Q12)#NS

as.data.frame( round(summary(MLH1cv_M1_Q12)$coefficients, 6))

```

The fixed sex effect has the largest effect sizes. All are negative (for male).  (I'm not sure if I should switch to testing the additive fixed effects by themselves) - I'm pretty sure the outcome would be that the sex effect gets very large in those models.
...
For LEW - females must have high variance,  for CV Lew strain alone has increased cv. Consider plotting the variance - and diving deeper into this pattern
Most of the strain specific differences in within mouse variance go away in the Q12 dataset.

### MLH1 variance M3

$$mouse \ CO \ metric  ~=~ subsp * sex * strain + \varepsilon$$

```{r M3.MLH1.var, echo=FALSE, include=FALSE}

MLH1var_M4 <- lm( var ~ sex * strain,  data=DF.HetC.MixedModel.HQ )

#summary(MLH1var_M4)

drop1(MLH1var_M4, test="Chisq")
anova(MLH1var_M4)
as.data.frame( round(summary(MLH1var_M4)$coefficients, 6))


MLH1cv_M4 <- lm(cV ~ sex * strain, data=DF.HetC.MixedModel.HQ)

#summary(MLH1cv_M4)

anova(MLH1cv_M4)  
as.data.frame( round(summary(MLH1cv_M4)$coefficients, 6))


### Q12 var
MLH1var_M4_Q12 <- lm( var ~ sex*strain, data=Q12_mouse_table_w.Ages )

#summary(MLH1var_M4_Q12)

anova(MLH1var_M4_Q12)
as.data.frame( round(summary(MLH1var_M4_Q12)$coefficients, 6))


#Q12 cv
MLH1cv_M4_Q12 <- lm( cV ~ sex*strain, data=Q12_mouse_table_w.Ages )

#summary(MLH1cv_M4_Q12)

anova(MLH1cv_M4_Q12)

as.data.frame( round(summary(MLH1cv_M4_Q12)$coefficients, 6))

```


#### Summary witin mouse Variance
...
strain.table.HQ_Fig <- ddply(MLH1_data, c("strain", "sex"), summarise,
                        Nmice =length(unique(mouse)),
...
# Female Specific Polymorphism

I should run the same questions for the female data -- 
...
Mol.female.plot <- ggplot(Mol.female_mouse.av, aes(color= strain))+geom_jitter(aes(x = as.factor(strain), y = var, color= strain, alpha=0.5), width = .2 )+ ggtitle("Mol female of within mouse MLH1 distributions")+scale_color_manual(values=colors_of_strains)
Mol.female.plot

Dom.female.plot <- ggplot(Dom.female_mouse.av, aes(color= strain))+geom_jitter(aes(x = as.factor(strain), y = var, color= strain, alpha=0.5), width = .2 )+ ggtitle("Dom feale ofwithin mouse  MLH1 distributions")+scale_color_manual(values=colors_of_strains)
Dom.female.plot
...
```{r Female.specific.test, echo=FALSE}

lm.female.strain.subsp <- lm(mean_co ~ subsp * strain, data= female_mouse.avs_for.analysis)
summry.female.strain.effects <- summary(lm.female.strain.subsp)
#strainG, 1.06e-05 ***
#strainLEW  0.0273 *

lm.female.strain <- lm(mean_co ~ strain, data= female_mouse.avs_for.analysis)
...
#how much greater is G female
G_female <-  female_mouse.avs_for.analysis %>% filter(strain == "G")
mean.G.female <- mean(G_female$mean_co)

sans_G_female <- female_mouse.avs_for.analysis %>% filter(strain != "G")
mean.all.other <- mean(sans_G_female$mean_co)


## within mouse variance models 
lmVAR.female.strain.subsp <- lm(var ~ subsp * strain, data= female_mouse.avs_for.analysis)
summry.female.strain.VAReffects <- summary(lmVAR.female.strain.subsp)

lmVAR.female.strain <- lm(var ~ strain, data= female_mouse.avs_for.analysis)
lmVAR.female.strain_sum <- summary(lmVAR.female.strain)

#Q12
Q12_female_table <-  Q12_mouse_table_w.Ages %>% filter(sex == "female")

lm.m1.Q12_female <- lm(var ~ subsp * strain, data= Q12_female_table)
lm.m1.Q12_female_sum <- summary(lm.m1.Q12_female)
#LEW not sig

lm.m2.Q12_female <- lm(var ~ strain, data= Q12_female_table)
...
(Try removing the alpha legend)  

This is the table and plot of new Musc strains

`r kable(table( droplevels( Musc.male_mouse.av$strain)  ))`
...
```{r Male.specific.test, echo=FALSE}
...
lm.male.strain.subsp <- lm(mean_co ~ subsp * strain, data= just.male_mouse.av)
summry.male.strain.effects <- summary(lm.male.strain.subsp)

lm.male.strain <- lm(mean_co ~ strain, data= just.male_mouse.av)
...
TODO -- try running a chi square test on the proportions.
...
```{r barlett.test, echo=FALSE, eval=FALSE}
...
As alex said, be careful about mixed models and interperting the random effect (radnom variance) -- assigning strain as random effect is slightly problamatic as i can give it a biological interpertation (it's due to strain -- genetics of each of the strains).  
...
```{r biv.data_table.4MM.HetC, echo=FALSE, eval=FALSE}
...
```{r biv.data_table.4MM.Male, echo=FALSE, eval=FALSE}
...
```{r MM_2.SC.Length, echo=FALSE, eval=FALSE}

#get a list of the old mice to double check
#old.mice <- DF.HetC.MixedModel.HQ$mouse[DF.HetC.MixedModel.HQ$age.weeks < 25,]
#MIXED MODEL
lme_MM_SC.length <- lme(mean_SC ~ subsp * sex, data= Mouse.Table_BivData_4MM, random=list(strain=pdDiag(~sex) ) )
...
lmer.SCLength_all_MM.M1 <- lmer(mean_SC ~ subsp * sex + (1|strain), data=Mouse.Table_BivData_4MM, REML = TRUE) 

#these 2 below are LR tests
drop1(lmer.SCLength_all_MM.M1, test="Chisq" ) #other options for test don't work
anova(lmer.SCLength_all_MM.M1) # also F values


exactRLRT(lmer.SCLength_all_MM.M1)
as.data.frame(round(summary(lmer.SCLength_all_MM.M1)$coefficients, 6))#these Dont return p values for the fixed effects


#glmer.SCLength_all_MM.M1 <- glmer(mean_SC ~ subsp * sex + (1|strain), data=Mouse.Table_BivData_4MM) 
#can't get p values from this summary

#1CO
SCLength_1CO_MM.M1 <- lmer(mean.SC_1CO ~ subsp * sex + (1|strain), data=Mouse.Table_BivData_4MM)
drop1(SCLength_1CO_MM.M1, test="Chisq")
exactRLRT(SCLength_1CO_MM.M1)
as.data.frame(round(summary(SCLength_1CO_MM.M1)$coefficients, 6))


#2CO
SCLength_2CO_MM.M1 <- lmer(mean.SC_2CO ~ subsp * sex + (1|strain), data=Mouse.Table_BivData_4MM)
drop1(SCLength_2CO_MM.M1, test="Chisq")
exactRLRT(SCLength_2CO_MM.M1)
as.data.frame(round(summary(SCLength_2CO_MM.M1)$coefficients, 6))


#Long.Biv
SCLength_long.biv_MM.M1 <- lmer(mean_SC ~ subsp * sex + (1|strain), data=Long.biv_mouse_table_HetC)

drop1(SCLength_long.biv_MM.M1, test="Chisq")
exactRLRT(SCLength_long.biv_MM.M1)
...
```{r M2_many.metrics, echo=FALSE, eval=FALSE}
...
$$mouse \ average \ SC  \ length ~=~ strain + \varepsilon $$
M2 anova for glm, of each subset, does at least one of the strains differ from others?


M3 logistic regression, can a bivalent metric predict the high or low group?

$$ log( strain ~=~ \ mouse \ average \ SC  \ length ) $$
...
```{r male.show.plotsEXXXample, echo=FALSE, eval=FALSE}

#REWRITE THIS FOR SC LENGTH COMPARISONS
...
```{r HetC_M1_IFD_many.metrics, echo=FALSE}
#get a list of the old mice to double check
#old.mice <- DF.HetC.MixedModel.HQ$mouse[DF.HetC.MixedModel.HQ$age.weeks < 25,]
#MIXED MODEL
...
#anova(lmer.IFD.PER_MM.M1)

#exactRLRT(lmer.IFD.PER_MM.M1)
#as.data.frame(round(summary(lmer.IFD.PER_MM.M1)$coefficients, 6))


#M1_IFD.RAW_2CO
lmer.IFD.RAW_MM.M1 <- lmer(mean_IFD.2CO_ABS ~ subsp * sex + (1|strain), data=Mouse.Table_BivData_4MM, REML = TRUE) 

#these 2 below are LR tests
#drop1(lmer.IFD.RAW_MM.M1, test="Chisq" ) #other options for test don't work
#anova(lmer.IFD.RAW_MM.M1) # also F values

#exactRLRT(lmer.IFD.RAW_MM.M1)#random strain not sig
#as.data.frame(round(summary(lmer.IFD.RAW_MM.M1)$coefficients, 6))#these Dont return p values for the fixed effects
```
...
#uggh how does this not work ... 

#more males have lower than 30% IFD?
...
```{r MM_CO_pos, echo=FALSE, eval=FALSE}
#use the 1CO
#Use MELT DF
#MELT_Curated_BivData
...
```{r log.reg.CO1_pos, echo=FALSE, eval=FALSE}
...
```{r t.test_1CO_pos, echo=FALSE, eval=FALSE}
...
**What is the overlap for cell images in the MLH1 comparisons and these bivalent comparisons?**

should all mice be represented.

```{r male.Nrm1CO_strain, echo=FALSE, eval=FALSE}
#isolate all musc
...
remeber the 3 predictions for subsp;

i) Dom, no difference  
 
ii) Musc, PWD and SKIVE greater than others
...
```{r Q2.SC.long.biv, echo=FALSE}

#mouse.av.long.biv.table

glm1.SC.male_long.biv <- glm(mean_SC ~ subsp * strain, data= mouse.av.long.biv.table)
summary(glm1.SC.male_long.biv) 

glm2.SC.male_long.biv <- glm(mean_SC ~ strain, data= mouse.av.long.biv.table)
summary(glm2.SC.male_long.biv) 

#Dom
glm_SC.male_Dom_long.biv <- glm(mean_SC ~ strain, data= mouse.av.long.biv.table[mouse.av.long.biv.table$subsp == "Dom",])
summary(glm_SC.male_Dom_long.biv)  #ALL strain are sig
#both G and LEW have significant mean.SC, doesn't meet predictions

#Musc
glm_SC.male_Musc_long.biv <- glm(mean_SC ~ strain, data= mouse.av.long.biv.table[mouse.av.long.biv.table$subsp == "Musc",])
summary(glm_SC.male_Musc_long.biv)

#Mol
glm_SC.male_Mol_long.biv <- glm(mean_SC ~ strain, data= mouse.av.long.biv.table[mouse.av.long.biv.table$subsp == "Mol",])
summary(glm_SC.male_Mol_long.biv)


## logistic regression for Rec group

log.SC.male_long.SC <- glm(Rec.group ~ mean_SC, 
              data=mouse.av.long.biv.table,  family=binomial(link="logit"))
...
The glm for SC lengths -- sub-grouped by subsp -- indicates that all strain effects are significant for the average mouseisn't met.  SC length.
My prediction for Dom, isn't met.

(indecation that there is more variation for the SC lengths than for gwRR / CO counts)
...
The logistic regression showed rec groups could be predicted by SC length - re-run them while seperating out by chrm class
...
- Is there a consistant pattern with the high rec strains?
...
Q2.show.nrmF1 <- ggplot(male.bivdata.1CO, aes(x=strain, y=PER_Foci_1, color=strain) )+geom_jitter(aes(alpha=.5) )+ geom_boxplot(aes(y= PER_Foci_1, alpha=.5) ) + scale_color_manual(values=colors_of_strains)+facet_wrap(~subsp, scales = "free")+ylim(c(0,1.1))+ggtitle("Normalized 1CO Foci pos")

Q2.show.nrmF1


nrmF1_mouse_level <- ggplot(Male.poly.Mouse.Table_BivData_4MM, aes(x=strain, y=norm.first.foci.pos, color=strain) )+geom_jitter(aes(alpha=.5) )+ geom_boxplot(aes(y= norm.first.foci.pos, alpha=.5) ) + scale_color_manual(values=colors_of_strains)+facet_wrap(~subsp, scales = "free")+ylim(c(0,1.1))+ggtitle("Mouse Level, \nNormalized 1CO Foci pos")
...
Q2.show.nrmF1_long.biv <- ggplot(male.long.biv.1CO, aes(x=strain, y=PER_Foci_1, color=strain) )+geom_jitter(aes(alpha=.5) )+ geom_boxplot(aes(y= PER_Foci_1, alpha=.5) ) + scale_color_manual(values=colors_of_strains)+facet_wrap(~subsp, scales = "free")+ylim(c(0,1.1))+ggtitle("Normalized 1CO Foci pos \nLong Biv")

Q2.show.nrmF1_long.biv

#2 version of the boxplot data -- not sure why th pattern differd

#more bivData
#nrmF1_with.SC.length <- ggplot(Curated_BivData_male, aes(x=strain, y=PER_Foci_1, color=strain) )+geom_jitter(aes(alpha=.5) )+ geom_boxplot(aes(y= PER_Foci_1, alpha=.5) ) + scale_color_manual(values=colors_of_strains)+facet_wrap(~subsp, scales = "free")+ylim(c(0,1.1))+ggtitle("Normalized 1CO Foci pos")



```

I think the plot shows that there are fewer 1CO for the High Rec group.

Maybe showing showin
...
#logistic regression for high and low group
...
```{r male.IFD_strain, echo=FALSE, eval=FALSE}
#integrate this into the setup scripts
...
```{r IFD.log.reg, echo=FALSE, eval=FALSE}
...
#contrast models -- what's the points for this block
