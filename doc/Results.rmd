...
make table of the strain averages 

```{r MLH1.summ.stats, echo=FALSE}

#table for the means by strain
...
F.strain.table <- ddply(female_mouse.avs_for.analysis, c("strain"), summarise,
                        Nmice =length(unique(mouse)),
                        Ncells  = sum(Ncells),
                        mean_co =as.numeric(format(round(mean(mean_co), 3 ), nsmall=3) ),
...
glm.female.strain.subsp <- lm(mean_co ~ subsp * strain, data= female_mouse.avs_for.analysis)
...
sans_high_female <- female_mouse.avs_for.analysis %>% filter(strain != "G") %>%  filter(strain != "MSM")
...
mean.MSN.female <- mean(MSM_female$mean_co)

sans_MSM_female <- female_mouse.avs_for.analysis %>% filter(strain != "MSM")
mean.all.other <- mean(sans_MSM_female$mean_co)

mean.MSN.female / mean.all.other
...
The range of strain means is `r min()`

the ranges of mouse means for each strain is X

MSM, G, LEW have significant strain effects.
...
MM.Fixed.pval <- drop1(MLH1.MM.M1, test="Chisq")$`Pr(Chi)`[2]
MM.rand.pval <- exactRLRT(MLH1.MM.M1)$p.value 
...
The significance of the interaction effect for the 2 fixed effects is `r MM.Fixed.pval`. The random effect for strain effect is `r MM.rand.pval`. 
...
The strain effects are only slightly significant for within mouse variance measures in the full data set (includes low quality cells)
...
Most of the strain specific differences in within mouse variance go away in the Q12 data set.
...
# Proportion of Chromosome Classes

In order to decompose the cell wide rate, we decided to look at the proportion or chromosomes with different numbers of COs)
...
x - list of the number of 'successes' across groups  (1COs)
y - list of the number of 'failures' across groups  (2COs)
...
Mouse.Table_BivData_4MM <- Mouse.Table_BivData_4MM[(!is.na(Mouse.Table_BivData_4MM$mouse)),]


female.mouse.av <-  Mouse.Table_BivData_4MM %>%  filter(sex == "female")
male.mouse.av <-  Mouse.Table_BivData_4MM %>%  filter(sex == "male")
  
n1CO_list_F <- c( female.mouse.av$n1CO)
n1CO_list_M <- c( male.mouse.av$n1CO)

total_biv_F_list <- c( female.mouse.av$tot.biv)
total_biv_M_list <- c( male.mouse.av$tot.biv)

nbiv_total_list <- c(Mouse.Table_BivData_4MM$tot.biv)

prop.test(n1CO_list_F, total_biv_F_list )

#perform test across strain 

Chrm.prop_strain_table <- ddply(Mouse.Table_BivData_4MM, c("strain","sex"), summarise,
...
female.0CO.prop.test <- prop.test(chrm.prop.strain.female$n0CO, chrm.prop.strain.female$nbiv )
female.1CO.prop.test <- prop.test(chrm.prop.strain.female$n1CO, chrm.prop.strain.female$nbiv )
female.2CO.prop.test <- prop.test(chrm.prop.strain.female$n2CO, chrm.prop.strain.female$nbiv )


chrm.prop.strain.male <-  Chrm.prop_strain_table %>%  filter(sex == "male")
#chrm.prop.strain.male.high.rec <-  Chrm.prop_strain_table %>%  filter(sex == "male")
#chrm.prop.strain.male.low.rec <-  Chrm.prop_strain_table %>%  filter(sex == "male")

male.0CO.prop.test <- prop.test(chrm.prop.strain.male$n0CO, chrm.prop.strain.male$nbiv )
male.1CO.prop.test <- prop.test(chrm.prop.strain.male$n1CO, chrm.prop.strain.male$nbiv )
male.2CO.prop.test <- prop.test(chrm.prop.strain.male$n2CO, chrm.prop.strain.male$nbiv )
...
The figure below shows the average percentages of chromosome classes per category gathered from BivData
...
These questions use different datasets, q1 sex matched strains  

Q2 all male observations with including strains not in the q1 data set
...
<!--
As alex said, be careful about mixed models and interpreting the random effect (random variance) -- assigning strain as random effect is slightly problematic as i can give it a biological interpretation (it's due to strain -- genetics of each of the strains).  
-->
...
Q2 Model 1, mixed model lmer()  

$$mouse \ av.\  metric ~=~ subsp *  + rand(strain) + \varepsilon $$


Q2 Model 2,  glm()

$$mouse \ av.\  metric ~=~ strain + \varepsilon $$
...
- Simple application of the same models as MLH1 based results.
...
> M2 subsp \* sex \* strain  (don't rally use)
...
lmer.Q1_SCLength_M1 <- lmer(mean_SC ~ subsp * sex + (1|strain), data=Mouse.Table_BivData_4MM, REML = TRUE) 
...
#drop1(Q1.lmer_SCLength_1CO_M1, test="Chisq")
exactRLRT(Q1.lmer_SCLength_1CO_M1)
...
exactRLRT(Q1.lmer_SCLength_2CO_M1)
as.data.frame(round(summary(Q1.lmer_SCLength_2CO_M1)$coefficients, 6))

#Long.Biv
Q1.lmer_SCLength_long.biv_M1 <- lmer(mean_SC ~ subsp * sex + (1|strain), data=Long.biv_mouse_table_HetC)

#drop1(Q1.lmer_SCLength_long.biv_M1, test="Chisq")
#exactRLRT(Q1.lmer_SCLength_long.biv_M1)
...
#Q1 plots for SC Lengths across Musc strains
...
Q1.glm_nrmF1pos_M2 <- glm(norm.first.foci.pos ~ strain * sex, data=Mouse.Table_BivData_4MM)
...
```{r Q1.Nrm.F1pos_M2_long.biv, echo=FALSE}

#table for the 1CO long biv across strain and sexes?
...
```{r Q1.MM_CO_pos, echo=FALSE, eval=FALSE}
#use the 1CO
#Use MELT DF
#MELT_Curated_BivData
...
```{r Q2.male.Nrm1CO_strain, echo=FALSE}
#isolate all musc
