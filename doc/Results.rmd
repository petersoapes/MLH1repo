...
In order to estimate the genome wide recombination rates across our panel of `r length(unique(  (main.table  %>% filter(species == "M.musculus") )$strain) )` _M. m. musculus_ strains, MLH1 foci were quantified from a total of `r sum( (main.table  %>% filter(sex == "male"))$Ncells)` spermatocytes and `r sum( (main.table  %>% filter(sex == "female"))$Ncells)` oocytes.

(mouse means were calculated and statisics in Table 2)
...
All of the effects from the mixed model significant, (LRT; p= `r  round(MLH1.MMlmer_results$sub.sp, 4)`, `r  round(MLH1.MMlmer_results$sex.results, 4)`, and  `r round(MLH1.MMlmer_results$interaction.pval, 4)` for the sex, subspecies and interaction fixed effects and `r  MLH1.MMlmer_results$rand.pval` for the random strain effect).
...
We followed up the mixed model effects with general linear models. The first glm contained subspecies, strain, sex, and all interactions, while the second glm, contains coefficients for strain, sex and the interactions. (*what are the points and differences? )*
...
Additionally, _domesticus^G^_ had significant strain and sex by strain effects (p =`r  summ.strain.fixed$coefficients[,4][3]` and `r summ.strain.fixed$coefficients[,4][10]` for strain and strain by sex effect). These results confirm the qualtative pattern of the _domesticus^G^_ strain having the largest degree of sexual dimorphism (heterochiasmy) in the dataset.
...
For both the mixed model and linear model, sex was the most significant effect while the subspecies, interaction and random strain effects were not significant (Table X) (LRT; p= `r LmerMLH1_VAR_results$sex.results`, glm; p= `r MLH1var_M3_Q12_sum$coefficients[,4][2]` mixed and glm models respectively.
...
To analyze the female specific data set, 
...
`r max( ( female_mouse.avs_for.analysis %>% filter(strain  == "G"))$mean_co  )` in _domesticus^G^_

`r min( ( female_mouse.avs_for.analysis %>% filter(strain  == "LEW"))$mean_co  )` to `r max( ( female_mouse.avs_for.analysis %>% filter(strain  == "LEW"))$mean_co  )` in _domesticus^LEW^_

In the musc subspecies the ranges of mouse means are `r min( ( female_mouse.avs_for.analysis %>% filter(strain  == "PWD"))$mean_co  )` to `r max( ( female_mouse.avs_for.analysis %>% filter(strain  == "PWD"))$mean_co  )` in  _ musculus^PWD^_,`r min( ( female_mouse.avs_for.analysis %>% filter(strain  == "KAZ"))$mean_co  )` to `r max( ( female_mouse.avs_for.analysis %>% filter(strain  == "KAZ"))$mean_co  )` in _musculus^KAZ^_

The range is `r min( ( female_mouse.avs_for.analysis %>% filter(strain  == "MSM"))$mean_co  )` to `r max( ( female_mouse.avs_for.analysis %>% filter(strain  == "MSM"))$mean_co  )` in  _molossinus^MSM^_
...
_domesticus^G^_ is `r mean.G.female / mean.all.other` higher the the other means. _ domesticu^LEW^_  is `r mean.LEW.female / mean.all.other` higher  and _musculus^MSM^_ is `r mean.MSM.female / mean.all.other`. These three will be designated as 'moderate high rec' strains for later sex specific analysis.
...
The male specific patterns were also investigated. (what's a better motivation for this section)
