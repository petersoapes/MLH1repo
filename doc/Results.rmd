...
**we should note that these interpertations are dependant on which count of DSBs is most reflective for the final number of matur crossovers.** A number of factors make this determintion less clear - including the potential for mulitple rounds of DSB formation (CO homeostasis) (cite) and difference in rates for repair (cite)

(also the data is very good)
...
P1.by.P2_plot_nrm.PWD.male <- ggplot(Curated_BivData.22 %>% filter(strain == "G") %>% filter(sex == "male"),
    aes(y = PER_Foci_2, x= PER_Foci_1, color=strain))+
  geom_jitter(alpha=.5)+
#  scale_x_continuous(position = "top") +
  scale_x_continuous(sec.axis = dup_axis(), position = "top")+
    ggtitle("Nrm IFD G, male")+scale_color_manual(values=colors_of_strains)+ ylim(c(0,1))+xlim(c(0,1))+ theme(  panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank() )
...
(both males and females IFD distributions have lower and upper thresholds around 25% and 75%) respectively. however the weight of observations are slightly shifted across the sexes. There's an enrichment of lower IFD^norm^ in females relative to males, while the opposite is true for the higher threshold -- where males have an enrichment of higher/long IFD^norm^ compared to females.

In the three high rec strains -- the distributions have a more striking difference of the lower threshold being much higher in males, aproximately 30% to 40% compared to 25% in females. Also while the female upper threshold is capped at 75%, in males (some strains have higher upper thresholds -- and it's more likely to observe IFD^norm^ above the threshold.
...
<!-- lmer/ glms  **M1. Mixed Model** $$mouse \ average \ IFD ~=~ subsp * sex + rand(strain) + \varepsilon $$
...
<!-- junk for Q1.IFD
  ...
  For the normalized values in both M2 and M3, sex is a significant effect, increasing 
with males nrm.IFD in males.
  SKIVE*male is the only other consistently significant effect, which also increases the nrm.IFD measure (p = male-SKIVE p =  ).
For the mixed models of IFDs, sex is a significant effect for both raw and nrmIFD. (IFD.PER), sex is most significant effect (LRT: p = `r Q1.IFD.PER_M1_lmer.results$sex.results`)
...
<!-- basic stats
`r length(OG.MLH1.merge.Skel$fileName)` total skelentonize measures were made from all of the MLH1 pachtyene cell images. X observations removed for having extra signal from other cells. `r length(unique( all.remove ) )` measures removed as potential outliers (by examining the category specific plots)<pattern for the single bivalent, pos correlation, The predicted pattern for SC length becomes more nuanced when the data are split up by Chromosome class. Mainly that 1CO are shorter in the high recombining strains then low recombining strains. This is likely due to the fact that more physically longer chromosomes have 1CO in the low recombining strains, which pushes the mean SC length up. Where as in the high recombining strains, physically longer chromosomes are more likely to be in the 2CO group.  This supports a general pattern of tighter clustering of SC lengths across chromosome classes in the high recombininggroup. They have a lower probability of 2COs below a certain SC length threshold.>  -->
...
<!--
In the short bivalent data set, analyzed by M1, both subspecies are 
 (glm; p= _Musculus_ `r sum.glmQ2_short.biv_M1$coefficients[,4][2]` and _Molossinus_ p= `r sum.glmQ2_short.biv_M1$coefficients[,4][3]`). While the models for the reduced data set suggest that the subspecies with the high recombining strains have evolved longer SC lengths --
...
```{r Q2.IFD_short.removed_t.test, echo=FALSE}
...
<!--
RAW IFD M1 results for M1  -- `r sum.Q2.M1glm_IFD.raw$coefficients[,4][6]` PWD, 
MSM `r sum.Q2.M1glm_IFD.raw$coefficients[,4][7]`
SKIVE `r sum.Q2.M1glm_IFD.raw$coefficients[,4][8]`
and M2
for PWD, `r Sum.Q2.M2glm_IFD.raw$coefficients[,4][4]`
MSM `r Sum.Q2.M2glm_IFD.raw$coefficients[,4][5]`
and SKIVE   `r Sum.Q2.M2glm_IFD.raw$coefficients[,4][7]`

In M1, the model build on the mouse averages of IFD^norm^ metrics with subspecies and strain as fixed effects, the high recombining strains had significant strain effects for (glm; p = `r sum.Q2.M1glm_IFD.PER$coefficients[,4][6]`, p = `r sum.Q2.M1glm_IFD.PER$coefficients[,4][7]` and p = `r sum.Q2.M1glm_IFD.PER$coefficients[,4][8]` for _musculus^PWD^_, _molossinus^MSM^_ and _musculus^SKIVE^_, respectively). 
...
With each unit increase in SC length, the IFD^norm^ also increases (for the high rec strains + MOLF). All of the high rec strains and MOLF have significantly longer IFD^norm^. There are also significant interaction effect of strain and chromsome length, in the high rec strains and MOLF, with each increase in bivalent length, the IFD^norm^ decreases slightly.
-->
