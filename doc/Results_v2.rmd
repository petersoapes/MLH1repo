...
- make table, save in figure folder, print head here
...
DMC1table.main99 <-  DMC1table.main[, c("category","mean.MLH1","ncells.x","mean.count.L", "MLH1.L_ratio", "ncells.y","mean.count.Z", "MLH1.Z_ratio"),  drop=FALSE]

colnames(DMC1table.main99) <- c("category", "mean MLH1",    "ncells",     "mean DMC1", "MLH1:DMC1 ratio", "ncells",     "mean DMC1", "MLH1:DMC1 ratio")

kable(DMC1table.main99, row.names = F) %>%
  kable_styling() %>%
...
```{r with.cow, echo=FALSE, warning=FALSE, message=FALSE}
library(cowplot)
...
#plot_grid(top.triangle, bottom.triangle, ncol=1)

#plot_grid(top.triangle, bottom.triangle, align = "v", rel_widths = c(1.3, 1.3))

```


```{r another.triangle, echo=FALSE, fig.width = 5, fig.height=5}
...
```{r Q1.IFD.triangle, echo=FALSE}

#make triangle for the IFD measures (try doing female top male bottom)
#Curated_BivData[Curated_BivData$hand.foci.count == 2,]
...
#use cowplot or another way to paste these together

```


```{r Q1.show.IFD_female.male.comparison, echo=FALSE}

#compare female IFD patterns which have telomeric bias (more similar to males)
