...
tested coefficients from the mixed models (using the lmer() function) with chisquare test between full and reduced models for mixed. the random effects were tested with the exactRLRT()


M2 to M4 are glm -- use lm()
...
**certeria**  MLH1 foci were quantified from cells with intact and compelete karyotypes (19 acrocentric bivalents and XY for spermatocytes or 20 acrocentric bivalents for oocytes) and distinct MLH1 foci. A quality score, between 1 to 5, was asigned to each cell based on staining quality and general spread of bivalents. <cell with small defects were included -- and coded as a score of 5 - but these cell weren't included in the final analysis.>  The distributions of MLH1 counts per cell was assessed for normality (supplemental figure). 

**the XY foci is not counted / analyzed in this study**

-Background for setting up/organizing models -- (heiarchal nature of multiple strains within subspecies -- ) - hierarchical nature of categories for inviual mouse averages ()

-(primary focus is mean MLH1 count per cell - at the mouse observation level - apply to more meiocyte traits)

**partioning variation in recombination rate**

-Our goal -- design an utilize a mixed model for our analytical framework - which leverages / accounts for the hierarchical/nested nature of categories across all the individual mice (e.g. sex, strain subspecies).
...
The main effects and their interaction of subspecies and sex were coded as fixed. 

strain (or genetic background) was coded as a **random** effect to reflect a random sample of natural genetic variation from each subspecies.

In addition to mouse mean of MLH1 count per cell
**to expand the analysis of meiotic features** We applied the following models to several dependant variables (mouse averages) <consider a figure/diagram>

 the mouse average of variance in crossover count per cell, 
 
 mouse mean total SC per cell, mouse mean single SC length, mouse mean IFD^norm^ and IFD^raw^, and position of crossover for single crossover bivalents. 
...
**M4.2** removed the subspecies coefficient for 
...
**combine any X and XY issue with reduced dataset** An additional challenge of the MLH1 framework is that the identities of individual autosomes and the XX in females cannot be easily obtained (the male XY is distinct).
...
**walk through how outliers are removed**
