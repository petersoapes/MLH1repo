...
We used a series of statistical models to interpret patterns of variation in the recombination traits we measured (Table 2). We first constructed a mixed model (M1) using the lmer() from the lmer4 package (cite) in R (v3.5.2) [@Rstudio]. In this model, strain was coded as a random effect, with significance evaluated using a likelihood ratio test (using exactRLRT()). Subspecies, strain, and their interaction were coded as fixed effects,  with significance evaluated using a chi-square test comparing the full and reduced models (drop1() and anova()). *discribe the hiearchal nature*. As observations were at the level of single mouse (mouse average) was uniquely coded within the dataset, nesting was implict, each strain only occurs within one strain, and was not explictly coded in our linear and mixed models.
...
In the dataset of short bivalents (to which the female X bivalent does not contribute), female to male ratios of mouse mean SC length range from 1.15 -- `r Q1SC.mus.av.short.biv_DIFF_MSM` ( _musculus^MSM^_) to 1.49 -- `r Q1SC.mus.av.short.biv_DIFF_WSB` ( _domesticus^WSB^_) across strains. 

That females have longer SCs is further supported by models that include covariates, which identify sex as the most consistently significant effect for total SC  (M1: p=`r Q1.lmer_totSC_M1_results$sex.results`; M2: p=`r sum.Q1.glm_totSC_M2$coefficients[,c(1,4)][8,2]`; M3: p =`r sum.Q1.glm_totSC_M3$coefficients[,c(1,4)][8,2]`) and short bivalent measures (M1: p= `r Q1.lmer_short_M1_results$sex.results`; M2: p= `r summ.Q1.glm_short.SC_M2$coefficients[,c(1,4)][8,]`; M3: `r summ.Q1.glm_short.SC_M3$coefficients[,c(1,4)][8,2]`).
...
double triangle plots made with different scripts
...
**why chrm proportion table for the sum numbers? - because I am summing all of the chrm classes** The number is much smaller (redo the proportions table?) - part of the difference is, these are from the new are from the Clean_Curated_BivData. (i'm not sure if the big number if right)
...
**these pvalues aren't the same anymore**
...
```{r Q2.IFD.glm.test, echo=FALSE, warning=FALSE, eval=FALSE}
...
correlations are from the fig strain table estimates
...
The spindle assembly checkpoint (SAC) prevents aneuploidy by ensuring that all bivalents are correctly attached to the microtubule spindle (“bi-oriented”) before starting the metaphase-to-anaphase transition via the release of the sister cohesin holding homologs together [@LaneKauppi2019]. Hence, selection seems likely to favor mutations that optimize the process of bi-orientation and smooth chromosome seperation , thereby prohibiting the SAC from delaying the cell cycle or triggering apoptosis.  
