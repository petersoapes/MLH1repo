...
#load(file = "C:/Users/April/Documents/MLH1repo/data/MLH1/MLH1_data_setup_1.31.20.RData")
...
Details of data set cleaning etc is in SetupData.rmd
...
ToDo: change the Mixed models to lmer() so that I can confidantly test random effect (variance)
...
I won't run the model above -- even tho it is logical and makes sense -- I can't make good estimates for random effects.



$$mouse .var CO ~=~ subsp * sex + rand(strain) + \varepsilon $$
...
*mouse averages exclude Q5 and male counts are adjusted +1 for PAR)*

## Mean CO Count

todo, add the random effect tests, consider changing all to lmer() (those have better random effects)

For now I will focus on 4 different models run mouse averages on MLH1 counts.

1. $$mouse .mean CO ~=~ subsp * sex  + rand(strain*sex) + \varepsilon$$

USE the bottom one

2. $$mouse .mean CO ~=~ subsp * sex  + rand(1|strain) + \varepsilon$$


3. $$mouse .mean CO ~=~ subsp * sex * strain + \varepsilon$$


4. $$mouse .mean CO ~=~ sex * strain + \varepsilon$$
...
```{r M1_HQ.MixedModel.1, echo=FALSE, digits = 2, eval=FALSE}
#table(DF.HetC.MixedModel$strain, DF.HetC.MixedModel$sex)
#mean_co.HQ  (this might remove some of the MOLF males)
...
> Model 2. THIS SHOULD BE THE MAIN MIXED MODEL --lmer

$$mouse .mean CO ~=~ subsp * sex  + rand(1|strain) + \varepsilon$$

```{r MixedModel.1, echo=FALSE, digits = 3}
...
All fixed effects with all interactions.
...
test a model for fixed effects of strains within musc. The purpose is to distinguihsi musc strains from low 'Dom' level strains. 



```{r Musc.male.Model.data.setup, echo=FALSE}
#nested mouse in strain

#remove the extra merged cols
Musc.male_mouse.av <- subset(AP_mouse_table_w.Ages, select= -c(sex.y,sex.x,
                            subsp.y,subsp.x,
                    category.y,category.x, strain.x,
                    strain.y, mouse.1  )  )


Musc.male_mouse.av <- add_strain(Musc.male_mouse.av)
Musc.male_mouse.av <- add_sex(Musc.male_mouse.av)
Musc.male_mouse.av <- add_subsp(Musc.male_mouse.av)
Musc.male_mouse.av <- add_species(Musc.male_mouse.av)

Musc.male_mouse.av$subsp <- as.character(Musc.male_mouse.av$subsp)

Musc.male_mouse.av$subsp[Musc.male_mouse.av$strain== "MSM"] <- 'Mol'
Musc.male_mouse.av$subsp[Musc.male_mouse.av$strain== "MOLF"] <- "Mol"

#use original for getting the other strains

Musc.male_mouse.av <- Musc.male_mouse.av[Musc.male_mouse.av$species == "M.musculus",]
Musc.male_mouse.av <- Musc.male_mouse.av[Musc.male_mouse.av$subsp == "Musc",]
Musc.male_mouse.av <- Musc.male_mouse.av[Musc.male_mouse.av$sex == "male",]

#set the factors
Musc.male_mouse.av$strain <- factor(Musc.male_mouse.av$strain, ordered = FALSE)
Musc.male_mouse.av$mouse <- factor(Musc.male_mouse.av$mouse, ordered = FALSE)

#compare the the higher quality plots
#Q1Q2
...
Q12_mouse_table_w.Ages$subsp <- as.character(Q12_mouse_table_w.Ages$subsp)

Q12_mouse_table_w.Ages$subsp[Q12_mouse_table_w.Ages$strain== "MSM"] <- 'Mol'
Q12_mouse_table_w.Ages$subsp[Q12_mouse_table_w.Ages$strain== "MOLF"] <- "Mol"

Musc.male.Q12 <- Q12_mouse_table_w.Ages[(Q12_mouse_table_w.Ages$subsp == "Musc")&(Q12_mouse_table_w.Ages$sex == "male"),]

Musc.male.Q12$strain <- factor(Musc.male.Q12$strain, ordered = FALSE)
Musc.male.Q12$mouse <- factor(Musc.male.Q12$mouse, ordered = FALSE)


#Q123  Q123_mouse_table

Q123_mouse_table <- add_subsp(Q123_mouse_table)
Q123_mouse_table <- add_species(Q123_mouse_table)

Q123_mouse_table$subsp <- as.character(Q123_mouse_table$subsp)

Q123_mouse_table$subsp[Q123_mouse_table$strain== "MSM"] <- 'Mol'
Q123_mouse_table$subsp[Q123_mouse_table$strain== "MOLF"] <- "Mol"

Musc.male.Q123 <- Q123_mouse_table[(Q123_mouse_table$subsp == "Musc")&(Q123_mouse_table$sex == "male"),]

Musc.male.Q123$strain <- factor(Musc.male.Q123$strain, ordered = FALSE)
Musc.male.Q123$mouse <- factor(Musc.male.Q123$mouse, ordered = FALSE)

```

```{r Musc.male.show.plots, echo=FALSE}

musc.male.plot <- ggplot(Musc.male_mouse.av, aes(color= strain))+geom_jitter(aes(x = as.factor(strain), y = mean_co, color= strain, alpha=0.5), width = .2 )+ ggtitle("Musc male of MLH1 distributions")+scale_color_manual(values=colors_of_strains)
...
Q123.musc.male.plot <- ggplot(Musc.male.Q123,
                             aes(color= strain))+geom_jitter(aes(x = as.factor(strain), y = mean_co, color= strain, alpha=0.5), width = .2 )+ ggtitle("Q123 Musc male of MLH1 distributions")+scale_color_manual(values=colors_of_strains)


Q123.musc.male.plot
#THE q12,data set only 
#Q123 has 5 strain rep (no TOM), there are still some mouse outliers
...
```{r M1_Musc.male, echo=FALSE}

#mouse.av ~ mouse | strain

#not sure the right syntax for nested factors
#Musc.male.M1 <- lm(mean_co ~ (mouse|strain), data=Musc.male_mouse.av )

Musc.male.M1 <- lm(mean_co ~ strain, data=Musc.male_mouse.av )

as.data.frame(  round(summary(Musc.male.M1)$coefficients, 4) )

#all of the strain have a significant effect agaisnt the group mean / intercept
#looking at the intercept -- the PWD observations are pushing up the average

#musc male Q123  Q123_mouse_table

Musc.male.Q123.M1 <- lm(mean_co ~ strain, data=Musc.male.Q123 )

as.data.frame(  round(summary(Musc.male.Q123.M1)$coefficients, 4) )

#mostly all strains, (but SKIVE) are significant -- similar to the top model

#shoul I compare with subsampling
...
A general pattern is that the female data has higher within mouse variance (both var and cV). Some strains more than others (invetigate potential outliers). (LEW females with top 3 variance are all from different dissection dates)
...
as.data.frame( round(summary(MLH1var_M1)$tTable, 6))
...
as.data.frame( round(summary(M1.cv_co)$tTable, 6))
...
summary(MLH1var_M3)
anova(MLH1var_M3)  
as.data.frame( round(summary(MLH1var_M3)$tTable, 6))
...
as.data.frame( round(summary(MLH1var_M3_Q12)$tTable, 6))
...
anova(MLH1var_M4)
as.data.frame( round(summary(MLH1var_M4)$tTable, 6))
...
drop1(MLH1cv_M4, test="Chisq")
anova(MLH1cv_M4)  
exactRLRT(MLH1cv_M4)
as.data.frame( round(summary(MLH1cv_M4)$tTable, 6))

### Q12 var
MLH1var_M4_Q12 <- lm( var ~ subsp*sex*strain, data=Q12_mouse_table_w.Ages )
anova(MLH1var_M4_Q12)
as.data.frame( round(summary(MLH1var_M4_Q12)$coefficients, 6))

#Q12 cv
MLH1cv_M4_Q12 <- lm( cV ~ subsp*sex*strain, data=Q12_mouse_table_w.Ages )

summary(MLH1cv_M4_Q12)
anova(MLH1cv_M4_Q12)#sex large F
...
```{r DMC1.plot, echo=FALSE, warning=FALSE, eval=FALSE}
...
```{r DMC1_MLH1.cor, echo=FALSE, eval=FALSE}
