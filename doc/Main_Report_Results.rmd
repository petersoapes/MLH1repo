...
# Distributions of MLH1 counts across Samples

The histograms below are for total / pooled cells (for now, maybe compare with HQ data to see what changes)
...
$$mouse .mean CO ~=~ subsp * sex + rand(strain* sex)$$

$$mouse .var CO ~=~ subsp * sex + rand(strain* sex)$$
...
subspecies levels: 2, Musc and Domesticus  
...
These are the List of mixed models I built

1. mean_co (chr coded, instead of factors)  

2. mean_co.HQ  

3. mean_co with Mol (Mol coded as a distinct subsp)  

4. sub-sampling (not sure if I made this one)



```{r HQ.MixedModel.1, echo=FALSE}
...
```{r try.blah, echo=FALSE}
#library(jtools)
#states <- as.data.frame(state.x77)
#fit <- lm(Income ~ Frost + Illiteracy + Murder, data = states)
#summ(fit)
#this is a nice display package for lm

res.table <- as.data.frame(coef(summary(Reduced.all.HQ)))
res.table
```
...
Issue presented with is that we can see evolution across the strains, but it isn't predicted from mixed models by strain.
make artificial data / models, like human labels


## Mixed Model for variance
