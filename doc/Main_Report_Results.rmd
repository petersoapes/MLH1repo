...
todo: re-run the analysis with MOLF data
make model with strain as fixed effect (since, there are interesting strain effects)

The mixed model lets us separate effects to see how they effect a mouse mean CO count. This separation can also serve as proxies for divergence and polymorphism.
...
Reading these outputs is not the clearest -- try just printing out the coefficient table with p values.

Mostly matches the base model results. the same fixed effects are significant in the reduced models. the random effects make sense
