...
load in more Data, chrm proportions, total SC,
write / standardize the data set up for those
...
Details of dataset cleaning ect is in SetupData.rmd
...
figure 1, change to all white, add the subspecies lines below.
...
*cast female data came from another publication

add code for the table table. may have to deal with the excess mouse issue


# A. Models for mean and variance of MLH1 foci per cell 


These are the basic Mixed Models we used as a frame work to analyze all the MLH1 data.
...
```{r mixed.models.setup, echo=FALSE, warning=FALSE}
...
```{r HQ.dataset, echo=FALSE}

#also make a HQ dataset, tighter window for ages,
...
Seting up the data set for using the mixed model (base data set, HQ dataset)

- only Mus musculus strains with sex matched observations  

- Unorder factors  

- male ages within range

Currently I ran through 3 datasets: 1) base, 2) HQ and 3) Molf (from HQ)

`r kable(table(DF.HetC.MixedModel$sex, DF.HetC.MixedModel$strain))`

Above are the total number of mice in the data set  


FOR FINAL ANALYSIS, MOL Will be coded as a seperate subsp (change all of the main model reports to)
For mouse averages of MLH1 counts, only strain with observations for both sexes and are from Mus species. 

subspecies levels: 2(3), Musc and Domesticus (Molossinus)

strain levels: Dom: 3, Musc: 4  (Mol: 2)
...
Above is the table of coefficients for MM.1, HQ dataset, lme(), random effects strain and strain specific sex effect.  


$$mouse .mean CO ~=~ subsp * sex  + rand(strain*sex) + \varepsilon$$

```{r MM.1.Mol, echo=FALSE}

HetC.MixedModel.HQ.mol <- lme(mean_co ~ subsp * sex, data=DF.HetC.MixedModel.HQ.mol, random=list(strain=pdDiag(~sex) ) )

as.data.frame( round(   summary(HetC.MixedModel.HQ.mol)$tTable, 6) )

```


Above is the table of coefficients for MM.1.mol, molssinus coded as seperate subspecies, lme(), random effects strain and strain specific sex effect.  
...
Above is the table of coefficients for MM.2, HQ dataset, lme(), random effects coded without sex specific effect.
This is model has sex and interaction coefficient is significant.

> MM.2.Mol

 
```{r MixedModel.2.mol, echo=FALSE}
#reduced models
Reduced.strain.HQ.mol <- lme(mean_co ~ subsp * sex, data=DF.HetC.MixedModel.HQ.mol, random = ~1|strain)
#summary(Reduced.strain.HQ)

as.data.frame(round(summary(Reduced.strain.HQ.mol)$tTable, 6))
#add a aption to this code chunk
#ranef(Reduced.strain.HQ.mol)

```

The model for MM.2.Mol has similar results as the above model, with just 2 subsps
This model has significant coefficients for sex and the 2 interaction effects (sex:musc, sex:mol)

> MM2.2

$$mouse .mean CO ~=~ subsp * sex  + \varepsilon$$

```{r MixedModel.2.2, echo=FALSE}
Reduced.all.HQ <- gls(mean_co ~ subsp * sex, data=DF.HetC.MixedModel.HQ)

as.data.frame( round(summary(Reduced.all.HQ)$tTable, 6))
#summary(Reduced.all.HQ)
#ranef(Reduced.all.HQ)
```

Above is the table of coefficients for MM.2.2, HQ dataset, gls(), No random effects.  
I think this gls has sex and interaction term as significant.
...
as.data.frame(round( summary(strain.fixed.full)$coefficients, 6))
...
Above is the table of coefficients for MM.3, HQ dataset, lm function, sex, strain and subsp are interacting fixed effects.  
...
Above is the table of coefficients for **MM.4**, HQ dataset, lm function, just sex and strain, no subspecies.

Significant factors are 
strainG (p=0.0005), strainMSM and strainPWD (very slight),
sex:G (p=0.001),
...
The **biological significance** of within mouse variance in CO per cell is...

I can show the variance boxplots?  Not sure if it's very helpful.
Use the same Datasets HQ and mol for the mean CO counts. Use both var and cV as dependant variables

DF.HetC.MixedModel.HQ.mol
...
lme function, Cv modeled. Only sex is significant.


```{r cv.mod1.mol, echo=FALSE}

HetC.MixedModel.HQ_mol.cV_co <- lme(cV ~ subsp * sex, data=DF.HetC.MixedModel.HQ.mol, random=list(strain=pdDiag(~sex) ) )

as.data.frame( round(summary(HetC.MixedModel.HQ_mol.cV_co)$tTable, 6))
...
Sex and sex*subsp interaction effect are significant.



```{r cV.mod.mol, echo=FALSE}

HQ.mol.cV_co <- lme(cV ~ subsp * sex, data=DF.HetC.MixedModel.HQ.mol, random = ~1|strain) 

as.data.frame( round(summary(HQ.mol.cV_co)$tTable, 6))


```

Sex is significant, when mol is a seperate subspecies interaction is no longer significant.


$$mouse .cV CO ~=~ subsp * sex + \varepsilon$$
```{r more.cV.CO, echo=FALSE}
HQfull.Reduced.cv_co <- gls(cV ~ subsp * sex, data=DF.HetC.MixedModel.HQ)
as.data.frame( round(summary(HQfull.Reduced.cv_co)$tTable, 6))
...
#main var.CO
HetC.MixedModel.var_co <- lme(var ~ subsp * sex, data=DF.HetC.MixedModel.HQ, random=list(strain=pdDiag(~sex) ) )

as.data.frame( round(summary(HetC.MixedModel.var_co)$tTable, 6))

#summary(HetC.MixedModel.var_co)
#ranef(HetC.MixedModel.var_co)
```
Above error says missing values in object -- I don't understand. Mol version of data frame works...

Base DF, lme function, random sex specific strain effects



$$mouse .var CO ~=~ subsp * sex  + \varepsilon$$
```{r base.var.mol, echo=FALSE}
...
$$mouse .var CO ~=~ subsp * sex + rand(1|strain) + \varepsilon$$

```{r var.model.2, echo=FALSE, eval=FALSE}
just.strain.var_co <- lme(var ~ subsp * sex, data=DF.HetC.MixedModel, random = ~1|strain) 

as.data.frame( round(summary(just.strain.var_co)$tTable, 6))
...
```{r var.mod.3, echo=FALSE, eval=FALSE}

full.Reduced.cv_co <- gls(cV ~ subsp * sex, data=DF.HetC.MixedModel)
#summary(full.Reduced.cv_co)

as.data.frame( (summary(full.Reduced.cv_co)$tTable))

```

gls model model above
...
The general pattern across all the models, is that sex is a significant effect for all models. The sex-subsp interaction effect pvalues were larger and only significant for one of the models.

When Molossinus is coded as a seperate subspecies, then interaction terms are no longer significant.
...
The outgroups (SPIC and SPRET) don't show strong evidence of evolution in gwRR.

#### Male Patterns

Rapid evolution in musculus and molossinus; high and low CO count males (two instances of rapid gwRR evo, PWD and MSM)

#### Female Patterns

seems like female specific evolution in molossinus and musculus.
more variation in domesticus females.
Females have more nuanced patterns, there is strain specific differences.
...
chrm.class.barplot <- ggplot(data = melt.Chrm.Prop.table, aes(y = value, x = strain, fill=as.factor(type)) ) + geom_col(position = "dodge")+facet_grid(sex~strain, scales = "free")+ggtitle("Chrm Proportions from Hand Measured")+ylim(c(0,1))
...
# Sexual dimorphism and heterochiasmy things

total sc length / SC compaction and telomere bias in positioning are conserved / consistantly sexually dimorphism (in these mice?)  for most species
