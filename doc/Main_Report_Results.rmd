...
as.data.frame(round(summary(MLH1.MM.M1)$tTable, 6))
...
#HQ var.CO
M1.var_co <- lme( var ~ subsp * sex, data=DF.HetC.MixedModel.HQ, random=list(strain=pdDiag(~sex) ) )

#summary(HetC.MixedModel.HQ.var_co)

as.data.frame( round(summary(M1.var_co)$tTable, 6))

#ranef(HetC.MixedModel.HQ.var_co)

#CV
M1.cv_co <- lme( cV ~ subsp * sex, data=DF.HetC.MixedModel.HQ, random=list(strain=pdDiag(~sex) ) )
...
```{r M1.Q12_table, echo=FALSE}
#use the Q1Q2 calq'd table

#Q12_mouse_table_w.Ages$subsp <- ifelse(grepl(1, Q12_mouse_table_w.Ages$subsp), "Dom", 
#        ifelse(grepl(2, Q12_mouse_table_w.Ages$subsp), "Musc", Q12_mouse_table_w.Ages$subsp))

#DF.HetC.MixedModel.HQ <- DF.HetC.MixedModel.HQ[!(is.na(DF.HetC.MixedModel.HQ$mouse) | #DF.HetC.MixedModel.HQ$mouse==""), ]

Q12_mouse_table_w.Ages <- add_subsp(Q12_mouse_table_w.Ages)

#need to change to chr first
...
M1.Q12_var <- lme( var ~ subsp * sex, data=Q12_mouse_table_w.Ages, random=list(strain=pdDiag(~sex) ) )

as.data.frame( round(summary(M1.Q12_var)$tTable, 6))


M1.Q12_CV <- lme( cV ~ subsp * sex, data=Q12_mouse_table_w.Ages, random=list(strain=pdDiag(~sex) ) )

as.data.frame( round(summary(M1.Q12_CV)$tTable, 6))
...
```{r M2.var, echo=FALSE}
#this model must be the same as M! -- since all the results are the same
M2.var <- lme(var ~ subsp * sex, data=DF.HetC.MixedModel.HQ, random = ~1|strain) 

as.data.frame( round(summary(M2.var)$tTable, 6))

#summary(HQjust.strain.var_co)

M2.CV <- lme(cV ~ subsp * sex, data=DF.HetC.MixedModel.HQ, random = ~1|strain) 

as.data.frame( round(summary(M2.CV)$tTable, 6))

```



```{r M2.var.Q12, echo=FALSE}
M2.Q12_var_co <- lme(var ~ subsp * sex, data=Q12_mouse_table_w.Ages, random = ~1|strain) 

as.data.frame( round(summary(M2.Q12_var_co)$tTable, 6))

M2.Q12_CV <- lme(cV ~ subsp * sex, data=Q12_mouse_table_w.Ages, random = ~1|strain) 

as.data.frame( round(summary(M2.Q12_CV)$tTable, 6))
```


$$mouse .var CO ~=~ subsp * sex  + \varepsilon$$
```{r M3.var, echo=FALSE, eval=FALSE}
M3.var <- lm(var ~ subsp * sex * strain, data=DF.HetC.MixedModel.HQ)
as.data.frame( round(summary(M3.var)$tTable, 6))

M3.cv <- lm(cV ~ subsp * sex * strain, data=DF.HetC.MixedModel.HQ)
as.data.frame( round(summary(M3.cv)$tTable, 6))
#these models won't work
```


```{r M3VAR.Q12, echo=FALSE, eval=FALSE}
#ugh, don't know how to get these to work 
M3.Q12_var <- glm(var ~ subsp * sex * strain, data=Q12_mouse_table_w.Ages)

as.data.frame(summary(M3.Q12_var)$tTable, 6)
#results don't show

M3.Q12_cv <- lm(cv ~ subsp * sex * strain, data=Q12_mouse_table_w.Ages)

as.data.frame( round(summary(M3.Q12_cv)$tTable, 6))
```


```{r M4.var, echo=FALSE, eval=FALSE}
M4.var <- lm(var ~ sex * strain, data=DF.HetC.MixedModel.HQ)

as.data.frame( round(summary(M4.var)$tTable, 6))

M4.cv <- lm(cV ~ sex * strain, data=DF.HetC.MixedModel.HQ)

as.data.frame( round(summary(M4.cv)$tTable, 6))
```


```{r M4.var_Q12, echo=FALSE, eval=FALSE}

M4.var_Q12 <- lm(var ~ sex * strain, data=Q12_mouse_table_w.Ages)
as.data.frame( round(summary(M4.var_Q12)$tTable, 6))

#these results not print / working
M4.CV_Q12 <- lm(cV ~ sex * strain, data=Q12_mouse_table_w.Ages)
as.data.frame( round(summary(M4.CV_Q12)$tTable, 6))
```
...
I couldn't get many of the models to return results.
