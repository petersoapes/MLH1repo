...
These results confirm that quality has a significant effect on MLH1 count per cell. How this affects further inferences is a little unclear since the quality score is a very rough indicator of technical error.
...
The mouse 22jun15_G_m2 has strange pattern, investigate the Python SC outputs. Some of the pattern goes away when redo.crop is set to equal 'no'.  
...
The brackets in the plots above reflect the output of 'compare_means()' which uses a wilcox.test. I double checked the wilcox p values for wilcox.test and compare_means, they're similar.
...
> Main Point
 ...
 - Qualitatively, it doesn't appear that there is a maternal age effect, but these data are underpowered.
...
Plotting euth date and mean CO number. 
