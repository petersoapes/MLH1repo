...
load(file="C:/Users/alpeterson7/Documents/MLH1repo/data/total_SC_30.12.19.RData")
...
The original plots weren't very effective, so I removed them. I made 

## Quality effect on SC
...
#merge
ll <- merge(mouse.level.skel, AP_mouse_table_w.Ages, by = "mouse")
...
ll.plot <- ggplot(ll, aes(y=mean.skel, x=euth.date.2, color=(strain.y) ))+geom_point()+
  scale_color_manual(values=colors_of_strains)+ #this list in setupdata
  geom_smooth(method='lm',alpha=0.1, size=0, span=0.5 )+ggtitle("")+facet_wrap(~sex.y)


ll.month.plot <- ggplot(ll, aes(y=mean.skel, x=month.2, color=(strain.y) ))+geom_point()+
  scale_color_manual(values=colors_of_strains)+ #this list in setupdata
  geom_smooth(method='lm',alpha=0.1, size=0, span=0.5 )+ggtitle("")+facet_wrap(~sex.y)
