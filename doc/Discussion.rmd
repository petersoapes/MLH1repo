...
To place our results in the broader context of meiotic variation we examine three modifier models and two functional and cell physiology models (table X). While there are challenges to fitting models built under different assumptions, we feel it's a useful exercise to highlight any unifying patterns for our specific traits across exsisting models. The three modifier models were built to explain variation in genome wide recombination rates between sexes (heterochiasmy), while the chromosome ocillatory movement (COM) model was built to describe positive interference. This spindle based selection model was developed to explain the results resented in this manuscript. In cases where the models can't be extended to predicting results, we list a 'NA' for the prediction.
...
Our results from musculus^MSM^ and musculus^PWD^ demonstrate that the SC length/chromatin compaction are uncoupled from the direction of heterochiasmy in house mice. These results slightly depart from predictions which nominate chromatin compaction/organization (axis length) as the primary driver of heterochiasmy [@petkov2007]. 

The relationship may hold for more general heterochiasmy patterns across sexes but fail to fully explain rapid evolution in the sex specific context.  (this can be attributed to distinct evolutionary trajectories due to sex specific meiotic context)

Our results indicate the sexual dimorphism in chromatin compaction is conserved in house mouse (e.g. females have longer SC than males) and that the rapid male-specific evolution in crossover number per cell proceeded through another aspect of crossover regulation. 
