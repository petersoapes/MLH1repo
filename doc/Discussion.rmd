...
This is how we are going to test/ synthesize / fit our results into the literuature and test hypothesis for which models best explain our results. In our analysis / synthesis of results, we decded to systematically compared models which have been proposed to explain our results (both for the sex dfferences and the faster male evolution of recombination rates). We attempted to test the hypotheses of if our choosen models could produce the main / general patterns oberved in this study (Table X)

We generally focus on models for the evolution of heterochiasmy since that is the primary data/set of results in this study. We search for a parsimonius explanation /model that fits both the results from sex differences and the faster male evolution. But we also test the male specific traits.
...
Below are the main traits from our results: more between cell variance in females, chromatin compaction (with consequence for interference strength), and broad scale landscape differences.
...
#1. chromatin organization, female shorter loops, longer axis

#2. A consequence of chromatin organization (loop size) is difference in interference strength in (MB / cM)


#seen in humans, mice (signal comes through even with cell to cell variation in chromatin compaction / organization)
#

#exceptions to female pattern (fish birds), lack of data
# interference strength reversal (At and cattle) -- we predict that the loop size will also reverse, -- but there is a lack of cytological data for both sexes.

#lots of data on genetic maps for both sexes, but lack of cytological data to support

#our results, female longer axis, 
#review caveats -- why we believe the pattern is true

#refs
#BF klecner?
#negative relationship of axis and loop size (Lynn? Gruhn?)
#global re-orgnaiziton of DNA / chromosomes (3D, new Chip-seq pubs)
```


1. basics and expectations for chromatin organization

2. Well supported model for interference is that it is mediated / communicated through the 2D chromatin axis or SC (depending on the timing)
...
The relationship // molecular organization of chromosome -- undergoes global re-orgzniation during meiosis. The basic take away is that eraly stage in meiosis) there is a central axis made up of chromatin and proteins while most of the DNA / chromatin are folded into loops that emenate off of the axis. This structure has been called 'lampbrush' by some (cite).


A pattern that has been long noticed --


Merging of two types of data

-total chromatin structure (axis length) (cytology)

-Interfere measured as cM or DNA base pairs Mb  (the spacing between markers from linkage map)
...
**lack of difference in the raw measures of IFD, but significant difference in **


The pattern of interference across raw and normalized measures -- also confirms previous results of male having stronger interfere -- due to the longer loops (Petkov) **these results mirror those seen in** [@petkov2007]
...
#revrsed heterochiasmy direction
...
Some of the traits related to faster male evolution -- distinguish in which models can redict them: 
