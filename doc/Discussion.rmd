...
Our results of greater between cell variation in females, these results are supported by human and mouse [@lynn2002, @gruhn2013].
...
In the **spindle based** selection model, the less efficient SAC in oocytes could lead to relaxed selection on the chromosome structure. If the SAC/checkpoint which normally acts in a stabilizing selection on the chromosome structure has a reduction in efficiency (relaxed selection), will result in increased variance in (these associated traits), not just across strains (populations), but across cells within individuals (quant-gen ref?). Oocytes at metaphase I, have an acentriolar spindle comprised of  diffuse network of microtubules with multiple microtubule organizing centers (aMTOCs) (Schuh Ellenburg 2007) which result in 
...
- which might have a consequence of reducing the (reaction/signal of the SAC (anaphase promoting complexes (cite) )).  (this is another way the spindle based selection model is linked to conserved sex differences in gametogenesis (cite))

<Bennabi et al 2016 --spindle assembly inoocytes, sun and kim 2011 -- SAC and regulators, Kolano et al 2012 --error prone oocytes, Manil-Segalen -- forces through MT breaking chromsomes>
Mogessie et al 2018 -- meiotic spindle (big)review>
(what generates MT nucleation is unclear (aMTOC or chromsoomes))
