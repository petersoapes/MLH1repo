...
```{r missing.mice, echo=FALSE, warning=FALSE}
...
Well, we have to talk about the subjective nature that goes into scoring the MLH1 foci. Sources of error or variance across scoring come from background noise (false positives), faint foci (false negatives) and just human error.
A general anecdote is that if the quality of the cell staining / image is high, then the repeatability is near 100%. (using some of Beth's data).
Todo: track down, re-count of Beth's data,  cells that were counted first then in batch 0 (anonimized)
...
The main take away is that higher quality scored cells have lower error in MLH1 counts.
...
```{r quant.repeatability_rep1, echo=FALSE, include=TRUE}
...
#i don't know how to compare quality in more 
...
```{r repeatability_rep2, echo=FALSE, include=FALSE, eval=FALSE}
...
```{r repeatability_rep3, echo=FALSE, include=FALSE}
...
This data has `r length(chrm.class.DF$Original.Name)` cells across these categories

`r kable(table(droplevels(chrm.class.DF$category)))`

More cells are planned to be quantified.

```{r show.hist_error_plot, echo=FALSE,  fig.width=10, fig.height=5, warning=FALSE}
hist_error_plot
```


From this histogram we see most of the cells in this dataset have a difference of 2 in the MLH1 count scores (`r length(chrm.class.DF$Original.Name[( abs( chrm.class.DF$MLH1.dif) <= 2)]) / length(chrm.class.DF$Original.Name)`).

```{r show.quality_error_plot, echo=FALSE,  fig.width=10, fig.height=5}
quality_error_plot
```


There is a positive relationship with the absolute difference and quality score (where 1 is best and 5 is worst quality) (i.e. poorer quality cells are more likely to have more error acorss multiple counts). The proportion of the cells which had the same count across the counting instances is `r length(chrm.class.DF$Original.Name[chrm.class.DF$same_count == "TRUE"]) / length(chrm.class.DF$Original.Name)`. The correlation between the two versions of MLH1 counts for the same cell is `r cor(chrm.class.DF$nMLH1.foci, chrm.class.DF$MLH1.sum.check)`.

```{r show.error_count_plot, echo=FALSE,  fig.width=10, fig.height=5}
error_count_plot
```
There doesn't seem to be a bias across cells with more or less foci. 


Modeling this relationship; (still in progress)
...
error associated with 'quality of cels'
...
(try looking at <5 quality scores?)
...
I will try doing a logistic regression for embryonic vs neonate. The embyonic estimates likely aren't very accurate, and there won't be good sampling across time points.
...
There seems to be a negative correlation with 
...
3 different rooms used to house mice. Access softmouse to be fixed. I should have some data on when the switch was. This would be limited to, WSB, LEW and PWD
...
# Initial Results (change these colors)
...
ff2 <- ggplot(MLH1_data[MLH1_data$species == "M.musculus", ])+
 ...
 ylim( c((min(MLH1_data$adj_nMLH1.foci)-3) ), (max(MLH1_data$adj_nMLH1.foci)+3)) + 
  facet_grid(subsp~ sex, scales="free_x")+scale_color_manual(values=colors_of_strains)
...
> Mouse average Models
  ...
  variables vs char coding of coefficents(opposite direction of sex)?
  select1 (remove old mice, remove mice with low ncells)
  select2 make sure there are equal numbers of observations
  select3 low Musc males
  
  
the pooled cell models are breaking for some reason, (the sexes)
-pooled cells (nMLH1)
  all 
  pooled.HQ (1-4)
  pooled.HQ.2 (sample equal numbers of cells)


```{r MxMdl.DF.setup, echo=FALSE, warning=FALSE, eval=TRUE}
library(lme4)
library(nlme)
...
#if(grepl("MSM", DF.HetC.MixedModel2$strain) ){ DF.HetC.MixedModel2$subsp <- "Mol" }

#subsp are charecters
#change sex to charecter? (unordered sex)

DF.HetC.MixedModel2$sex <- as.character(DF.HetC.MixedModel2$sex)

model.ff2 <- lme(mean_co ~ subsp * sex, data=DF.HetC.MixedModel2, random=list(strain=pdDiag(~sex) ) )#adding mol 
...
#intercept should be the Dom female mean, looks right
mean(DF.HetC.MixedModel2$mean_co[(DF.HetC.MixedModel2$sex == "female" &
                                    DF.HetC.MixedModel2$subsp == "Dom" )] ) #26.5
...
```{r mixed.model.make, echo=FALSE, warning=FALSE, eval=FALSE, message=FALSE}
#build mixed model
...
#L,Q,C  appear for ordered factors in the summary mean lme doesn't believe you used ordered factors
#change all the DFs / variables to UNORDERED factors!
#redo the orders just to be sure
#sex, strain, subsp,

DF.HetC.MixedModel$sex <- factor(DF.HetC.MixedModel$sex, ordered = FALSE, levels =c("female", "male") )

DF.HetC.MixedModel$subsp <- factor(DF.HetC.MixedModel$subsp, ordered = FALSE, levels =c( "Dom", "Musc") )

DF.HetC.MixedModel$strain<- factor(DF.HetC.MixedModel$strain, ordered = FALSE, levels =c( "WSB", "G", "LEW",
                                                  "PWD", "MSM", "SKIVE", "KAZ") )
...
1. mean_co --DF.HetC.MixedModel
2. mean_co.chr  variables coded as factors (for ggplots), how is this changed when they are coded as chr
3. mean_co.fac -- non-ordered factors
4. mean_co.HQ
5. mean_co.HQ.mol
6. Sub-sampling for equal numbers
...
DF.HetC.MixedModel.chr <- DF.HetC.MixedModel
DF.HetC.MixedModel.chr$subsp <- as.character(DF.HetC.MixedModel$subsp)
DF.HetC.MixedModel.chr$sex <- as.character(DF.HetC.MixedModel$sex)
DF.HetC.MixedModel.chr$strain <- as.character(DF.HetC.MixedModel$strain)
...
#mean_co.chr.HQ.mol
DF.HetC.MixedModel.chr.HQ.mol <- DF.HetC.MixedModel.HQ.mol

DF.HetC.MixedModel.chr.HQ.mol$subsp <- as.character(DF.HetC.MixedModel.chr.HQ.mol$subsp)
DF.HetC.MixedModel.chr.HQ.mol$sex <- as.character(DF.HetC.MixedModel.chr.HQ.mol$sex)
DF.HetC.MixedModel.chr.HQ.mol$strain <- as.character(DF.HetC.MixedModel.chr.HQ.mol$strain)


```


```{r MixedModels.stff, echo=FALSE}
#main mean.CO
HetC.MixedModel.mean_co <- lme(mean_co ~ subsp * sex, data=DF.HetC.MixedModel, random=list(strain=pdDiag(~sex) ) )

#mean.co.chr
HetC.MixedModel.mean_co.chr <- lme(mean_co ~ subsp * sex, data=DF.HetC.MixedModel.chr, random=list(strain=pdDiag(~sex) ) )

#mean.co.HQ
HetC.MixedModel.HQ <- lme(mean_co ~ subsp * sex, data=DF.HetC.MixedModel.HQ, random=list(strain=pdDiag(~sex) ) )

#mean.CO.HQ.mol
HetC.MixedModel.HQ.mol <- lme(mean_co ~ subsp * sex, data=DF.HetC.MixedModel.HQ.mol, random=list(strain=pdDiag(~sex) ) )

#mean.HQ.mol.chr
HetC.MixedModel.chr.HQ.mol <- lme(mean_co ~ subsp * sex, data=DF.HetC.MixedModel.chr.HQ.mol, random=list(strain=pdDiag(~sex) ) )

```

The intercept level is Dom female. These are the coefficients



`r kable(HetC.MixedModel.mean_co$coefficients$fixed, col.names = "coefficients")`
...
`r kable(HetC.MixedModel.mean_co.chr$coefficients$fixed, caption="Chr coded Model", col.names = "coefficients")`

`r kable(ranef(HetC.MixedModel.mean_co.chr) )`

The variable names now appear. The values are now quite different. Male, sex coefficient is negative. The fixed sex coefficient is the sex effect independant of subspecies or strain.
...
The factor coded DF the more high quality cells.


`r kable(HetC.MixedModel.HQ.mol$coefficients$fixed, caption="Main model with molossinus", col.names = "coefficient")`
`r kable(ranef(HetC.MixedModel.HQ.mol) )`
With adding molossinus, the subsp names are listed.


`r kable(HetC.MixedModel.chr.HQ.mol$coefficients$fixed, caption="Main model, chr coded with molossinus", col.names = "coefficients")`
`r kable(ranef(HetC.MixedModel.chr.HQ.mol) )`

When the variables/levels are coded as chr, the interaction effects get larger and the other subspecies get smaller. The base sex effect is also larger. The subsp effect is relative to Dom female, 


I don't think the subspecies and strains should be order, (maybe factors still makes sense.)


This table doesn't display well

`r table(DF.HetC.MixedModel.pooled$sex,DF.HetC.MixedModel.pooled$strain)`

KAZ, LEWES has ~100 more for males, PWD has ~200 for females.


Fixed effects

look at the p-values for each of the effects as an indication if they are significant predictors.
...
data has been collection for RWang's total SC. These results are consistant with the patterns seen in the bivalent level resutls. (total SC PWD > KAZ )
