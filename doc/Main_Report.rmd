...
```{r MxMdl.DF.setup, echo=FALSE, warning=FALSE}
library(lme4)
library(nlme)
...
#remove Castaneus (not females)
DF.HetC.MixedModel <- DF.HetC.MixedModel[DF.HetC.MixedModel$subsp.x != "Cast",]
#remove musc strains
...
DF.HetC.MixedModel2$subsp <- ifelse(grepl("MSM", DF.HetC.MixedModel2$strain), "Mol", 
        ifelse(grepl("MOLF", DF.HetC.MixedModel2$strain), "Mol", DF.HetC.MixedModel2$subsp))

DF.HetC.MixedModel2$subsp <- ifelse(grepl(1, DF.HetC.MixedModel2$subsp), "Dom", 
        ifelse(grepl(2, DF.HetC.MixedModel2$subsp), "Musc", DF.HetC.MixedModel2$subsp))
...
#male effect is -2
#Musc and mol effec is almost -1
...
str(DF.HetC.MixedModel)

#remove ncells <10

DF.HetC.MixedModel <- DF.HetC.MixedModel[DF.HetC.MixedModel$Ncells > 9,]
DF.HetC.MixedModel <- DF.HetC.MixedModel[DF.HetC.MixedModel$age.weeks < 25,]
...
`r table(DF.HetC.MixedModel$sex, DF.HetC.MixedModel$strain)`


```{r mixed.model,make, echo=FALSE, warning=FALSE}
#build mixed model
...
#L,Q,C  after the factors in the summary mean lme doesn't believe you used ordered factors

#redo the orders just to be sure
#sex, strain, subsp,

DF.HetC.MixedModel$sex <- factor(DF.HetC.MixedModel$sex, ordered = TRUE, levels =c("female", "male") )

DF.HetC.MixedModel$subsp <- factor(DF.HetC.MixedModel$subsp, ordered = TRUE, levels =c( "Dom", "Musc") )

DF.HetC.MixedModel$strain<- factor(DF.HetC.MixedModel$strain, ordered = TRUE, levels =c( "WSB", "G", "LEW",
                                                  "PWD", "MSM", "SKIVE", "KAZ") )
...
DF.HetC.MixedModel$mean_co <- as.integer(DF.HetC.MixedModel$mean_co)

DF.HetC.MixedModel <- droplevels(DF.HetC.MixedModel)
DF.HetC.MixedModel <- DF.HetC.MixedModel[!(is.na(DF.HetC.MixedModel$mouse) | DF.HetC.MixedModel$mouse==""), ]

#female Dom are the first levels:
#Intercept, 25.767065 mean for Dom female -- NS
#
#make sure the default (ordered factors of strains is set)
#check level order etc.
#build initialize model
model.ff <- lme(mean_co ~ subsp * sex, data=DF.HetC.MixedModel, random=list(strain=pdDiag(~sex) ) )

#pull out values / other things
summary(model.ff)# indeces for this object: [1]-structure
ranef(model.ff)
