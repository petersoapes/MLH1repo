...
Q12_MLH1_means_males <- ddply(.data=Q12_MLH1_DF[Q12_MLH1_DF$sex == "female",], 
                 .(category,subsp),
                 summarize, 
                 ncells = length(unique(Original.Name)),
                 nmice = length(unique(mouse)),
...
tot.DF_MLH1_means_males <- ddply(.data=MLH1_data[MLH1_data$sex == "female",], 
                 .(category,subsp),
                 summarize, 
                 ncells = length(unique(Original.Name)),
                 nmice = length(unique(mouse)),
...
Ranges for male means across subspecies. Ranges of means (not ranges of all).

(try turning this into a table...)
Dom            female Dom
old,   new      old     new
22-24  23-25    24-27  25-29 

Musc           female Musc
23-31   25-31   25-27   26-29
22-25   22-27    25       25
(ASTmale has 27 mean, outlier)

Outgroup       female outgroup
25-26   26-27    27    29-31

I think the ranges across the groups 
