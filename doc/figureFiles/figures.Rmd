...
# Option 1, strain averages, sexes matched
...
Fig1_Bv2_strain.male <- ggplot(strain.table.HQ[strain.table.HQ$sex == "male",], aes(y=mean_co, x=strain, color=strain))+facet_grid(~sex, scales = "free", space="free")+ 
  ylab("")+
  geom_point(size=3)+
  geom_errorbar(aes(ymin=mean_co-(se*2), ymax=mean_co+(se*2) ), width=.3)+
  ylim(c(18, 35))+
...
#remove y labfor male 
  #Dom
  annotate("segment", x=.75, xend=3.6, y=19.5, yend=19.5, colour="black", size=1)+
  #Musc
  annotate("segment", x=3.9, xend=6.15, y=19.5, yend=19.5, colour="black", size=1)+
  #mol
  annotate("segment", x=6.5, xend=8.1, y=19.5, yend=19.5, colour="black", size=1)+
  #label
  annotate("text", x = c(2.2,5.9,6.1), #list of all x points 
           y = c(18.5,18.5, 33),
...
# 2
Fig1_Bv2_strain.female <- ggplot(strain.table.HQ[strain.table.HQ$sex == "female",], aes(y=mean_co, x=strain, color=strain))+facet_grid(~sex, scales = "free", space="free")+ ylab("Average MLH1 Foci per Cell")+
  geom_point(size=3)+
  ylim(c(18, 35))+
  geom_errorbar(aes(ymin=mean_co-(se*2), ymax=mean_co+(se*2)), width=.3) +
  scale_color_manual(values=colors_of_strains)+ 
...
#Labels
 annotate("text", x = c(2.25,5.9,6.1), #list of all x points 
           y = c(18.5,18.5,33),
           label = c("M. m. domesticus","M. m. musculus", "M.m.molossinus"),
           fontface = 'italic', hjust = 0.5, size=3.5)
...
fig1_B_v2  <- ggdraw() +
  draw_plot(Fig1_Bv2_strain.female, x=0, y = 0, width=.47, height=.97) +
  draw_plot(Fig1_Bv2_strain.male, x=.49, y=0, width=.48, height=.97) +
...
Molf female data point is missing because there is only 1 female, so NAs were generated.
Mol text should be moved back above the points.


## Option 2, strain averages, more strains
...
#MOl
#MSM, MOLF
strain.table.HQ.backup$strain<- factor(strain.table.HQ.backup$strain, ordered = TRUE, levels =c( "WSB", "G", "LEW", "PERC",
                           "PWD", "SKIVE", "KAZ", "TOM", "AST","CZECH",
                                                    "MSM", "MOLF",
                                          "CAST", "HMI",
                             "SPRET", "SPIC", "CAROLI", "F1", "other") )
...
Fig1_B_strain.male <- ggplot(strain.table.HQ.backup[strain.table.HQ.backup$sex == "male",], aes(y=mean_co, x=strain, color=strain))+facet_grid(~sex, scales = "free", space="free")+ 
  ylab("")+
  geom_point(size=3)+
