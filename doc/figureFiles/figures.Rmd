...
cell.output <- readPNG('C:/Users/alpeterson7/Documents/WildMiceMadison/Images/whole_cell_example.png')
...
spermatocyte.auto <- ggplot(empty.DF, aes(x = col1, y = col2)) +
  background_image(cell.output) + theme(axis.title.x=element_blank(),
...
library("cowplot")
fig1  <- ggdraw() +
  draw_plot(MLH1.fig1, x = 0, y = .42, width=1, height=.55) +
  
  draw_plot(Better_MAD_map, x=0, y=0, width=.4, height=.4) +
  
  draw_plot(spermatocyte, x=.38, y = 0, width=.31, height=.39) +
  
  draw_plot(spermatocyte.auto, x=.69, y = 0, width = .31, height = .39) +
  
  draw_plot_label(label = c("A", "B", "C"), size = 15,
                  x = c(0, 0, .36), y = c(1, .43, 0.43) )
...
Fig1_Bv2_strain <- ggplot(strain.table.HQ, aes(y=mean_co, x=strain, color=strain, shape = sex))+
  ...
  geom_point(aes(shape = sex), size=3.7, position=position_dodge(.5))+
