...
because of chromosome size effect on CO position and rate, I'd like to try seperate bins of the 'loss' bivalents
how does within cell variance in SC length compare across cells?


```{r chrm.size.effect, echo=FALSE}

show.the.cells_females <- ggplot(whole.cell.data_manual[whole.cell.data_manual$sex == "female",], aes(x=fileName, y= SC.length, color=strain))+geom_point()+ facet_wrap(~strain, scales="free")+ylim(c(30,200))+ggtitle("Female whole cell manual measures")
show.the.cells_females
#PWD seems to have the biggest variance across cells

#the min is pretty stable

show.the.cells_male <- ggplot(whole.cell.data_manual[whole.cell.data_manual$sex == "male",], aes(x=fileName, y= SC.length, color=strain))+geom_point()+ facet_wrap(~strain, scales="free")+ylim(c(30,150))+ggtitle("Male whole cell manual measures")
show.the.cells_male
...
show.the.almost_cells_DOM_m <- ggplot(almost_whole_cell_DOM[almost_whole_cell_DOM$sex == "male",], aes(x=fileName, y= chromosomeLength, color=strain))+
  geom_point()+ facet_wrap(~strain, scales="free")+ylim(c(30,150))+ggtitle("Male whole cell BivData measures")

show.the.almost_cells_DOM_m
```


given the distributions of sc lengths within cells -- do I think I can predict a random SC's length (rank)
