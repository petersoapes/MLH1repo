...
```{r long.chrm.dataset, echo=FALSE, eval=FALSE}

#adding the extra cols, messed up this part

#how many biv's are from cells with >15 bivalents

al.15_bivs <- cell.level.BivData_table[cell.level.BivData_table$Nbiv >15, ]#150 cells have al 15

long_BivData_set <- Curated_BivData[ Curated_BivData$fileName %in% al.15_bivs$fileName, ] #across those 150 cells are 2654 biv measures

#table(potential.data.set$category)
long_BivData_set$fileName <- as.character(long_BivData_set$fileName)
Biv_Data_mean_table$fileName <- as.character(Biv_Data_mean_table$fileName)




#pull out the biv's which exceed the cell's Q4
#for each line of long_BivData_set
# check if it's SC length is greater than the Q4 for the given cell

long_BivData_set99 <- long_BivData_set

long_BivData_set99 <- long_BivData_set99[1:50,]
#3 unique fileNames, in abbridged DF


ifelse(grepl("PWD", mouse.avs_4MM_male$strain), 1, 
                                    ifelse(grepl("MSM", mouse.avs_4MM_male$strain), 1, 0))

newfile = strsplit(i, "_")[[1]][1:7]

Biv_Data_mean_table[grepl(newfile, Biv_Data_mean_table$fileName )]

#i is obj.id, this returns a table of all falses -- because it's look for the whole string

o<- strsplit(i, "_")

Biv_Data_mean_table$fileName[which(grepl(newfile, Biv_Data_mean_table$fileName ) )]
#above returns the BivData, filename which the grepl matches

Biv_Data_mean_table$Qar.4[which(grepl(newfile, Biv_Data_mean_table$fileName ) )]
#this returns the Q4

#this will put the cell's Q4
#I think I need a counter
counter = 1

length(duplicated(long_BivData_set99$Obj.ID))#there are 2605 duplicated Obj.ID
anyDuplicated(long_BivData_set99$Obj.ID)
dup.curated <- long_BivData_set99[duplicated(long_BivData_set99$Obj.ID),]#65 duplicated obj.ID
long_BivData_set99 <- long_BivData_set99 %>% distinct()

#REDO
#61 unique biv.id

#this must be charecter for str split



long_BivData_set99$Obj.ID <- as.character(long_BivData_set99$Obj.ID )
long_BivData_set99.mini <- long_BivData_set99[905:2605,]

long_BivData_set99.mini$Obj.ID <- paste(long_BivData_set99.mini$fileName, long_BivData_set99.mini$boxNumber, sep = "_")


anyDuplicated(long_BivData_set99.mini$Obj.ID)#1093
dup.mini.curated <- long_BivData_set99.mini[duplicated(long_BivData_set99.mini$Obj.ID),]

#do a pipe remove
long_BivData_set99.mini <- long_BivData_set99.mini[ ! long_BivData_set99.mini$fileName %in% dup.mini.curated$fileName, ]
#think this removes the duplicated rows


#IT COULD BE THAT THERE are filenames in the Biv_Data_mean_table which don't have the 

#make sure the biv.mini only have filenames which are also in the mean teabl
long_BivData_set99.mini <- long_BivData_set99.mini[ ! long_BivData_set99.mini$fileName %in% dup.mini.curated$fileName, ]


long.data.set.cells <- al.15_bivs

long_BivData_set99.mini <- long_BivData_set99.mini[long_BivData_set99.mini$fileName %in% long.data.set.cells$fileName, ]

long_BivData_set99.mini <- long_BivData_set99.mini[long_BivData_set99.mini$fileName %in% Biv_Data_mean_table$fileName, ]
#removed fileNames which don't appear in Biv_Data_mean_table (which has the Q4)


#long_BivData_set99.mini <- long_BivData_set99[905:2605,]

for(  i in 1:length( unique(long_BivData_set99.mini$Obj.ID) ) ){
  newfile.str = strsplit( long_BivData_set99.mini$Obj.ID[i], "_")[[1]][1:7]
  ...
  long_BivData_set99.mini$cell.Q4.99[i] <- the.Q4
  #these are just using the last index, 
    
  the.FN <- Biv_Data_mean_table$fileName[which(grepl(new.egg, Biv_Data_mean_table$fileName ) )]
  
  long_BivData_set99.mini$Q4.FN[i] <- the.FN
  
  print(c(i, the.FN, the.Q4))

}
#some kind of error at 903 -- the loop should continue past it


long_BivData_set99.mini$FN2 <- long_BivData_set99.mini$fileName
#mini, looks good so far

long_BivData_set99.mini$long.Biv <- ifelse( (long_BivData_set99.mini$chromosomeLength >= long_BivData_set99.mini$cell.Q4), "yes", "no" )

long_BivData_set99.mini$wtf <- long_BivData_set99.mini$chromosomeLength - long_BivData_set99.mini$cell.Q4


real.long.bivData <- long_BivData_set99.mini[long_BivData_set99.mini$long.Biv == "yes",]
...
#OG try
#for(  i in unique(long_BivData_set99$Obj.ID) ){
#  newfile = strsplit(i, "_")[[1]][1:7]
#  the.Q4 <- Biv_Data_mean_table$Qar.4[which(grepl(newfile, Biv_Data_mean_table$fileName ) )]
#  long_BivData_set99$cell.Q4.99[counter] <- the.Q4
#  #these are just using the last index, 
#  the.FN <- Biv_Data_mean_table$fileName[which(grepl(newfile, Biv_Data_mean_table$fileName ) )]
#  long_BivData_set99$Q4.FN[counter] <- the.FN
#  print(c(i, the.FN, the.Q4))
#  counter = counter+1
#}
#not sure what the final error is in regards to
#the file name matches are wronge
...
Ugggg, still something wrong,
9apr18_1feb18_LEW_f1_sp1_9_rev

 0%    25%    50%    75%   100% 
 77.00 114.50 136.00 152.75 212.00 


Started compiling the long_biv data set, some cells have more than I expect: 

13apr15_1apr15_PWD_f2_sp1_4_rev, 5aug17_14jul17_MSM_f1_sp1_12_rev, 5aug17_14jul17_MSM_f1_sp1_26_rev, 30jun17_4jan17_LEW_f3_sp1_1_rev, 8feb17_4jan17_LEW_f1_sp1_5_rev, 8feb17_4jan17_LEW_f1_sp1_3_rev, 9apr18_1feb18_LEW_f1_sp1_9_rev, 8jun15_23apr15_PWD_f2_sp1_17_rev

They have like ~10, it might have to do with not haveing = in the >=

Qar.4 = quantile(chromosomeLength)[[4]],
quantile(Curated_BivData$chromosomeLength[Curated_BivData$fileName == "8feb17_4jan17_LEW_f1_sp1_3_rev"]) #148.00
Curated_BivData$chromosomeLength[Curated_BivData$fileName == "8feb17_4jan17_LEW_f1_sp1_3_rev"]


9apr18_1feb18_LEW_f1_sp1_9_rev


the correct Q4 is 148, but the value in the Q4 col is 116, and the filName listed is completely wrong


The dataset is unbalanced in some ways -- integrating manual measures might fix. MSM under rep, since most cell had high overlap.
...
3) Interference, measured in the physical units, has been shown to lack sexual dimorphism (petkov).
...
A) **SC lengths** will be longer for high Rec  
...
C) It's unknown if **1CO normalized positions** between high and low musc strains, will differ. (no good predictions come to mind)


D) Sis-co-ten predictions fall into 
...
I used the mixed model with the lme()



$$mouse \ average \ SC  \ length ~=~ subsp * sex + rand(strain) + \varepsilon $$

```{r MM_2_SC.length, echo=FALSE}

#as.data.frame( round(  summary(BivData.MixedModel.HQ)$tTable, 6 ) )

#can't make the 
SC_length.MM <- lmer(mean_SC ~ subsp * sex + (1|strain), data=mouse.avs_4MM, REML = TRUE)

anova(SC_length.MM) 

thing1  <- summary(SC_length.MM)
thing2 <- drop1(SC_length.MM, test="Chisq") 

rand_SC.Length_MM <- exactRLRT(SC_length.MM)
rand_SC.Length_MM
...
The exactRLRT() test indicates that the random strain effect might be significant, p= `r rand_SC.Length_MM$p.value`.
...
#the summary for both these models is the same ... should I be worried?
...
MELT_Curated_BivData <- MELT_Curated_BivData[MELT_Curated_BivData$hand.foci.count <5, ]
# remove the na's
MELT_Curated_BivData <- MELT_Curated_BivData[!(is.na(MELT_Curated_BivData$hand.foci.count) | MELT_Curated_BivData$hand.foci.count==""), ]
...
sis.co.ten_with.plot <- ggplot(data = Curated_BivData[Curated_BivData$category == "PWD female",], aes(y=SisCoTen, x=hand.foci.count, color = as.factor(hand.foci.count) ))+
  geom_jitter()+facet_wrap(sex~strain)
sis.co.ten_with.plot



sis.co.ten_with.plot_2 <- ggplot(data = Curated_BivData[Curated_BivData$category == "PWD male",], aes(y=SisCoTen, x=hand.foci.count, color = as.factor(hand.foci.count) ))+
  geom_jitter()+facet_wrap(sex~strain)
sis.co.ten_with.plot_2
