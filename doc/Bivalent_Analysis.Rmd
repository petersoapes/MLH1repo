...
mouse.av_log.reg_SC.length <- summary(log.reg_SC.length_Musc)


# bivalent level log regression
log.reg_SC.length_Musc_BIV <- glm(Rec.group ~ chromosomeLength, 
                            data=Curated_BivData[(Curated_BivData$subsp == "Musc")& (Curated_BivData$sex == "male"),], family=binomial(link="logit"))

bivalent_log.reg_SC.length <- summary(log.reg_SC.length_Musc_BIV)

```

The mean SC logistic regresion model for mouse averages 

`r mouse.av_log.reg_SC.length$coefficients`  

and for the single bivalent levels  

`r bivalent_log.reg_SC.length$coefficients`
...
T.test_Poly_SC.Len_mouse.av <- t.test(mouse.avs_4MM_male$mean_SC[mouse.avs_4MM_male$Rec.group == 1],
       
  mouse.avs_4MM_male$mean_SC[(mouse.avs_4MM_male$subsp == "Musc")&(mouse.avs_4MM_male$sex == "male")&(mouse.avs_4MM_male$Rec.group == 0)]
)#p-value = 0.001567

T.test_Poly_SC.Len_single.biv <- t.test(Curated_BivData$chromosomeLength[Curated_BivData$Rec.group == 1],
...
The mouse average and single bivalents are significantly longer in the high rec group p = `r T.test_Poly_SC.Len_mouse.av$p.value` and p=`r T.test_Poly_SC.Len_single.biv$p.value`.
