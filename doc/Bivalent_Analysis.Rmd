...
#load Clean BivData
load(file="data/CleanBivData_1.20.20.RData")
...
nBiv.per.cell <- ggplot(cells.BivData_table, aes(nbivs)) + ggtitle("Number of passing bivalents per cell") + geom_histogram(bins = 30) +facet_wrap( ~ category)
nBiv.per.cell
...
Double Check the dataset for the mixed models.
...
#MELT_Curated_BivData.PER

MELT_Curated_BivData.PER <- add_subsp(MELT_Curated_BivData.PER)

Musc.female_MELT_curBivData <- MELT_Curated_BivData.PER[ ((MELT_Curated_BivData.PER$sex == "female") & (MELT_Curated_BivData.PER$subsp == "Musc") ), ]

CO.Pos.density_Musc_female <- ggplot(Musc.female_MELT_curBivData, aes(x=PER_Position, fill=strain) ) + geom_density(alpha=.2) + facet_wrap(~hand.foci.count, ncol=1)+ggtitle("Musc female \nNormalized Foci Positions")
CO.Pos.density_Musc_female


#Dom female
Dom.female_MELT_curBivData <- MELT_Curated_BivData.PER[ ((MELT_Curated_BivData.PER$sex == "female") & (MELT_Curated_BivData.PER$subsp == "Dom") ), ]

CO.Pos.density_Dom_female <- ggplot(Dom.female_MELT_curBivData, aes(x=PER_Position, fill=strain) ) + geom_density(alpha=.2) + facet_wrap(~hand.foci.count, ncol=1)+ggtitle("Dom female \nNormalized Foci Positions")
CO.Pos.density_Dom_female



#Musc male
Musc.male_MELT_curBivData <- MELT_Curated_BivData.PER[ ((MELT_Curated_BivData.PER$sex == "male") & (MELT_Curated_BivData.PER$subsp == "Musc") ), ]

CO.Pos.density_Musc_male <- ggplot(Musc.male_MELT_curBivData, aes(x=PER_Position, fill=strain) ) + geom_density(alpha=.2) + facet_wrap(~hand.foci.count, ncol=1)+ggtitle("Musc male \nNormalized Foci Positions")
CO.Pos.density_Musc_male



#Dom male
Dom.male_MELT_curBivData <- MELT_Curated_BivData.PER[ ((MELT_Curated_BivData.PER$sex == "male") & (MELT_Curated_BivData.PER$subsp == "Dom") ), ]

CO.Pos.density_Dom_male <- ggplot(Dom.male_MELT_curBivData, aes(x=PER_Position, fill=strain) ) + geom_density(alpha=.2) + facet_wrap(~hand.foci.count, ncol=1)+ggtitle("Dom male \nNormalized Foci Positions")
...
Pos.CO.avs <- ddply(MELT_Curated_BivData.PER, c("mouse", "Foci_Type"), summarise,
                        Nmice = length(unique(mouse)),
                        Ncells  = length(Obj.ID),
                        
                    mean_PER_pos = as.numeric(format(round(  mean(PER_Position), 3 ), nsmall=3) )
)
...
Pos.CO.avs_1CO <- Pos.CO.avs[Pos.CO.avs$Foci_Type == "PER_Foci_1",]

Pos.CO.avs_1CO <- Pos.CO.avs_1CO[!(is.na(Pos.CO.avs_1CO$mouse) | Pos.CO.avs_1CO$mouse==""), ]


BivData.CO.PER.POS.MixedModel.HQ <- lme(mean_PER_pos ~ subsp * sex, data=Pos.CO.avs_1CO, random=list(strain=pdDiag(~sex) ) )
...
#F1.x.F2 <- ggplot(MELT_Curated_BivData.PER, aes(x=F1, fill=F2) ) + geom_density(alpha=.4) + facet_wrap(~hand.foci.count)+ggtitle("Normalized Foci Positions")
