...
for(  i in 1:length( long_BivData_set99$fileName)  ){
  long_BivData_set99$cell.Q4[i] <- Biv_Data_mean_table[ which(Biv_Data_mean_table$fileName == long_BivData_set99$fileName[i]), ]
}


for(  k in 1:length( long_BivData_set99$fileName)  ){
long_BivData_set99$above.Q4[k] <- ifelse( (long_BivData_set99$cell.Q4 < long_BivData_set99$chromosomeLength), "yes", "no")
}
#This isn't working either



#for(i in 1:length( long_BivData_set$fileName)  ){ 
  # print(i)
#  current_obj_fileName <- long_BivData_set$fileName[i]
#  current_obj_SC_length <- long_BivData_set$chromosomeLength[i]  
#  current.Q4 <- Biv_Data_mean_table$Qar.4[which(Biv_Data_mean_table$fileName == current_obj_fileName) ]
# current.Q4 <- Biv_Data_mean_table$Qar.4[which(Biv_Data_mean_table$fileName == long_BivData_set$fileName[i]) ]
#  long_BivData_set$above.Q4 <-ifelse( (current.Q4 < current_obj_SC_length), 1, 0)
#  print( paste( c(current.Q4, current_obj_fileName, current_obj_SC_length), sep = "_" ) )
#}
...
There are sex effects for the lme() models for both the ABS and PER IFD measures. There isn't evidence of Random effects from the lmer() models.
...
> Is the Male polymorphism Prediction met? High rec strains have shorter IFDs?
...
> Is there evidence for the alternative IFD meansure


INSERT THE SISCOTEN ESTIMATES!!

```{r siscoten.test, echo=FALSE}
