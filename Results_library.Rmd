...
#load(file = here( "MLH1/MLH1_data_setup_2.24.20.RData") )
...
Crossover interference is one of the major determinates of the CO landscape. We use X number of 2CO bivalents, to test if there are diffferences between males and females in interference. 
The mean interfocal distance (IFD) for raw and normalized values was calculated from X `r ` female and X`r ` male mice.
The raw IFD measures -- are usuful for thinking of crossover interference as a mechnical force mediated through the SC. Previous measures found no sex difference in IFD in B6 mice (de Boer et al )
...
Mean IFD^PER^ measures are highly significant (t.test; p = `r Q1.IFD.PER_FULL_t.test$p.value`) between sexes, with males have stronger crossover interference (in 2CO bivalents)
...
MSM   `r Q1.IFD.PER_t.test_MSM$p.value`
SKIVE  `r Q1.IFD.PER_t.test_SKIVE$p.value`
PWD  `r Q1.IFD.PER_t.test_PWD$p.value`
MSM  `r Q1.IFD.PER_t.test_MSM$p.value`
LEW  `r Q1.IFD.PER_t.test_LEW$p.value`
G  `r Q1.IFD.PER_t.test_G$p.value`
WSB  `r Q1.IFD.PER_t.test_WSB$p.value`
...
(raw measues of IFD, female lengths are weakly significantly longer reflecting the longer axis lengths). When the underlying differences in chromatin compaction are controlled for (IFD^PER^), males have a greater propotion of single bivalents seperating two foci on the same bivalent, indicative of stronger interference.
...
```{r Q1.show.IFD.things, echo=FALSE, eval=FALSE}
...
off. Is there a default expectation for why .25 would be the cut off...(given female uniform CO positioning?  (uniform landscape in females -- predicts a mean 1CO position of .5)

Female landscape / distribution of interference – seems like there might be constraints -- a lower cut off og 25% and an upper cutt off of 75%

there is a cut off of 75% -- it is rare to oberserve bivalents with very strong interference (more than 75% or less than 25% of the chrm separating 2 foci on the same chromsomes) (is this differen in males  -- do their distributions exceed 75% IFD^PER^?)
...
Since the most significant t.test were significantly different across sexes for the NORMALIZED effects -- we follow up with the model framework to --check out the potential evolutionary patterns--


We applied the same frame work of models, Mixed model analysis for IFD (interference), the first set of models are made with the lme() functions.
...
**Linear model M1 ** $$mouse \ average \ IFD^PER^ ~=~ subsp * strain + \varepsilon $$
**Linear model M2 ** $$mouse \ average \ IFD^PER^ ~=~ strain + \varepsilon $$
...
Within the models, PWD and SKIVE seem to have higher support of longer IFD^PER^ compared to MSM
MSM does have longer IFD^PER^ compared to MOLF (and dom strains)

In M1, the model build on the mouse averages of IFD^PER^ metrics with subspecies and strain as fixed effects, the high rec strains had significant strain effects (glm; p = `r sum.Q2.M1glm_IFD.PER$coefficients[,4][6]`, p = `r sum.Q2.M1glm_IFD.PER$coefficients[,4][7]` and p = `r sum.Q2.M1glm_IFD.PER$coefficients[,4][8]`  for PWD, MSM and SKIVE respectively). In M2, the mouse average for normalzied IFD^PER^ is modeled with only strain effects. Only PWD and SKIVE IFD^PER^ had significant strain effects (glm; p = `r sum.Q2.M2glm_IFD.PER$coefficients[,4][4]` and p = `r sum.Q2.M2glm_IFD.PER$coefficients[,4][7]` for PWD and SKIVE respectively).

Additionally when M2 models (for just strain) was run, they confirmed the within subspecies predictions informed by the variation in mean MLH1 foci number. (IFD is the only single bivalent based trait wcich supported these predictions)

no significant strain effects across the _domesticus_ strains (glm; p = `r`).  
_musculus^PWD^_ and _musculus^SKIVE^_ have significantly longer IFD compared to the other strains (glm p= ?, Table). And _molossinus^MSM^_ has significantly longer IFD^PER^ compared to _molossinus^MOLF^_.
...
Given the differences in chromosomes proportions (Figure X), one might expect that --- the rec landscapes for 2CO bivalents in PWD and MSM would be more variable given that a larger proportion of chromsomes belong to this class (60% to 50%) (there are more opportunities for chromosome specific effect to affect the landscape). Instead the increased variation in the 2CO rec landscape is seen in the lower rec strains (which have on average 30% 2COs. However, we determine that the lower mean IFD^PER^ in these strains is driven by an enrichment of 'close foci', IFD^PER^s less than 30% of the total SC in low rec strains. The rate of these close foci range from 8.2% ( _domesticus^G^_ ) to 16% ( _musculus^KAZ^_ ) in the low rec strains, while the high strains all had rates under 5% (0%, 1.3%, and 3.3% for _musculus^SKIVE^_, _molossinus^MSM^_ and _musculus^PWD^_ respectively). <no chi sqaure test see above>
...
**the difference in interference measures is mainly driven by close foci in the 2COs of low rec strains**
